...
$sql = "select UserName,Money,CurType,Pay_Type,FT_ROU_Scene,FT_ROU_Bet,OpenType,Language from ".DBPREFIX.MEMBERTABLE." where Oid='$uid' and Status=0";
...
$curl->set_referrer("".$site."/app/member/BS_index.php?rtype=re&uid=$suid&langx=zh-cn&mtype=3");
$html_data=$curl->fetch_url("".$site."/app/member/BS_order/BS_order_rou.php?gid=$gid&uid=$suid&type=$type&gnum=$gnum&odd_f_type=$odd_f_type");
preg_match("/棒球滚球大小交易单/Usi",$html_data,$m_temp);
if(!$m_temp){ 
...
$mysql = "select M_Date,M_Time,$mb_team as MB_Team,$tg_team as TG_Team,$m_league as M_League,MB_Dime_RB,TG_Dime_RB,MB_Dime_Rate_RB,TG_Dime_Rate_RB,MB_Ball,TG_Ball from `".DBPREFIX."match_sports` where Type='BS' and `MID`=$gid and Cancel!=1 and Open=1 and $mb_team!=''";
...
if($cou==0){
	...
	$havesql="select sum(BetScore) as BetScore from ".DBPREFIX."web_report_data where M_Name='$memname' and MID='$gid' and LineType=10 and Mtype='$mtype' and Active=1";
	$result = mysqli_query($dbLink,$havesql);
	$haverow = mysqli_fetch_assoc($result);  
	$have_bet=$haverow['BetScore']+0;  
	  
    $sql = "select CS,ROU from ".DBPREFIX."match_league where  $m_league='$M_League'";
    $result = mysqli_query($dbLink,$sql);
...
<link href="/style/member/mem_order_ft.css?v=<?php echo AUTOVER; ?>" rel="stylesheet" type="text/css">
  ...
  <script type="text/javascript" src="../../../js/jquery.js"></script> <script language="JavaScript" src="../../../js/betcommon.js"></script><script type="text/javascript" src="../../../js/football_order.js"></script>
...
<form name="LAYOUTFORM" action="/app/member/BS_order/BS_order_re_finish.php" method="post" onSubmit="return false">
  <div class="ord">
      ...
      <div class="main">
          <div class="ord_betArea">
                <div class="gametype"><?php echo $U_36 ?></div>
...
<input type="hidden" name="ioradio_r_h" id="ioradio_r_h" value="<?php echo $M_Rate?>">
