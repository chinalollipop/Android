...
$sql = "select UserName,Money,CurType,Pay_Type,BS_R_Scene,BS_R_Bet,OpenType,Language from ".DBPREFIX.MEMBERTABLE." where Oid='$uid' and Status=0";
...
$btset=singleset('R');
$GMIN_SINGLE=$btset[0];
$GMAX_SINGLE=$row['BS_R_Scene'];
$GSINGLE_CREDIT=$row['BS_R_Bet'];
$open=$row['OpenType'];
$langx=$row['Language'];
...
$mysql = "select M_Date,M_Time,$mb_team as MB_Team,$tg_team as TG_Team,$m_league as M_League,ShowTypeHR,M_LetB_H,MB_LetB_Rate_H,TG_LetB_Rate_H from `".DBPREFIX."match_sports` where Type='BS' and `M_Start`>now() and `MID`='$gid' and Cancel!=1 and Open=1 and $mb_team!=''";
...
if($cou==0){
	...
	if ($row['M_Date']==date('Y-m-d')){
		...
		$active=33;
		$class="OBE";
		$caption=$Order_BS.$Order_Early_Market.$Order_1st_Half_Handicap_betting_order;
	}
	...
	$havesql="select sum(BetScore) as BetScore from ".DBPREFIX."web_report_data where m_name='$memname' and MID='$gid' and linetype=12 and Mtype='$mtype' and (Active=3 or Active=33)";
	$result = mysqli_query($dbLink,$havesql);
	$haverow = mysqli_fetch_assoc($result);
	$have_bet=$haverow['BetScore']+0;

    $sql = "select CS,OU,VR from ".DBPREFIX."match_league where  $m_league='$M_League' and Type='BS'";
    $result = mysqli_query($dbLink,$sql);
  ...
  <div class="ord">
    ...
    <div class="main">
        <div class="ord_betArea">
          <div class="gametype"><?php echo $U_47 ?></div>
