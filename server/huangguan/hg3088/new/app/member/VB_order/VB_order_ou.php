...
$sql = "select UserName,Money,CurType,Pay_Type,OpenType,Language,VB_OU_Scene,VB_OU_Bet from ".DBPREFIX.MEMBERTABLE." where Oid='$uid' and Status=0";
...
$btset=singleset('OU');
$GMIN_SINGLE=$btset[0];
$GMAX_SINGLE=VB_OU_Scene;
$GSINGLE_CREDIT=VB_OU_Bet;
$open=$row['OpenType'];
$langx=$row['Language'];
...
$mysql = "select M_Date,M_Time,$mb_team as MB_Team,$tg_team as TG_Team,$m_league as M_League,MB_Dime,TG_Dime,MB_Dime_Rate,TG_Dime_Rate from `".DBPREFIX."match_sports` where Type='VB' and `M_Start`>now() and `MID`='$gid' and Cancel!=1 and Open=1 and $mb_team!=''";
...
if($cou==0){
	...
	if ($row['M_Date']==date('Y-m-d')){
		...
		$active=55;
		$class="OVU";
		$caption=$Order_VB.$Order_Early_Market.$Order_Over_Under_betting_order;
	}
	...
	$havesql="select sum(BetScore) as BetScore from ".DBPREFIX."web_report_data where M_Name='$memname' and MID='$gid' and LineType=3 and Mtype='$mtype' and (Active=5 or Active=55)";  
	$result = mysqli_query($dbLink,$havesql);
	$haverow = mysqli_fetch_assoc($result);  
	$have_bet=$haverow['BetScore']+0;  
	
    $sql = "select CS,OU from ".DBPREFIX."match_league where  $m_league='$M_League' and Type='VB'";
    $result = mysqli_query($dbLink,$sql);
