...
$sql = "select UserName,Money,CurType,Pay_Type,OpenType,Language,VB_ROU_Scene,VB_ROU_Bet from ".DBPREFIX.MEMBERTABLE." where Oid='$uid' and Status=0";
...
preg_match("/排球滚球大小交易单/Usi",$html_data,$m_temp);
if(!$m_temp){ 
	echo attention("$Order_This_match_is_closed_Please_try_again",$uid,$langx);
	exit();
}
//////////////////////////////////////////////////
$mysql = "select M_Date,M_Time,$mb_team as MB_Team,$tg_team as TG_Team,$m_league as M_League,MB_Dime_RE,TG_Dime_RE,MB_Dime_Rate_RE,TG_Dime_Rate_RE,MB_Ball,TG_Ball from `".DBPREFIX."match_sports` where Type='TN' and `MID`=$gid and Cancel!=1 and Open=1 and $mb_team!=''";
...
if($cou==0){
	...
	$rate=get_other_ioratio($odd_f_type,$row["MB_Dime_Rate_RE"],$row["TG_Dime_Rate_RE"],100);
	switch ($type){
	case "C":
		$M_Place=$row["MB_Dime_RE"];
		if ($langx=="zh-cn"){
	...
	$havesql="select sum(BetScore) as BetScore from ".DBPREFIX."web_report_data where M_Name='$memname' and MID='$gid' and LineType=10 and Mtype='$mtype' and Active=5";
	$result = mysqli_query($dbLink,$havesql);
	$haverow = mysqli_fetch_assoc($result);  
	$have_bet=$haverow['BetScore']+0;  
	  
    $sql = "select CS,ROU from ".DBPREFIX."match_league where  $m_league='$M_League' and Type='VB'";
    $result = mysqli_query($dbLink,$sql);
...
<form name="LAYOUTFORM" action="/app/member/VB_order/VB_order_re_finish.php" method="post" onSubmit="return false">
  <div class="ord">
