...
$GMAX_SINGLE=　VB_R_Scene;
$GSINGLE_CREDIT=　VB_R_Bet　;
$open=$row['OpenType'];
$langx=$row['Language'];
...
$mysql = "select M_Date,M_Time,$mb_team as MB_Team,$tg_team as TG_Team,$m_league as M_League,ShowTypeR,MB_LetB_Rate,TG_LetB_Rate,M_LetB from `".DBPREFIX."match_sports` where Type='VB' and `M_Start`>now() and `MID`='$gid' and Cancel!=1 and Open=1 and $mb_team!=''";
...
if($cou==0){
    ...
    $sql = "select CS,R from ".DBPREFIX."match_league where  $m_league='$M_League' and Type='VB'";
    $result = mysqli_query($dbLink,$sql);
