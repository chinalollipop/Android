...
if ($action=='hg2vg' || $action == 'vg2hg'){
    ...
    if ($sUserlayer==3 || $sUserlayer==5){
        ...
        if ($layer['status']==1) {
            exit(json_encode( [ 'err'=>2066,'msg'=>'账号分层异常，请联系我们在线客服' ] ));
        }
    }
}
...
if($action){
    ...
    if(!$vgExist){ // 未创建账号前请求余额接口
        if($action == 'b'){
            ...
            $aResult = createAccount($aUser); // 请求VG创建游戏账号接口
            
            if(in_array($aResult['errcode'] , array(0 , -99))) {  //errcode 0 创建成功  -99 用户已存在
                $data = [
                    'userid' => $aUser['ID'],
                    'username' => $aUser['UserName'],
                    'channel' => $vgConfig['channel'],
                    'agents' => $aUser['Agents'],
                    'world' => $aUser['World'],
                ...
                exit(json_encode(['code' => $aResult['errcode'], 'message' => 'VG棋牌创建账号异常2，请您稍后重试！']));
            }
        }
    }
}
...
switch ($action){
    case "cm":  // 会员 免费试玩或立即游戏
        if($testFlag !== 'test')  { // 免费试玩不创建VG用户
            $cResult = createAccount($aUser); // 请求VG创建游戏账号接口
            if(!in_array($cResult['errcode'] , array(0 ,-99))) {  //errcode 0 创建成功  -99 用户已存在
                exit(json_encode(['code' => $cResult['errcode'], 'message' => 'VG棋牌创建账号异常4，请您稍后重试！']));
            }
        }
        $aResult = $testFlag == 'test' ? tryGame() : loginGame($aUser); // 试玩 或 登录进入游戏

        if($aResult['errcode'] == 0) { // vg登录接口成功
            ...
            exit(json_encode(['code' => 0, 'message' => $aResult['errtext'], 'url' => $aResult['result']])); // 登录成功
    ...
    case 'b':   //点击棋牌游戏页面   额度转换
        ...
        $data = [];
        if($aResult['errcode'] == 0){ //获取余额成功
            $data = [
                'vg_balance' => sprintf('%.2f', $aResult['result']), // 返回vg会员账户余额
                'hg_balance' => sprintf('%.2f', $aUser['Money'])
            ];
            exit(json_encode(['code' => 0, 'data' => $data]));
        }else{
    ...
    case 'hg2vg':
        ...
        $orderId = $vgConfig['channel'] . $time_str . $aUser['UserName'];
        ...
        if($aResult){
            if($aResult['errcode'] == 0){ // 存款成功
                $dbMasterLink->commit();
                $data = [
                    'vg_balance' => sprintf('%.2f', $aResult['result']), // 元模式
 ...
 * @return array|mixed
 */
function createAccount($aUser)
{
    $params = [
        'action' => 'create', // 创建游戏账号
        'username' => $aUser['UserName'],
        'agent' => $aUser['Agents'],
    ];
    $aResult = vgApi($params);
...
function tryGame()
{
    $params = [
        'action' => 'trygame', // 登录游戏
        'gametype'  => 1000,   //游戏类型  1000游戏大厅
        'gameversion' => 2,  //游戏版本 1=flash 2=h5
    ];
    $aResult = vgApi($params);
    return $aResult;
}
...
function vgApi($params){
    global $vgConfig;
    $off = new \app\member\ApiVg($vgConfig);
    try {
        ob_start();
        $res = $off->main($params);
