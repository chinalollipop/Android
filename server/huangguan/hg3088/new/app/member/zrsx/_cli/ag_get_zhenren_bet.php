...
//    $data['enddate'] = '2018-03-30 04:59:00';
    if (isset($argv['1']) && isset($argv['2']) && isset($argv['3']) && isset($argv['4']) ) {
        ...
        // 每隔9分钟抓取一次数据（9分钟 + 1秒钟）
        $data['startdate'] = date('Y-m-d H:i:s', time() - 541);
...
//    var_dump( $xml_array ); die;
    ...
    if ($xml_array['info'] == 0) {
        ...
        $totalpages = $data_page['totalpages']; // 总页数
        ...
        if ($total > 0 and $total <= 500 && $totalpages == 1) {
            ...
            $oAg->zrsxInDb($xml_array['row'], $rowCount,$data['cagent'].$userPrefix."_");
            ...
            for ($i = 1; $i <= $totalpages; $i++) {
                ...
                $xml_array = json_decode($xml_json, true);
                // 入库
                $oAg->zrsxInDb($xml_array['row'], $xml_array['addition']['perpage'],$data['cagent'].$userPrefix."_");
                // 处理完最后一页正常结束，无需sleep
                if ($i < $totalpages)
                    sleep(15);
