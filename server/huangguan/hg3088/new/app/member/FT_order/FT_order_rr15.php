...
$btset=singleset('R');
$GMIN_SINGLE=$btset[0];
$mysql = "select M_Date,M_Time,$mb_team as MB_Team,$tg_team as TG_Team,$m_league as M_League,ShowTypeR from ".DBPREFIX."match_sports where `M_Start`>now() and `MID`='$gid' and Cancel!=1 and Open=1 and $mb_team!=''";
...
if($cou==0 || $couMore==0){
	...
	if(substr($_REQUEST['wtype'],0,1)=="A"){ $gametype=$U_74.' '.$Running_Ball.':'.$U_74A.'-'.$U_R; }
	if(substr($_REQUEST['wtype'],0,1)=="B"){ $gametype=$U_74.' '.$Running_Ball.':'.$U_74B.'-'.$U_R; }
	if(substr($_REQUEST['wtype'],0,1)=="C"){ $gametype=$U_74.' '.$Running_Ball.':'.$U_74C.'-'.$U_R; }
	if(substr($_REQUEST['wtype'],0,1)=="D"){ $gametype=$U_74.' '.$Running_Ball.':'.$U_74D.'-'.$U_R; }
	if(substr($_REQUEST['wtype'],0,1)=="E"){ $gametype=$U_74.' '.$Running_Ball.':'.$U_74E.'-'.$U_R; }
	if(substr($_REQUEST['wtype'],0,1)=="F"){ $gametype=$U_74.' '.$Running_Ball.':'.$U_74F.'-'.$U_R; }
