...
$sql = "select UserName,Money,CurType,Pay_Type,OpenType,Language,OP_M_Scene,OP_M_Bet from ".DBPREFIX.MEMBERTABLE." where Oid='$uid' and Status=0";
...
$btset=singleset('M');
$GMIN_SINGLE=$btset[0];
$GMAX_SINGLE=OP_M_Scene;
$GSINGLE_CREDIT=OP_M_Bet;
$open=$row['OpenType'];
$langx=$row['Language'];
...
$mysql = "select M_Date,M_Time,$mb_team as MB_Team,$tg_team as TG_Team,$m_league as M_League,MB_Win_Rate,TG_Win_Rate,M_Flat_Rate from `".DBPREFIX."match_sports` where Type='OP' and `M_Start`>now() and `MID`='$gid' and Cancel!=1 and Open=1 and $mb_team!=''";
...
if($cou==0){
	...
	if ($row['M_Date']==date('Y-m-d')){
		...
		$active=66;
		$class="OOM";
		$caption=$Order_Other.$Order_Early_Market.$Order_1_x_2_betting_order;
	}
    ...
    $sql = "select CS,M from ".DBPREFIX."match_league where  $m_league='$M_League' and Type='OP'";
    $result = mysqli_query($dbLink,$sql);
