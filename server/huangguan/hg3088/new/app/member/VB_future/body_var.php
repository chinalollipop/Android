...
<!--
if(self == top) location='<?php echo BROWSER_IP?>/app/member/'
...
switch ($rtype){
case "r":
	$mysql = "select MID,M_Date,M_Time,$mb_team as MB_Team,$tg_team as TG_Team,$m_league as M_League,MB_Win_Rate,TG_Win_Rate,M_LetB,MB_LetB_Rate,TG_LetB_Rate,MB_Dime,TG_Dime,MB_Dime_Rate,TG_Dime_Rate,S_Single_Rate,S_Double_Rate,MB_MID,TG_MID,ShowTypeR,M_Type from `".DBPREFIX."match_sports` where Type='VU' and `M_Date` >'$m_date' and S_Show=1 and $mb_team<>'' ".$date." order by m_start,mid";		
	...
	break;
case "pd":
	$mysql = "select MID,M_Date,M_Time,$mb_team as MB_Team,$tg_team as TG_Team,$m_league as M_League,MB2TG0,MB2TG1,MB3TG0,MB3TG1,MB3TG2,MB0TG2,MB1TG2,MB0TG3,MB1TG3,MB2TG3,ShowTypeR from `".DBPREFIX."match_sports` where Type='VU' and `M_Date` >'$m_date' ".$league." and PD_Show=1 and MB2TG1!=0 and $mb_team<>'' ".$date." order by m_start,mid";
	$result = mysqli_query($dbMasterLink, $mysql);
	...
	break;
case "p":
	$mysql = "select MID,M_Date,M_Time,$mb_team as MB_Team,$tg_team as TG_Team,$m_league as M_League,MB_P_Win_Rate,TG_P_Win_Rate,MB_MID,TG_MID,ShowTypeR from `".DBPREFIX."match_sports` where Type='VU' and `M_Date` >'$m_date' and P_Show=1 and $mb_team<>'' order by m_start,mid";
	$result = mysqli_query($dbMasterLink, $mysql);
...
case "p3":
	$mysql = "select MID,M_Date,M_Time,$mb_team as MB_Team,$tg_team as TG_Team,$m_league as M_League,M_P_LetB,MB_P_LetB_Rate,TG_P_LetB_Rate,MB_P_Dime,TG_P_Dime,MB_P_Dime_Rate,TG_P_Dime_Rate,MB_MID,TG_MID,ShowTypeR from `".DBPREFIX."match_sports` where Type='VU' and `M_Date` >'$m_date' and PR_Show=1 and $mb_team<>'' order by m_start,mid";		
	$result = mysqli_query($dbMasterLink, $mysql);
	...
	while ($row=mysqli_fetch_assoc($result)){
		...
		echo "parent.GameFT[$K]= Array('$row[MID]','$pdate','$row[M_League]','$row[MB_MID]','$row[TG_MID]','$mb_team','$row[TG_Team]','$row[ShowTypeR]','$row[M_P_LetB]','$MB_P_LetB_Rate','$TG_P_LetB_Rate','$row[MB_P_Dime]','$row[TG_P_Dime]','$MB_P_Dime_Rate','$TG_P_Dime_Rate','$row[MID]','3','10');\n";
		$K=$K+1;	
	}
	break;
}
...
//
 ...
 function onLoad()
 {
  if(top.SI2_mem_index.mem_order.location == 'about:blank')
   top.SI2_mem_index.mem_order.location = '<?php echo BROWSER_IP?>/app/member/select.php?uid=<?php echo $uid?>&langx=<?php echo $langx?>';
  if(parent.retime > 0)
