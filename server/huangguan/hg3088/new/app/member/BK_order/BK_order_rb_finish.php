...
foreach($accoutArr as $key=>$value){//在扩展表中获取账号重新刷水
	...
	switch ($line) {
	    case '21': // 滚球独赢
	        ...
	        break;
	    case '23': // 滚球得分大小
	        $html_data = $curl->fetch_url("" . $site . "/app/member/BK_order/BK_order_rouhc.php?gid=$gid&uid=$suid&wtype=$wtype&type=$type&odd_f_type=$odd_f_type&langx=$langx");
	        break;
	}
...
if($cou==0){
	...
	if((isset($row['MB_Ball'])&&$row['MB_Ball']>0) || (isset($row['TG_Ball'])&&$row['TG_Ball']>0)){
		...
		$mysqlBall = "select MB_Ball,TG_Ball from `".DBPREFIX."match_sports` where Type='BK' and Open=1 and MB_Team='".$row['MB_Team']."' and TG_Team='".$row['TG_Team']."' and M_League='".$row['M_League']."' and M_Start='".$row['M_Start']."' limit 1";
    ...
    <span><h1><?php echo $caption?></h1></span>
      <div id="info">
       ...
       <div class="fin_title">
           ...
           <p class="p-underline"> <?php echo $Order_Bet_success?><?php echo $showVoucher;?>请勿关闭窗口</p> <?php /*echo $Order_Bet_success*/?><!--&nbsp;--><?php /*echo show_voucher($line,$ouid)*/?>  <!--< 原来的的 -->
