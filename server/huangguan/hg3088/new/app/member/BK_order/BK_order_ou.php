...
if($cou==0 && $couMore==0){
	...
	if($couMore>0){
		...
		if($detailsData['sw_'.$wtype]=="Y" && $detailsData["ior_".$rtype]>0){
			...
			if($type=="C") $M_Place ='小'. $detailsData["ratio_u"];
			if($type=="H") $M_Place ='大'. $detailsData["ratio_o"];
		}	
	}else{
	...
	if($cou==0){
		$mysql = "select M_Date,M_League,MB_Team,TG_Team from `".DBPREFIX."match_sports` where `m_start`>now() and `MID`='{$_REQUEST['id']}' and Cancel!=1 and Open=1 and $mb_team!=''";
  ...
  <div class="ord">
      ...
      <div class="main">
          <div class="ord_betArea">
            <div class="gametype"><?php echo $BK_NFL.' '.$U_OU ?></div>
