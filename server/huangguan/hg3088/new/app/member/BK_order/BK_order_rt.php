...
$mysql = "select M_Date,M_Time,$mb_team as MB_Team,$tg_team as TG_Team,$m_league as M_League,S_Single_Rate,S_Double_Rate from `".DBPREFIX."match_sports` where Type='BK' and `MID`=$gid and Cancel!=1 and Open=1 and $mb_team!=''";
...
if($cou==0 && $couMore==0){
	...
	exit();
		...
		if($cou==0){
			$mysql = "select M_Date,M_Time,$mb_team as MB_Team,$tg_team as TG_Team,$m_league as M_League,S_Single_Rate,S_Double_Rate from `".DBPREFIX."match_sports` where `m_start`>now() and `MID`='{$_REQUEST['id']}' and Cancel!=1 and Open=1 and $mb_team!=''";
