...
$mdate = $row['M_Date'];
$result = mysqli_query($dbMasterLink,"select M_Date,MID,nowSession,MB_Team,TG_Team,M_League from `".DBPREFIX."match_sports` where Type='BK' and Checked=0 and Cancel!=1 and Open=1 and M_League ='$mleague' and MB_Team like '$mbteam%' and TG_Team like '$tgteam%' and M_Date = '$mdate' ");
...
if($cou==0 && $couMore==0){
	...
	if((isset($row['MB_Ball'])&&$row['MB_Ball']>0) || (isset($row['TG_Ball'])&&$row['TG_Ball']>0)){
        ...
        // 根据主队名称、客队名称去查出实时比分
        $row_MB_Team = explode('<font color=gray>',$row['MB_Team'])[0];
        $row_TG_Team = explode('<font color=gray>',$row['TG_Team'])[0];
		$mysqlBall = "select MB_Ball,TG_Ball from `".DBPREFIX."match_sports` where Type='BK' and Open=1 and MB_Team='".$row_MB_Team."' and TG_Team='".$row_TG_Team."' and M_League='".$row['M_League']."' and M_Start='".$row['M_Start']."' limit 1";
...
if($begin && $lockResult){
	...
	$sql = "INSERT INTO ".DBPREFIX."web_report_data	(danger,Glost,playSource,MID,Userid,testflag,Active,orderNo,LineType,Mtype,M_Date,BetTime,BetScore,Middle,Middle_tw,Middle_en,BetType,BetType_tw,BetType_en,M_Place,M_Rate,M_Name,Gwin,OpenType,OddsType,ShowType,Agents,World,Corprator,Super,Admin,A_Point,B_Point,C_Point,D_Point,BetIP,Ptype,Gtype,CurType,Ratio,MB_MID,TG_MID,Pay_Type,Orderby,MB_Ball,TG_Ball,betid) values (0,$Money,2,'$gid',$memid,$test_flag,'$active','$showVoucher','$line','$w_gtype','$m_date','$bettime','$gold','$lines','$lines_tw','$lines_en','$bet_type','$bet_type_tw','$bet_type_en','$grape','$w_m_rate','$memname','$gwin','$open','$oddstype','$showtype','$agents','$world','$corprator','$super','$admin','$a_point','$b_point','$c_point','$d_point','$ip_addr','$ptype','BK','$w_current','$w_ratio','$w_mb_mid','$w_tg_mid','$pay_type','$order','$mb_ball','$tg_ball','$betid')";
	$insertBet=mysqli_query($dbMasterLink,$sql);
	if($insertBet){
        $lastId=mysqli_insert_id($dbMasterLink);
		$moneyLogRes=addAccountRecords(array($memid,$memname,$test_flag,$Money,$gold*-1,$havemoney,1,2,$lastId,"BK投注$w_gtype"));
		if($moneyLogRes){
