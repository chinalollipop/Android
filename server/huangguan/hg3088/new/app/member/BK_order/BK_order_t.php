...
$mysql = "select M_Date,M_Time,$mb_team as MB_Team,$tg_team as TG_Team,$m_league as M_League,S_Single_Rate,S_Double_Rate from `".DBPREFIX."match_sports` where `m_start`>now() and `MID`='$gid' and Cancel!=1 and Open=1 and $mb_team!=''";
...
if($cou==0 && $couMore==0){
    ...
    if ($rtype=='ODD' or $rtype=='EVEN'){
        $sql = "select CS,OU,R from ".DBPREFIX."match_league where  $m_league='$M_League' and Type='BK'";
        $result = mysqli_query($dbLink,$sql);
  ...
  <div class="ord">
	...
	<div class="main">
        <div class="ord_betArea">
         <div class="gametype"><?php if($rtype=='EVEN' or $rtype=='ODD'){ ?><?php echo $BK_NFL.' '.$U_08 ?><?php }else{ ?><?php echo $BK_NFL.' '.$U_08 ?><?php } ?></div>
