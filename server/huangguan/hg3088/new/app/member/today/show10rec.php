...
<?php
$m_date=date('Y-m-d');
$sql = "select MID,Mtype,MB_MID,TG_MID,Active,ShowType,LineType,BetTime,$middle as Middle,BetScore,M_Date,Gwin,M_Place,M_Rate,Cancel,Danger,Confirmed from ".DBPREFIX."web_report_data where M_Name='$memname' and M_Result='' and M_Date>='$m_date' and LineType!=8 order by BetTime desc limit 0,10";
$result = mysqli_query($dbLink,$sql);
$cou=mysqli_num_rows($result);
?>
<div <?php if($cou<=0){ ?>style='display:none'<?php } ?>>
<div class="ord">
...
while ($row = mysqli_fetch_assoc($result)){
       ...
       switch ($row['LineType']){
			 ...
			 case 16:
			 echo '<p><em>'.$times."<BR>";
			  echo $row['Middle'].'<b>&nbsp;&nbsp;&nbsp;$'.$betscore.'</b></p>';;
			  break;
			 default:
             $middle=explode('<br>',$row['Middle']);
			 echo '<p><em>'.$times.'&nbsp;['.$mb_mid.']vs['.$tg_mid.']</em><br>';
...
<div style=' <?php if($cou>0){ ?>display:none<?php } ?>' class="show_info"><span class="show_top"></span>您没有未结算的交易。</div>
</body>
</html>
