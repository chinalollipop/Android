...
$sql = "select UserName,Money,CurType,Pay_Type,OpenType,Language,TN_M_Scene,TN_M_Bet from ".DBPREFIX.MEMBERTABLE." where Oid='$uid' and Status=0";
...
$btset=singleset('M');
$GMIN_SINGLE=$btset[0];
$GMAX_SINGLE=$row['TN_M_Scene'];
$GSINGLE_CREDIT=$row['TN_M_Bet'];
$open=$row['OpenType'];
$langx=$row['Language'];
...
$mysql = "select M_Date,M_Time,$mb_team as MB_Team,$tg_team as TG_Team,$m_league as M_League,MB_Win_Rate,TG_Win_Rate from `".DBPREFIX."match_sports` where Type='TN' and `M_Start`>now() and `MID`='$gid' and Cancel!=1 and Open=1 and $mb_team!=''";
...
if($cou==0){
	...
	if ($row['M_Date']==date('Y-m-d')){
		...
		$active=44;
		$class="OTU";
		$caption=$Order_TN.$Order_Early_Market.$Order_1_x_2_betting_order;
	}
	...
	$havesql="select sum(BetScore) as BetScore from ".DBPREFIX."web_report_data where m_name='$memname' and MID='$gid' and linetype=1 and Mtype='$mtype' and (Active=4 or Active=44)";
