...
$sql = "select UserName,Money,CurType,Pay_Type,OpenType,Language,TN_EO_Scene,TN_EO_Bet from ".DBPREFIX.MEMBERTABLE." where Oid='$uid' and Status=0";
...
$btset=singleset('T');
$GMIN_SINGLE=$btset[0];
$GMAX_SINGLE= TN_EO_Scene ;
$GSINGLE_CREDIT= TN_EO_Bet ;
$open=$row['OpenType'];
$langx=$row['Language'];
...
$mysql = "select M_Date,M_Time,$mb_team as MB_Team,$tg_team as TG_Team,$m_league as M_League,S_Single_Rate,S_Double_Rate from `".DBPREFIX."match_sports` where Type='TN' and `m_start`>now() and `MID`='$gid' and Cancel!=1 and Open=1 and $mb_team!=''";
...
if($cou==0){
	...
	$havesql="select sum(BetScore) as BetScore from ".DBPREFIX."web_report_data where m_name='$memname' and MID='$gid' and linetype='$linetype' and (Active=4 or Active=44)";
    ...
    if ($rtype=='ODD' or $rtype=='EVEN'){
        ...
        $sql = "select CS,EO from ".DBPREFIX."match_league where  $m_league='$M_League' and Type='TN'";
        $result = mysqli_query($dbMasterLink,$sql);
