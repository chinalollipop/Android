...
$sql = "select UserName,Money,CurType,Pay_Type,OpenType,Language,TN_RE_Scene,TN_RE_Bet from ".DBPREFIX.MEMBERTABLE." where Oid='$uid' and Status=0";
...
$btset=singleset('RE');
$GMIN_SINGLE=$btset[0];
$GMAX_SINGLE= TN_RE_Scene ;
$GSINGLE_CREDIT= TN_RE_Bet ;
$open=$row['OpenType'];
$langx=$row['Language'];
...
$html_data=$curl->fetch_url("".$site."/app/member/TN_order/TN_order_re.php?gid=$gid&uid=$suid&type=$type&gnum=$gnum&strong=$strong&odd_f_type=$odd_f_type");
preg_match("/网球滚球交易单/Usi",$html_data,$m_temp);
if(!$m_temp){ 
	echo attention("$Order_This_match_is_closed_Please_try_again",$uid,$langx);
	exit();
}

$mysql = "select M_Date,M_Time,$mb_team as MB_Team,$tg_team as TG_Team,$m_league as M_League,ShowTypeRB,M_LetB_RB,MB_LetB_Rate_RB,TG_LetB_Rate_RB,MB_Ball,TG_Ball from `".DBPREFIX."match_sports` where Type='TN' and `MID`=$gid and Cancel!=1 and Open=1 and $mb_team!=''";
$result = mysqli_query($dbMasterLink,$mysql);
...
if($cou==0){
	...
	$havesql="select sum(BetScore) as BetScore from ".DBPREFIX."web_report_data where M_Name='$memname' and MID='$gid' and LineType=9 and Mtype='$mtype' and Active=4";
	$result = mysqli_query($dbLink,$havesql);
	$haverow = mysqli_fetch_assoc($result);  
	$have_bet=$haverow['BetScore']+0;  
	
    $sql = "select CS,RB from ".DBPREFIX."match_league where  $m_league='$M_League' and Type='TN'";
    $result = mysqli_query($dbMasterLink,$sql);
  ...
  <div class="ord">
    ...
    <div class="main">
        <div class="ord_betArea">
            <div class="gametype"><?php echo $U_46 ?></div>
