...
//                @error_log(json_encode($v) . PHP_EOL, 3, '/tmp/group/AGbet.log');
                ...
                if ($id <= 0) {
                    echo "找不到user({$v['playName']})，解析失败!，跳过\n";
                    continue;
                }
...
//                @error_log($sql . PHP_EOL, 3, '/tmp/group/AGbet.log');

                if (!empty($sql)) {
                    $id = mysqli_query(self::$_mysqli, $sql);
                    echo "第{$doneCount}笔，".DBPREFIX."ag_projects纪录写入成功\n";
                    $doneCount = $doneCount + 1;
                }
                if (!isset($id) || $id <= 0) {
                    echo "第{$doneCount}笔，".DBPREFIX."ag_projects纪录写入失败，回滚\n";
                    mysqli_rollback(self::$_mysqli);
                    break;
                }
                mysqli_commit(self::$_mysqli);
...
//                @error_log(json_encode($v) . PHP_EOL, 3, '/tmp/group/AGbet.log');
                ...
                if ($id <= 0) {
                    echo "找不到user({$v['username']})，解析失败!，跳过\n";
                    continue;
                }
                ...
                $tmp['gamecode'] = '';
                $tmp['thirdprojectid'] = $v['billno'];
                $tmp['type'] = $v['platformType']; // BR AG真人数据  SLOT  電子游戲廳
