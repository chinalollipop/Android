...
function VerifyData(par){
    ...
    var flag = true ;
    if (_$('introducer').value && !isNum(_$('introducer').value)) {  // 介绍人
        alert('请输入正确的帐号!') ;
        flag =false ;
        _$('introducer').focus() ;
        return false;
    ...
    if(!par){ // 代理注册才有
        ...
        //     return false;
        // }
        if (!isWechat(_$('wechat').value) || _$('wechat').value==''){
            alert('请输入正确的微信号码!') ;
            flag =false ;
            _$('wechat').focus() ;
            return false;
    ...
    if (_$('paypassword').value.length < 4 || _$('paypassword').value.length > 6 || !isNumber(_$('paypassword').value) ) {
        alert('提款密码为4-6位数字!') ;
        flag =false ;
        _$('paypassword').focus() ;
        return false;
