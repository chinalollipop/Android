...
function setFontAction() {
    var str ='<div id="skin" class="zoomChange">字体显示：<a id="skin_0" data-val="1" class="zoom zoomSmaller" href="javascript:;" title="点击切换原始字体">小</a>' +
        ...
        '</div>';
  // parent.body_browse.document.getElementsByClassName('top_h1')[0].insertAdjacentHTML('afterend', str) ;
    $('.top').append(str) ;
   // 切换字体大小
...
function seaGameList(obj) {
   ...
   // console.log(parent.myLeg) ;
    ...
    reload_var('',txt) ;
  /*  $('.leg_bar').each(function () {
       var oldtxt = $(this).text() ;
       // console.log(oldtxt) ;
       // console.log(oldtxt.indexOf(txt)) ;
      if(oldtxt.indexOf(txt) > -1){ // 有匹配结果
            seaArr.push($(this).parents('.b_hline').parent());
            $(this).parents('.b_hline').parent().css('display','table-row').siblings('tr').css('display','none') ;
           // $(this).parents('.b_hline').parent().siblings('tr').css('display','none') ;
            var parentArrLen = parent.myLeg[oldtxt].length ;
            //console.log(parent.myLeg[oldtxt]);
            for(var i=0;i<parentArrLen;i++){ // 显示匹配项
                console.log(parent.myLeg[oldtxt][i])
                $("#TR_"+parent.myLeg[oldtxt][i]).css('display','table-row') ;
                $("#TR1_"+parent.myLeg[oldtxt][i]).css('display','table-row') ;
                $("#TR2_"+parent.myLeg[oldtxt][i]).css('display','table-row') ;
                $("#TR3_"+parent.myLeg[oldtxt][i]).css('display','table-row') ;
            }
        }else{
          // $(this).parents('.b_hline').parent().css('display','none') ;
          var parentArrLen = parent.myLeg[oldtxt].length ;
      /!*    for(var i=0;i<parentArrLen;i++){ // 显示匹配项
              $("#TR_"+parent.myLeg[oldtxt][i]).css('display','none') ;
              $("#TR1_"+parent.myLeg[oldtxt][i]).css('display','none') ;
              $("#TR2_"+parent.myLeg[oldtxt][i]).css('display','none') ;
              $("#TR3_"+parent.myLeg[oldtxt][i]).css('display','none') ;
          }*!/
      }

    }) ;
    if(seaArr.length ==0){ // 没有任何匹配
        $('#showtable').find('tr').css('display','none') ;
    }*/
    // console.log(seaArr)
...
function chg_league(){
    ...
    // var $body_browse_set=document.getElementsByClassName('body_browse_set')[0] ;
    // if($body_browse_set){
    //     $body_browse_set.style.overflowY='hidden' ;
    // }
    try{
        legFrame.location.href="./body_var_lid.php?uid="+parent.uid+"&rtype="+parent.rtype+"&langx="+parent.langx+"&mtype="+parent.ltype+"&FStype="+parent.FStype;
    }catch(e){
        legFrame.src="./body_var_lid.php?uid="+parent.uid+"&rtype="+parent.rtype+"&langx="+parent.langx+"&mtype="+parent.ltype+"&FStype="+parent.FStype;
    }
