...
function onLoad(){
    ...
    // if(sel_gtype=="FU"){ // 早盘才有
    //     selgdate(parent.rtype,'','future');
    // }
...
function showPicLove(){
    ...
    try{
        ...
        if(gtypeNum!=0){
            ...
            //document.getElementById("fav_num").style.display = "inline-block";
            if(top.swShowLoveI){
...
function reload_var(Level,leaname,myloveid){
    ...
    var sortpar = '&sorttype='+top.sorttype ; // 排序
    ...
    if(parent.rtype=='FS'){ // 冠军
        ...
        if(leaarr){
            gj_homepage +="&myleaArr="+leaarr.toString()+sortpar ;
        }
    ...
    if(leaarr && leaarr !='undefined'){
        homepage +="&myleaArr="+leaarr.toString() ;
    }
   // parent.body_var.location = homepage+sortpar;
   // console.log(homepage+sortpar)
    $(".load_body_var").load(homepage+sortpar,function () {
        ShowGameList();
    }) ;
...
function show_page(){
    ...
    obj_pg = document.getElementById('show_page_txt');
...
// 切换参数 主要盘口、所有盘口
function chgMarket(level,fake){ //Main,All
    if(level=='main'){ // 主要盘口
        if(parent.rtype=='r_main' || parent.rtype=='r' || parent.rtype=='all'){ // 今日赛事
            parent.rtype='r_main' ;
        }else{ // 滚球
            parent.rtype='re_main' ;
        }
        document.getElementById('all_sel_Market').style.display='inline-block' ;
        document.getElementById('sel_Market').style.display='none' ;
    }else{ // 全部盘口
        if(parent.rtype=='r_main' || parent.rtype=='r' ){ // 今日赛事
            parent.rtype='r' ;
        }else if(parent.rtype=='all'){ // 篮球
            parent.rtype='all' ;
        } else{ // 滚球
            parent.rtype='re' ;
        }
        document.getElementById('all_sel_Market').style.display='none' ;
        document.getElementById('sel_Market').style.display='inline-block' ;
    }
    refreshReload() ;
}
...
// 删除空赔率，否则会造成样式错乱
function deleteNullRate() {
    var ele = $('.bet_game_table ').find('td').find('a').find('font');
    ele.each(function () {
        var rate = $(this).text() ;
        if(rate==''){ // 空赔率
...
function sortAction() {
    ...
    $sel_sort.hover(function () {
        $('#show_sort').toggle() ;
    });

    if(top.sorttype=='time'){
        $('#sort_time').addClass('hover') ;
    }else if(top.sorttype=='league'){
        $('#sort_leg').addClass('hover') ;
    }
