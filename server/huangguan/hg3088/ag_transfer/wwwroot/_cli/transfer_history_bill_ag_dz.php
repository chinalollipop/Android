...
function transfer_history_bill($stop_time, $conn){
	...
	// ag注单表中的数据都是已结算完毕的数据
	$sql = "insert into ".DBPREFIX."ag_dz_projects_history(`billno`,`username`,`prefix`,`account`,`valid_account`,`cus_account`,`billtime`,`slottype`,`gametype`,`mainbillno`,`flag`) 
	select * from ".DBPREFIX."ag_dz_projects where billtime < '{$stop_time}' and flag=1 ";
	...
	$sql = "delete from ".DBPREFIX."ag_dz_projects where billtime<'{$stop_time}' and flag=1 ";
	$result=mysqli_query($conn, $sql);
	if(!$result) {
		$result=mysqli_query($conn, "ROLLBACK");
