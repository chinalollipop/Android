...
if ($_GET['ids']!="") {$ids=$_GET['ids'];}else{$ids="����";}
...
$exe=mysqli_query($dbLink,"update ".DBPREFIX."adad set ys='".$_POST['tm']."',ys1='".$_POST['tm1']."'  Where id=1");
}
...
echo "<script>alert('���ֳɹ�!');window.location.href='index.php?action=pz_wx&ids=".$ids."&kithe=".$kithe."';</script>"; 
exit;
...
if (i!=(arrResult.length-2)){
var zmzm1="index.php?action=look&kithe=<?php echo $kithe?>&class2=<?php echo $ids?>&lx=0&id=4&class3="+arrTmp[12];
...
var zmzm2="index.php?action=look&lx=1&kithe=<?php echo $kithe?>&class2=<?php echo $ids?>&id=4&class3="+arrTmp[12];
...
while($image = mysqli_fetch_assoc($result)){
        ...
        <form name=form55 action="index.php?action=pz_wx&zsave=zsave&kithe=<?php echo $kithe?>&ids=<?php echo $ids?>" method=post>
          <td width="85" align="right" nowrap>������</td>
          <td width="30"><span class="STYLE2">
            <input name="tm" class="input1" id="tm" value='<?php echo $ys?>' size="10" />
          </span></td>
          <td width="50" align="right" nowrap>��ˮ��</td>
          <td width="63"><input name="tm1" class="input1" type="text" id="tm1" value="<?php echo $ys1?>" size="5"></td>
...
<DIV id=rs_window style="DISPLAY: none; POSITION: absolute">
 <form action="index.php?action=pz_wx&save=save&ids=<?php echo $ids?>&kithe=<?php echo $kithe?>"    method="post" name="form1" > <?php
include("d1.php");
?>
</form>
</div>
...
<SCRIPT language=javascript>
 makeRequest('index.php?action=server_wx&class1=����&class2=<?php echo $ids?>&kithe=<?php echo $kithe?>')</script>
