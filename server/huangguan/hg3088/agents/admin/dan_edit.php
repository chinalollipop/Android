...
$results=mysql_query("select * from ka_guan where  id='".$zongid."'  order by id desc"); 
$rows=mysql_fetch_array($results);
$vvz=(100-$rows['sj']-$_POST['sf']-$_POST['sj'])/10;
$gusj=$rows['sj']/10;
$exe=mysql_query("update ka_mem set dagu_zc=".$vvz.",guan_zc=".$gusj.",dan_zc=".($_POST['sf']/10).",zong_zc='".($_POST['sj']/10)."' where danid=".$_GET['id']);
}
...
$exe=mysql_query("update ka_quota Set yg='".$yg[$I]."',ygb='".$ygb[$I]."',ygc='".$ygc[$I]."',ygd='".$ygd[$I]."',xx='".$_POST['mm'.$I]."',xxx='".$_POST['mmm'.$I]."' where  id=".$ygid[$I]." ",$conn);



///会员
if ($yg1>$yg[$I]){
$exe=mysql_query("update ka_quota Set yg='".$yg[$I]."'  where ds='".$ds[$I]."'  and abcd='A'  and flag=1  and danid=".$_GET['id']." ",$conn);
}
if ($yg1>$ygb[$I]){
$exe=mysql_query("update ka_quota Set yg='".$ygb[$I]."'  where ds='".$ds[$I]."'   and abcd='B'  and flag=1  and danid=".$_GET['id']." ",$conn);
}
if ($yg1>$ygc[$I]){
$exe=mysql_query("update ka_quota Set yg='".$ygc[$I]."'  where ds='".$ds[$I]."'  and abcd='C'  and flag=1  and danid=".$_GET['id']." ",$conn);
}
if ($yg1>$ygd[$I]){
$exe=mysql_query("update ka_quota Set yg='".$ygd[$I]."'  where ds='".$ds[$I]."'   and abcd='D'  and flag=1 and  danid=".$_GET['id']." ",$conn);
}
...
if ($xx1>$_POST['mm'.$I]){
$exe=mysql_query("update ka_quota Set xx='".$_POST['mm'.$I]."'  where ds='".$ds[$I]."'   and  danid=".$_GET['id']." ",$conn);}
if ($xxx1>$_POST['mmm'.$I]){
$exe=mysql_query("update ka_quota Set xxx='".$_POST['mmm'.$I]."'  where ds='".$ds[$I]."'  and  danid=".$_GET['id']." ",$conn);}
...
echo "<script>alert('代理修改成功!');window.location.href='index.php?action=dan_edit&id=".$_GET['id']."';</script>"; 
exit;
...
$result2=mysql_query("select *  from ka_guan where  id=".$_GET['id']." and lx=3"); 
$row2=mysql_fetch_array($result2);

if ($row2!=""){


$result=mysql_query("select id,kauser,sf,cs,tmb,rs   from ka_guan where  id=".$row2['zongid']." and lx=2"); 
$row=mysql_fetch_array($result);
if ($row!=""){
...
$result1 = mysql_query("Select Count(ID) As memnum2 From ka_mem Where danid=".$row2['id']." order by id desc");
	$rsw = mysql_fetch_array($result1);
...
if (rtype=='SP' && (eval(goldvalue) > <?php echo $maxnum?>)) {gold.focus(); alert("对不起,上级总信用额度最高可使用 : <?php echo $maxnum?>!!"); return false;}
}
...
<table width="100%" border="0" cellspacing="0" cellpadding="5">
  <tr class="tbtitle">
    <td width="29%"><span class="STYLE3">代理资料修改</span></td>
    <td width="34%">&nbsp;</td>
    <td width="37%">&nbsp;</td>
  </tr>
  <tr >
  ...
  <table width="99%"  border="1" cellpadding="2" cellspacing="2" bordercolor="#ECE9D8">
    <form name=testFrm onSubmit="return SubChk()" method="post" action="index.php?action=dan_edit&act=添加&id=<?php echo $_GET['id']?>"><tr>
      <td height="30" align="right" bordercolor="#CCCCCC" bgcolor="#FDF4CA">上级总代：</td>
      <td bordercolor="#CCCCCC"><font color="ff6600">
        <?php echo $row2['guan']?>
        [股]--
        <?php echo $row2['zong']?>
        [总]
        <input name="zongid" type="hidden" value="<?php echo $zongid?>" />
      </font></td>
      <td height="30" align="right" bordercolor="#CCCCCC" bgcolor="#FDF4CA">分配余额：</td>
      <td bordercolor="#CCCCCC"><?php	 $result1 = mysql_query("Select SUM(cs) As sum_m  From ka_mem Where   danid=".$row2['id']." order by id desc");
	$rsw = mysql_fetch_array($result1);
    ...
    <tr>
      ...
      <td bordercolor="#CCCCCC"><input type="hidden" name="tv5" value="<?php if ($row2['pz']==0) {?>是<?php }else{?>否<?php }?>" />
          <img src="images/<?php if ($row2['pz']==0) {?>icon_21x21_selectboxon.gif<?php }else{?>icon_21x21_selectboxoff.gif<?php }?>" name="tv5_b" align="absmiddle" class="cursor" id="tv5_b" onclick="javascript:ra_select('tv5')" /> (充许走飞/禁止走飞)<span class="STYLE2">*</span></td>
      <td height="30" align="right" bordercolor="#CCCCCC" bgcolor="#FDF4CA">状态：</td>
      <td bordercolor="#CCCCCC"><input type="hidden" name="tv6" value="<?php if ($row2['stat']==0) {?>是<?php }else{?>否<?php }?>" />
          <img src="images/<?php if ($row2['stat']==0) {?>icon_21x21_selectboxon.gif<?php }else{?>icon_21x21_selectboxoff.gif<?php }?>" name="tv6_b" align="absmiddle" class="cursor" id="tv6_b" onclick="javascript:ra_select('tv6')" />(开启/禁止)<span class="STYLE2">* </span></td>
    </tr>
    <tr>
...
?>
    ...
    <tr>
      ...
      <td height="30" bordercolor="#CCCCCC"><select name="tmb" id="tmb">
          <?php if ($tmb!=1){?>
          <option value="0" <?php if ($row2['tmb']=="0"){?>selected="selected"<?php }?>>允许</option>
          <?php }?>
  ...
  <?php echo $rs1?>
    ...
    <tr>
      <td height="30" colspan="4" bordercolor="#CCCCCC"><table width="100%" border="1" cellpadding="3" cellspacing="1" bordercolor="f1f1f1">
          ...
          <?php $result = mysql_query("select * from  ka_quota where lx=0 and userid=".$_GET['id']." and flag=0 order by id ");   
					  $t=0;
