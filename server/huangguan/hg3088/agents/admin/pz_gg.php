...
$exe=mysqli_query($dbLink,"update ".DBPREFIX."adad set zlm='".$_POST['tm']."',zlm1='".$_POST['tm1']."'  Where id=1");
}
...
echo "<script>alert('���ֳɹ�!');window.location.href='index.php?action=pz_gg&ids=".$ids."&kithe=".$kithe."';</script>"; 
exit;
...
var zmzm1="index.php?action=look&kithe=<?php echo $kithe?>&class2="+arrTmp1[1]+"&lx=0&id=9&class3="+arrTmp1[2];
var zmzm2="index.php?action=look&lx=1&kithe=<?php echo $kithe?>&class2="+arrTmp1[1]+"&id=9&class3="+arrTmp1[2];
...
num2 = "<center>"+zmzm+"</center>"; //�ֶ�num2��ֵ
}else{
...
num4 = "<center><button class=headtd4  onmouseover=this.className='headtd3';window.status='����'; return true; onMouseOut=this.className='headtd4';window.status='����';return true;  onClick=ops('"+zmzm1+"',400,700)>"+arrTmp[2]+"</button></center>"; //�ֶ�num2��ֵ
...
while($image = mysqli_fetch_assoc($result)){
        ...
        <form name=form55 action="index.php?action=pz_gg&zsave=zsave&kithe=<?php echo $kithe?>&ids=<?php echo $ids?>" method=post>
          <td width="85" align="right" nowrap>������</td>
          <td width="30"><span class="STYLE2">
            <input name="tm" class="input1" id="tm" value='<?php echo $zlm?>' size="10" />
          </span></td>
          <td width="50" align="right" nowrap>��ˮ��</td>
          <td width="63"><input name="tm1" class="input1" type="text" id="tm1" value="<?php echo $zlm1?>" size="5"></td>
...
<DIV id=rs_window style="DISPLAY: none; POSITION: absolute">
 <form action="index.php?action=pz_gg&save=save&ids=<?php echo $ids?>&kithe=<?php echo $kithe?>"    method="post" name="form1" > <?php
include("d1.php");
?>
</form>
</div>
...
<SCRIPT language=javascript>
 makeRequest('index.php?action=server_gg&class1=����&class2=<?php echo $ids?>&kithe=<?php echo $kithe?>')</script>
