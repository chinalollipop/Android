...
<table width="100%" border="0" cellspacing="0" cellpadding="5">
  <tr class="tbtitle">
    <td width="15%"><strong><font color="#FFFFFF">[<?php echo $username?>]�ɶ�ע����ѯ[<?php echo $kithe?>��]</font></strong></td>
    <td width="48%">&nbsp;</td>
...
<table id="tb"  border="1" align="center" cellspacing="1" cellpadding="1" bordercolordark="#FFFFFF" bordercolor="f1f1f1" width="99%">
  ...
  $z_st=0;
		...
		$z_dai=0;
		 ...
		 $result = mysqli_query($dbLink,"select distinct(zong)   from   ka_tan where Kithe='".$kithe."' and guan='$username' order by zong desc");   
$ii=0;
while($rs = mysqli_fetch_assoc($result)){



$result1 = mysqli_query($dbLink,"Select sum(sum_m) as sum_m,count(*) as re,sum(sum_m*dagu_zc/10) as dagu_zc,sum(sum_m*guan_zc/10) as guan_zc,sum(sum_m*zong_zc/10) as zong_zc,sum(sum_m*dai_zc/10) as dai_zc,sum(sum_m-sum_m*user_ds/100) as sum_st from ka_tan   where Kithe='".$kithe."' and zong='".$rs['zong']."'");
...
?>
  <tr >
    ...
    <td align="center" nowrap="nowrap" bordercolor="cccccc"><button onClick="javascript:location.href='index.php?action=x3&kithe=<?php echo $kithe?>&username=<?php echo $rs['zong']?>'"  class="headtd4" style="width:80;height:22" ;><font color=ff6600>
      <?php echo $Rs5['sum_m']?>
    </font></button></td>
