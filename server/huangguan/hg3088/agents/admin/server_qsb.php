...
if($sum_tm[$ii]=="�첨"||$sum_tm[$ii]=="�̲�"||$sum_tm[$ii]=="����"){
$z_re_ds+=$Rs5['re'];
...
for($i=0;$i<$ii;$i++)
{
if($sum_tm[$i]=="�첨"||$sum_tm[$i]=="�̲�"||$sum_tm[$i]=="����"){
$sum_sx[$i]=$z_suma-$z_ds-$z_pz*((1-$qsb1/100))-($sum_ma[$i]-$sum_pz1[$i])*$sum_mbl[$i];
}elseif($sum_tm[$i]=="�Ͼ�"){
$sum_sx[$i]=$sum_ma[$i]-$sum_ds[$i]-$sum_pz1[$i]*((1-$qsb1/100))-($sum_ma[$i]-$sum_pz1[$i])*$sum_mbl[$i];
}
...
$ffxx=0;}else{
$ffxx=(-$sum_sx[$i]-$ztm_tm)/($sum_mbl[$i]+$qsb1/100-1);
/*
if ($i==0){
