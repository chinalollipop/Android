...
if ($_GET['ids']!="") {$ids=$_GET['ids'];}else{$ids="��벨";}
...
$exe=mysqli_query($dbLink,"update ".DBPREFIX."adad set bbb='".$_POST['tm']."',bbb1='".$_POST['tm1']."'  Where id=1");
}
...
echo "<script>alert('���ֳɹ�!');window.location.href='index.php?action=pz_bbb&ids=".$ids."&kithe=".$kithe."';</script>"; 
exit;
...
if (i!=(arrResult.length-2)){
var zmzm1="index.php?action=look&kithe=<?php echo $kithe?>&class2=<?php echo $ids?>&lx=0&id=61&class3="+arrTmp[12];
num4 = "<center><button class=headtd4  onmouseover=this.className='headtd3';window.status='��벨'; return true; onMouseOut=this.className='headtd4';window.status='��벨';return true;  onClick=ops('"+zmzm1+"',400,700)>"+arrTmp[2]+"</button></center>"; //�ֶ�num2��ֵ
...
var zmzm2="index.php?action=look&lx=1&kithe=<?php echo $kithe?>&class2=<?php echo $ids?>&id=61&class3="+arrTmp[12];
num11 = "<center><button class=headtd4  onmouseover=this.className='headtd3';window.status='��벨'; return true; onMouseOut=this.className='headtd4';window.status='��벨';return true;  onClick=ops('"+zmzm2+"',400,700)>"+arrTmp[9]+"</button></center>"; //�ֶ�num2��ֵ
}else{
...
function UpdateRate(commandName,inputID,cellID,strPara)
{
	...
	switch(commandName)
	{
		...
		makeRequest('index.php?action=server_bb&class1=��벨&class2=<?php echo $ids?>&kithe=<?php echo $kithe?>')
...
while($image = mysqli_fetch_assoc($result)){
			     echo "<OPTION value=index.php?action=pz_bbb&ids=".$ids."&kithe=".$image['nn'];
				 if ($kithe!="") {
				 if ($kithe==$image['nn']) {
        ...
        <form name=form55 action="index.php?action=pz_bbb&zsave=zsave&kithe=<?php echo $kithe?>&ids=<?php echo $ids?>" method=post>
          <td width="85" align="right" nowrap>������</td>
          <td width="30"><span class="STYLE2">
            <input name="tm" class="input1" id="tm" value='<?php echo $bbb?>' size="10" />
          </span></td>
          <td width="50" align="right" nowrap>��ˮ��</td>
          <td width="63"><input name="tm1" class="input1" type="text" id="tm1" value="<?php echo $bbb1?>" size="5"></td>
...
<DIV id=rs_window style="DISPLAY: none; POSITION: absolute">
 <form action="index.php?action=pz_bbb&save=save&ids=<?php echo $ids?>&kithe=<?php echo $kithe?>"    method="post" name="form1" > <?php
include("d1.php");
?>
</form>
</div>
...
<SCRIPT language=javascript>
 makeRequest('index.php?action=server_bbb&class1=��벨&class2=<?php echo $ids?>&kithe=<?php echo $kithe?>')</script>
