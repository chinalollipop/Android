...
if (rtype=='SP' && (eval(goldvalue) > <?php echo $maxnum?>)) {gold.focus(); alert("对不起,股东总信用额度最高限制 : <?php echo $maxnum?>!!"); return false;}
}
...
<div align="left">
  <?php $result=mysql_query("select * from ka_guan where id=".$_GET['id']."  order by id"); 
$row=mysql_fetch_array($result);

?>
  <table width="99%"  border="1" align="center" cellpadding="2" cellspacing="2" bordercolor="#ECE9D8">
    <form action="index.php?action=guan_edit&amp;act=添加&amp;id=<?php echo $_GET['id']?>" method="post" name="testFrm" id="testFrm" onsubmit="return SubChk()">
      <tr>
        ...
        <td width="49%" bordercolor="#CCCCCC"><input name="xm" type="text" class="input1"  id="xm" value="<?php echo $row['xm']?>" />
            <span class="STYLE2">*</span>
            <?php  $result1 = mysql_query("Select SUM(cs) As sum_m  From ka_guan Where lx=2 and   guanid=".$row['id']." order by id desc");
	$rsw = mysql_fetch_array($result1);
	if ($rsw[0]<>""){$mumu=$rsw[0];}
	...
	if ($rsw[0]<>""){$mkmk=$rsw[0];}else{$mkmk=0;}?>
          分配余额：
          <input name="kylllx2" type="text" class="input1" id="kylllx2" value="<?php echo $row['cs']-$mumu-$mkmk?>"  readonly="readonly" /></td>
      </tr>
      <tr>
        ...
        <td bordercolor="#CCCCCC"><input
					   ...
					   name="cs" type="text" class="input1"  id="cs" value="<?php echo $row['cs']?>" />
          可用信用额度：
      ...
      <tr>
        ...
        <td bordercolor="#CCCCCC"><input type="hidden" name="tv5" value="<?php if ($row['pz']==0) {?>是<?php }else{?>否<?php }?>" />
            <img src="images/<?php if ($row['pz']==0) {?>icon_21x21_selectboxon.gif<?php }else{?>icon_21x21_selectboxoff.gif<?php }?>" name="tv5_b" align="absmiddle" class="cursor" id="tv5_b" onclick="javascript:ra_select('tv5')" /> (充许走飞/禁止走飞)<span class="STYLE2">*</span></td>
        <td height="30" align="right" bordercolor="#CCCCCC" bgcolor="#FDF4CA">状态：</td>
        <td bordercolor="#CCCCCC"><input type="hidden" name="tv6" value="<?php if ($row['stat']==0) {?>是<?php }else{?>否<?php }?>" />
            <img src="images/<?php if ($row['stat']==0) {?>icon_21x21_selectboxon.gif<?php }else{?>icon_21x21_selectboxoff.gif<?php }?>" name="tv6_b" align="absmiddle" class="cursor" id="tv6_b" onclick="javascript:ra_select('tv6')" />(开启/禁止)<span class="STYLE2">* </span></td>
      </tr>
      <tr>
...
{
?>
            <option value="<?php   echo $bb; ?>"  <?php   if ($row["sj"]==$bb)
  {
...
{
?>
            <option value="<?php   echo $bb; ?>"  <?php   if ($row["sf"]==$bb)
  {
?> selected="selected"<?php   } ?>>
            <?php   switch ($bb)
  {
...
?>
      ...
      <tr>
        ...
        <td height="30" bordercolor="#CCCCCC"><select name="tmb" id="tmb">
            <option value="0" <?php if ($row['tmb']=="0"){?>selected="selected"<?php }?>>允许</option>
            <option value="1" <?php if ($row['tmb']=="1"){?>selected="selected"<?php }?>>不允许</option>
          </select>        </td>
        ...
        <td bordercolor="#CCCCCC"><input onblur="return CountGold2(this,'blur','SP');"
						...
						name="rs" type="text" class="input1"  id="rs" value="<?php echo $row['rs']?>" size="10" />
          <span class="STYLE2">
...
?>
          <tr>
            ...
            <td align="center" bgcolor="#FEFBE9"><input name="mm<?php echo $t?>" 
						   onblur="return CountGold1(this,'blur','MP','<?php echo $image['xx']?>','<?php echo $t?>');" onkeyup="return CountGold1(this,'keyup');"
						  class="input1" id="mm<?php echo $t?>" value='<?php echo $image['xx']?>' size="10" /></td>
            <td align="center" bgcolor="#FEFBE9"><input name="mmm<?php echo $t?>" 
						   
						   onblur="return CountGold1(this,'blur','XP','<?php echo $image['xxx']?>','<?php echo $t?>');" onkeyup="return CountGold1(this,'keyup');"
