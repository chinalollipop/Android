...
$exe=mysqli_query($dbLink,"update ".DBPREFIX."adad set bb='".$_POST['tm']."',bb1='".$_POST['tm1']."'  Where id=1");
}
...
echo "<script>alert('���ֳɹ�!');window.location.href='index.php?action=pz_bb&ids=".$ids."&kithe=".$kithe."';</script>"; 
exit;
...
var zmzm1="index.php?action=look&kithe=<?php echo $kithe?>&class2=<?php echo $ids?>&lx=0&id=6&class3="+arrTmp[12];
...
var zmzm2="index.php?action=look&lx=1&kithe=<?php echo $kithe?>&class2=<?php echo $ids?>&id=6&class3="+arrTmp[12];
...
while($image = mysqli_fetch_assoc($result)){
        ...
        <form name=form55 action="index.php?action=pz_bb&zsave=zsave&kithe=<?php echo $kithe?>&ids=<?php echo $ids?>" method=post>

          <td width="85" align="right" nowrap>������</td>
          <td width="30"><span class="STYLE2">
            <input name="tm" class="input1" id="tm" value='<?php echo $bb?>' size="10" />
          </span></td>
          <td width="50" align="right" nowrap>��ˮ��</td>

          <td width="63"><input name="tm1" class="input1" type="text" id="tm1" value="<?php echo $bb1?>" size="5"></td>
...
<DIV id=rs_window style="DISPLAY: none; POSITION: absolute">
 <form action="index.php?action=pz_bb&save=save&ids=<?php echo $ids?>&kithe=<?php echo $kithe?>"    method="post" name="form1" > <?php
include("d1.php");
?>
</form>
</div>
...
<SCRIPT language=javascript>
 ...
 makeRequest('index.php?action=server_bb&class1=�벨&class2=<?php echo $ids?>&kithe=<?php echo $kithe?>')</script>
