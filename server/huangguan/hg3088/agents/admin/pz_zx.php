...
if ($_GET['ids']!="") {$ids=$_GET['ids'];}//else{$ids="��ѡ";}
...
echo "<script>alert('���ֳɹ�!');window.location.href='index.php?action=pz_zx&ids=".$ids."&kithe=".$kithe."';</script>"; 
exit;
...
if (i!=(arrResult.length-2)){
var zmzm1="index.php?action=look&kithe=<?php echo $kithe?>&class2="+arrTmp[14]+"&lx=0&id=12&class3="+arrTmp[12];
...
var zmzm2="index.php?action=look&lx=1&kithe=<?php echo $kithe?>&class2="+arrTmp[14]+"&id=12&class3="+arrTmp[12];
...
while($image = mysqli_fetch_assoc($result)){
			     echo "<OPTION value=index.php?action=pz_zx&ids=".$ids."&kithe=".$image['nn'];
				 if ($kithe!="") {
				 if ($kithe==$image['nn']) {
        ...
        <form name=form55 action="index.php?action=pz_zx&zsave=zsave&kithe=<?php echo $kithe?>&ids=<?php echo $ids?>" method=post>
...
<DIV id=rs_window style="DISPLAY: none; POSITION: absolute">
 <form action="index.php?action=pz_zx&save=save&ids=<?php echo $ids?>&kithe=<?php echo $kithe?>"    method="post" name="form1" > <?php
include("d1.php");
?>
</form>
</div>
...
<SCRIPT language=javascript>
 makeRequest('index.php?action=server_zx&class1=��ѡ&class2=<?php echo $ids?>&kithe=<?php echo $kithe?>')</script>
