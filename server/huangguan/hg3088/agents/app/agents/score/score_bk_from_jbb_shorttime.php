...
$html_data=$curl->fetch_url("https://sb-results.188sbk.com/zh-cn/info-centre/sportsbook-info/results/2/normal/2?tzoff=-240");
...
foreach ($data as $k => $v){
    ...
    foreach ($item as $k2 => $v2){
        ...
        $res[$k][$k2]['M_Start'] = date('Y-m-d H:i:s',strtotime(trim(str_replace($a,$b,explode("<span>", $M_Start_others[0])[1])))-24*60*60);
...
$cou = count($datainfo);
if($cou>0){//可以抓到数据
    log_note("score_bk_from_jbb_shorttime"."\r\n");
    log_note(date('Y-m-d H:i:s')."\r\n");
