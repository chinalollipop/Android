			...
			for($i=0;$i<count($dataInfo);$i++){
				...
				$tg_inball_hr = $dataHalf[1];	//主队全场比分
					...
					*/
					...
					$sql="select MID,MB_Inball,TG_Inball,MB_Inball_HR,TG_Inball_HR,M_Start,Score_Source,M_League_tw,TG_Team_tw,MB_Team_tw from `".DBPREFIX.SPORT_FLUSH_MATCH_TABLE."` where Type='FT' and MID='$mid_m' and  M_Date='".$list_date."'";
...
//						if( $tg_inball_hr != $TG_Inball_HR_O && $tg_inball_hr != '' ){
						...
						foreach($updateArr as $key=>$val){
							...
							//if( ( $key=='MB_Inball' || $key=='TG_Inball' ) && ($val >0 || $val===0)){
								if(strtotime($row['M_Start'])+10*60 < time()){
									$tmp[]=$key.'=\''.$val.'\'';
								}
							}else{
								$tmp[]=$key.'=\''.$val.'\'';
...
//								    fclose($handle);
								...
								$mysql="update `".DBPREFIX.SPORT_FLUSH_MATCH_TABLE."` set ".implode(',',$tmp).",Checked='".$check."',Score_Source=1 where Type='FT' and M_Date='".$list_date."' and M_League_tw='".$row['M_League_tw']."' and TG_Team_tw='".$row['TG_Team_tw']."' and MB_Team_tw='".$row['MB_Team_tw']."'";
//                                print_r($mysql);
								mysqli_query($dbMasterLink, $mysql) or die('abc');

                                $sql="select MID,M_League_tw,TG_Team_tw,MB_Team_tw from `".DBPREFIX.SPORT_FLUSH_MATCH_TABLE."` where Type='FT' and  M_Date='".$list_date."' and M_League_tw='".$row['M_League_tw']."' and TG_Team_tw='".$row['TG_Team_tw']."' and MB_Team_tw='".$row['MB_Team_tw']."'";
