	...
	if($cou>0 && $data[0]!='' ){
            for ($i=1;$i<sizeof($data);$i++){
				...
				if ($tg_inball==$Score1){
					...
					$sql="select MB_Team,TG_Team,M_League,MB_Inball,Score_Source from `".DBPREFIX.SPORT_FLUSH_MATCH_TABLE."` where Type='FT' and MID='$mid_m' and  M_Date='".$list_date."'";
					...
					if ($cou>0){
						...
						if($mb_team<>"" or $tg_team<>""){
							$sqlq="select MID,MB_Inball,TG_Inball,MB_Inball_HR,TG_Inball_HR from `".DBPREFIX.SPORT_FLUSH_MATCH_TABLE."` where Type='FT' and M_League='$M_League' and  TG_Team='$TG_Team' and MB_Team='$MB_Team' and M_Date='".$list_date."'";
							...
							while($rowq = mysqli_fetch_assoc($resultq)){
								...
								if( ($mb_inball<0 && $mb_inball!=$rowq["MB_Inball"]) || ($mb_inball_hr<0 && $mb_inball_hr!=$rowq['MB_Inball_HR']) ){
										//-----------------取消注单---start
										if($mb_inball_hr==-51){//斩半场
											$sql="update `".DBPREFIX.SPORT_FLUSH_MATCH_TABLE."` set TG_Inball_HR='-2',MB_Inball_HR='-2',Checked=1,Score_Source=2,Cancel=1,Score=1 where `Type`='FT' and `MID`='".(int)$mid."'";
											mysqli_query($dbMasterLink,$sql) or die ("操作失败1");
										}elseif($mb_inball_hr==-52){//斩全场
											$sql="update `".DBPREFIX.SPORT_FLUSH_MATCH_TABLE."` set MB_Inball='-2',TG_Inball='-2',Checked=1,Score_Source=2,Cancel=1,Score=1 where `Type`='FT' and `MID`='".(int)$mid."'";
											mysqli_query($dbMasterLink,$sql) or die ("操作失败1");	
										}else{
											...
											if($beginFrom){
												...
												if( $rowCheck['Cancel']==0 ){
													...
													if($resultMem){
                                                       ...
                                                       if($m_result==''){ //未结算
															if($mb_inball_hr==-51 || $mb_inball_hr==-52){
																if($mb_inball_hr==-51&&in_array($rowCheck['LineType'],$halfLineType)){//斩半场
																	$u_sql = "update ".DBPREFIX.MEMBERTABLE." set Money=Money+$betscore where ID=".$userid;
																}
																if($mb_inball_hr==-52&&!in_array($rowCheck['LineType'],$halfLineType)){//斩全场
																	$u_sql = "update ".DBPREFIX.MEMBERTABLE." set Money=Money+$betscore where ID=".$userid;
																}
															}else{
																$u_sql = "update ".DBPREFIX.MEMBERTABLE." set Money=Money+$betscore where ID=".$userid;
