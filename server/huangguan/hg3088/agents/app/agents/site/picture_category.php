...
switch ($type){
    case 'del':
        $sqlPromos = "SELECT `id` FROM ". DBPREFIX . "web_carousel` WHERE category_id = {$id}";
        $resultPromos = mysqli_query($dbMasterLink, $sql);
        $count = mysqli_num_rows($resultPromos);
        if($count){
            exit(json_encode(['code' => -2, 'msg' => '请先删除此类型下的优惠图片！']));
        }
        $sql = "DELETE FROM `" . DBPREFIX . "web_picture_category` WHERE `id` = {$id}";
        ...
        break;
    case 'edit':
        $sql = "UPDATE `" . DBPREFIX . "web_picture_category` SET `name`='{$name}', `tag`='{$tag}', `status`='{$status}', `created_at`='{$now}', `updated_at`='{$now}' WHERE `id` = {$id}";
        $result = mysqli_query($dbMasterLink, $sql);
        if($result){
        ...
        break;
    case 'add':
        $sql = "INSERT INTO `" . DBPREFIX . "web_picture_category` VALUES ('','{$name}','{$tag}','{$status}','{$now}','{$now}')";
        $insertId = mysqli_query($dbMasterLink, $sql);
        if($insertId){
            echo "<script> alert('添加成功！'); </script>";
            echo "<meta http-equiv='Refresh' content='0;URL=picture_category.php?uid=$uid'>";
        }else{
...
$sql = "SELECT `id`, `name`, `tag`, `status`, `created_at`, `updated_at` FROM " . DBPREFIX . "web_picture_category";
...
<body>
<dl class="main-nav"><dt>图片类型管理</dt>
    <dd>
        &nbsp;<input type="button" class="za_button" onclick="location.href='picture.php?uid=<?php echo $uid;?>&lv=<?php echo $lv;?>&langx=<?php echo $langx;?>'" value="图片管理列表" />
    </dd>
</dl>
<div class="main-ui">
    <table  class="m_tab">
    ...
    <div id="adds" style="display: none;">
        <div class="connects">
            <form id="newsadd" method="post" action="">
                ...
                <table class="m_tab">
                    <tbody><tr><th>图片类型</th><th>图片标识</th><th>启用状态</th></tr>
                    <tr>
...
<script type="text/javascript">
    function btn_del(id,uid) {
        var type = 'del';
        if(confirm("确定删除此类型？请先删除掉此类型下所有优惠图片")){
            $.ajax({
                type: "POST",
                url:"picture_category.php",
                data:{id : id,uid: uid,type : type},
                success:function (response) {
                    response = $.parseJSON(response);
                    alert(response.msg);
                    if (response.code == 0){
                        window.location.href='picture_category.php?uid='+uid;
                    }
                }
            });
        }
