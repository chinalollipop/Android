...
if( $_REQUEST['is_search'] == 'Y'){ // 搜索数据
    ...
    $result_data = mysqli_query($dbLink,"select username, R_date_hour, COUNT(userid) as total_num, sum(count_pay) as count_pay, sum(R_total) as R_total, 
...
where R_date_hour BETWEEN '$sdate' and '$edate' ".$ac_sql." group by R_date_hour ");
...
<dl class="main-nav">
    <dt>时时返点查询</dt>
    <dd>
        <form name="myform"  action="rebate_hour.php" method="POST">
...
<div class="">
    ...
    <table class="m_tab">
        ...
        <tr  class="m_title" style="font-weight: bold;" >
            ...
            ?>
                <tr>
                    <td class="check_date"><?php echo $v['R_date_hour'];?></td>
                    ...
                    <td>
                        <div style="text-align: center"> <!-- rebate_user_search -->
                            <a style="display: inline-block;width: 48px;line-height: 23px;" class="za_button" href="rebate_hour_user_view.php?sdate=<?php echo $sdate;?>&edate=<?php echo $edate;?>&user_account=<?php echo $user_account?>" >查询</a>
                        </div>
                    </td>
                </tr>
            <?php } ?>
