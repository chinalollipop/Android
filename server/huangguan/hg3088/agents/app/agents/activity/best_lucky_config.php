...
switch ($type){
    case 'edit':
        ...
        foreach ($_REQUEST['probability'] as $id => $probability){
            $sum_probability=bcadd($sum_probability,$probability,2); //bcadd — 将两个高精度数字相加
        }
        if ($sum_probability!=1){
            echo "<script>alert('几率有误，几率综合不能大于或小于1，请重新填写几率重新修改');</script>";
        }else{
            $ids = implode(',',array_values($_REQUEST['id']));
            $sql = "UPDATE ".DBPREFIX."best_lucky_config SET money = CASE id ";
...
<body >
<dl class="main-nav"><dt style="width: 150px;">幸运大转盘 <a href="?uid=<?php echo $uid?>&langx=<?php echo $langx?>&type=Y"></a></dt><dd> </dd></dl>
<div class="main-ui">
    <table  class="m_tab">
        ...
        <form id="myform" name="myform" action="" method="post">
            ...
            foreach ($lists as $value){
                ...
                <tr  class=m_cen>
                    ...
                    <td>
                        <?php echo $value['best_lucky_content'];?>
                    </td>
                    <td>
