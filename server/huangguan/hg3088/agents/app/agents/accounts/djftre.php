...
$mysql = "select udp_djft_time,udp_time_djft from ".DBPREFIX."web_system_data";
$result = mysqli_query($dbLink,$mysql);
$row = mysqli_fetch_assoc($result);
$dffttiem=$row['udp_djft_time'];
$tiemdfft=$row['udp_time_djft'];
$timedjft=date('Y-m-d H:i:s',time()-1*$dffttiem);
$timesdjft=date('Y-m-d H:i:s',time()-1*$tiemdfft);
//$timeMin=date('Y-m-d H:i:s',time()-1*$dffttiem-150);
//$timesMin=date('Y-m-d H:i:s',time()-1*$tiemdfft-150);
$datetime=date('Y-m-d H:i:s');
$date=date('Y-m-d');
//神游~ 如果(今日非早盘)滚球危险球投注时间    < $timesdjft 则直接更新Danger=0 9:滚球让球|19:半场滚球让球|10:滚球大小|20:半场滚球大小|21:滚球独赢|31:半场滚球独赢
...
$sql = "update ".DBPREFIX."web_report_data set Danger=0,updateTime='".date('Y-m-d H:i:s',time())."' where Danger=1 and BetTime<'$timesdjft' and Cancel=0 and Active=1 and (".$methodStr.") and M_Result=''";
//mysqli_query($dbMasterLink,$sql);
//获取(今日非早盘)滚球危险球投注时间 	< $timedjft (系统配置秒数30-60s)的投注时间
$sqls = "select ID,Userid,MID,BetTime,Middle,BetScore,M_Name,M_Result,MB_Ball,TG_Ball from ".DBPREFIX."web_report_data where Danger=1 and BetTime<'$timedjft' and Cancel=0 and Active=1 and (".$methodStr.") and M_Result=''";
...
while ($rows = mysqli_fetch_array ($results)){
	...
	if($mb_ball==$ft_mb_ball and  $tg_ball==$ft_tg_ball){//$timedjft(系统设置时间30s-60s)内没有进球变化
