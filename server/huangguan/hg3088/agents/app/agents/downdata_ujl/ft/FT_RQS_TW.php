...
// 獲取刷新時間
$mysql = "select udp_ft_t from ".DBPREFIX."web_system_data where ID=1";
$result = mysqli_query($dbLink,$mysql);
$row = mysqli_fetch_assoc($result);
$settime = $row['udp_ft_t'];
...
for($page = 1; $page <= 10; $page ++) {
	$curl->set_referrer("" . FLUSH_WEBSITE_196 . "/touzhu/FT_Browser/FT_RQS_l.aspx");
    $htmlData = $curl->fetch_url("" . FLUSH_WEBSITE_196 . "/touzhu/FT_Browser/FT_RQS.aspx?p=" . $page);
...
if($dataCount > 0 and count($dataArray) > 0){
    ...
    foreach ($dataArray as $id => $ordinal) {
        ...
        $s_sql .= "WHEN $id THEN '$ordinal[3]' " ;
        $ts_sql .= "WHEN $id THEN '1' " ;
    }
