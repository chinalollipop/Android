...
for($page = 1; $page <= 10; $page ++) {
    $curl->set_referrer("" . FLUSH_WEBSITE_196 . "/touzhu/FT_Browser/FT_SBC_l.aspx");
    $htmlData = $curl->fetch_url("" . FLUSH_WEBSITE_196 . "/touzhu/FT_Browser/FT_SBC.aspx?p=" . $page);
...
if($dataCount > 0 and count($dataArray) > 0) {
    ...
    foreach ($dataArray as $id => $ordinal) {
        ...
        $tb_sql .= "WHEN $id THEN '$ordinal[4]' ";
        $mr_sql .= "WHEN $id THEN '$ordinal[5]' ";
        $tr_sql .= "WHEN $id THEN '$ordinal[6]' ";
        $tt_sql .= "WHEN $id THEN '$ordinal[7]' ";
        $pr_sql .= "WHEN $id THEN '$ordinal[8]' ";
        $mf_sql .= "WHEN $id THEN '$ordinal[9]' ";
        $hs_sql .= "WHEN $id THEN '1' " ;
    }
