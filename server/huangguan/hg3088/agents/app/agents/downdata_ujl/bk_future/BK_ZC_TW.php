...
// 獲取刷新時間
$mysql = "select udp_bk_r from ".DBPREFIX."web_system_data where ID=1";
$result = mysqli_query($dbLink,$mysql);
$row = mysqli_fetch_assoc($result);
$settime = $row['udp_bk_r'];
...
for($page = 1; $page <= 20; $page ++){
    $curl->set_referrer("" . FLUSH_WEBSITE_196 . "/touzhu/BK_Browser/BK_zc_l.aspx");
    $htmlData = $curl->fetch_url("" . FLUSH_WEBSITE_196 . "/touzhu/BK_Browser/BK_ZC.aspx?p=" . $page);
...
//    $htmlData = file_get_contents('./contents.php');
    ...
    if($count > 0){
        $start = 0;
        $insertSql = "INSERT INTO `".DBPREFIX.SPORT_FLUSH_MATCH_TABLE."` (MID,Type,M_Start,M_Date,M_Time,M_League,M_League_tw,MB_Team,TG_Team,MB_Team_tw,TG_Team_tw,M_LetB,MB_LetB_Rate,TG_LetB_Rate,MB_Dime,TG_Dime,TG_Dime_Rate,MB_Dime_Rate,S_Single_Rate,S_Double_Rate,M_Type,S_Show,more)VALUES";
        for($i = 0; $i < $count; $i++){
            $messages = $arrData[$i];
            ...
            if($check == 0){
                if($start == 0) {
                    $insertSql .= "('$dataInfo[0]','BU','$timestamp','$m_date','$m_time','$dataInfo[3]','$dataInfo[3]','$dataInfo[4]','$dataInfo[5]','$dataInfo[4]','$dataInfo[5]','$dataInfo[10]','$dataInfo[8]','$dataInfo[9]','$dataInfo[11]','$dataInfo[11]','$dataInfo[13]','$dataInfo[12]','$dataInfo[14]','$dataInfo[15]','$dataInfo[6]','1','0')" ;
                }else {
                    $insertSql .= ",('$dataInfo[0]','BU','$timestamp','$m_date','$m_time','$dataInfo[3]','$dataInfo[3]','$dataInfo[4]','$dataInfo[5]','$dataInfo[4]','$dataInfo[5]','$dataInfo[10]','$dataInfo[8]','$dataInfo[9]','$dataInfo[11]','$dataInfo[11]','$dataInfo[13]','$dataInfo[12]','$dataInfo[14]','$dataInfo[15]','$dataInfo[6]','1','0')" ;
                }
                $start ++;
...
$redisObj->setOne("BK_Future_Num",(int)$dataCount);
$updateCount = 0;
...
//        $mlh_sql .= "WHEN $id THEN '$ordinal[19]' " ;
        $mlrh_sql .= "WHEN $id THEN '$ordinal[14]' " ;
        $tlrh_sql .= "WHEN $id THEN '$ordinal[15]' " ;
...
//        $play_sql .= "WHEN $id THEN '$ordinal[36]' " ;
        ...
        $mtyp_sql .= "WHEN $id THEN '$ordinal[16]' " ;
        $updateCount ++;
    }
    if($updateCount>0){
        $sql .= $ty_sql.$e_sql.$m_sql.$e_sql.$t_sql.$e_sql.$l_sql.$e_sql.$jm_sql.$e_sql.$lm_sql.$e_sql.$mt_sql.$e_sql.$tt_sql.$e_sql.$mtw_sql.$e_sql.$ttw_sql.$e_sql.$stp_sql.$e_sql.$mr_sql.$e_sql.$mrr_sql.$e_sql.$trr_sql.$e_sql.$mbd_sql.$e_sql.$tgd_sql.$e_sql.$tgr_sql.$e_sql.$mbr_sql.$e_sql.$mlrh_sql.$e_sql.$tlrh_sql.$e_sql.$more_sql.$e_sql.$sshow_sql.$e_sql.$mtyp_sql;
