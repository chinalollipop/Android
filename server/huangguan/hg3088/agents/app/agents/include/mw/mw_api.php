...
if ($aUser['test_flag']){
    exit(json_encode( ['code' => '-2', 'msg' => '请使用真实账号登入MW电子'] ) );
}
    ...
    if(!$lyExist){
        ...
        if ($res['ret']=='0000'){
            ...
            if (!mysqli_query($dbMasterLink, $sql)) {
                exit(json_encode(['code' => -5, 'msg' => 'MW账号异常，请您稍后重试！'], JSON_UNESCAPED_UNICODE));
            }
            else{
                exit(json_encode(['code' => 200, 'msg' => 'MW账号初始化成功，请您继续转账'], JSON_UNESCAPED_UNICODE));
            }
        }else{
            exit(json_encode(['code' => -6, 'msg' => '注册MW账号失败'.json_encode($res)], JSON_UNESCAPED_UNICODE));
        }
...
switch ($action){
    ...
    case 'gameLobby':// 授权进入游戏大厅
        ...
        if ($res['ret']=='0000'){
            ...
            exit(json_encode(['code' => -6, 'msg' => 'MW账号授权失败'.json_encode($res)], JSON_UNESCAPED_UNICODE));
        }
...
//        print_r($res);die;
        if ($res['ret'] == '0000'){
            ...
            exit(json_encode( [ 'code'=>'-1','msg'=>'MW余额获取失败'.json_encode($res, JSON_UNESCAPED_UNICODE) ] ));
        }
        break;
    case 'hg2mw':
        ...
        if($lock){
            ...
            if ($lockMoney){
                ...
                if($up){
                    ...
                    if($in){
                        $moneyLogRes=addAccountRecords(array($userid,$aUser['UserName'],$aUser['test_flag'],$moneyf,$fShiftMoney*-1,$currency_after,44,6,$insertId,"MW电子游艺额度转换"));
                        if($moneyLogRes){
                            ...
                            if ($res['ret']!='0000'){
                                mysqli_rollback($dbMasterLink);
                                exit(json_encode( [ 'code'=>'-12','msg'=>'mw上分货币转入准备失败-'.$res['msg'].'-'.$transferOrderTime ] ));
                            }
                            else{
                                ...
                                if ($res['ret']!='0000'){
                                    mysqli_rollback($dbMasterLink);
                                    exit(json_encode( [ 'code'=>'-13','msg'=>'mw上分货币转入确认失败-'.$res['msg'] ] ));
                                }else{
    ...
    case 'mw2hg':
        ...
        if (floatval($mwGetbalance["balance"]) < floatval($b)){
            exit(json_encode( [ 'code'=>'-6','msg'=>'MW余额不足~~' ] ));
        }
        ...
        if ($res['ret']!='0000'){
            mysqli_rollback($dbMasterLink);
            exit(json_encode( [ 'code'=>'-12','msg'=>'mw下分货币转出准备失败-'.$res['msg'].'-'.$transferOrderTime ] ));
        }
        else{
            $toURL = $domainUrl.'api/transferPay?';
            $asinTransferOrderNo = $res['asinTransferOrderNo'];
            ...
            if ($res['ret']!='0000'){
                mysqli_rollback($dbMasterLink);
                exit(json_encode( [ 'code'=>'-13','msg'=>'mw下分货币转出确认失败-'.$res['msg'] ] ));
            }
            else{
                $data['userid']= $userid ;
                $data['Checked']=1;
                    ...
                    if($lockMoney){
                        ...
                        if($up){
                            ...
                            if($in){
                                ...
                                $moneyLogRes=addAccountRecords(array($userid,$aUser['UserName'],$aUser['test_flag'],$rowMoney['Money'],$fShiftMoney,$rowMoney['Money']+$fShiftMoney,45,6,$insertId,"MW电子游艺额度转换"));
                                if($moneyLogRes){
                                    mysqli_commit($dbMasterLink);
                                    mysqli_close($dbMasterLink);
                                    $data = [
