...
// 1.检测当前登录会员&查询会员信息
$stmt = $dbLink->prepare("SELECT `ID`, `Money`, `test_flag`, `UserName`, `Alias`, `Agents`, `World`, `Corprator`, `Super`, `Admin`, `Phone`, `birthday` FROM ".DBPREFIX.MEMBERTABLE." where `UserName` = ? and Status <= 1 LIMIT 1");
...
if ($aUser['test_flag']){
    exit(json_encode( ['code' => '-11', 'msg' => '请登录真实账号登入OG视讯！'] ) );
}
...
function og_do_login_redis_token(){
    ...
    if ($resp['success']){
        ...
        exit(json_encode(['code' => -4, 'msg' => 'getToken失败'.json_encode($resp)]));
    }
}
...
//                echo $sql;die;
                if (!mysqli_query($dbMasterLink, $sql)) {
                    exit(json_encode(['code' => -5, 'msg' => 'OG账号异常，请您稍后重试！']));
                }else{
                    exit(json_encode(['code' => 222, 'msg' => 'OG账号初始化成功，请您继续转账']));
                }
            }else{
                exit(json_encode(['code' => -6, 'msg' => '注册OG账号失败'.json_encode($res)]));
            }
        }
    }
}
...
//            exit( json_encode( ['code'=>0, 'balance_og'=>number_format($res["body"]['balance'],2) ] ) );
            $data = [
                'og_balance' => number_format($res["body"]['balance'],2),
                'hg_balance' => sprintf('%.2f', $aUser['Money'])
            ];
            exit( json_encode( ['code'=>0, 'data' => $data ] ) );
        }
        ...
        if($lock){
            ...
            if ($lockMoney){
                ...
                if($up){
                    ...
                    if($in){
                        $moneyLogRes=addAccountRecords(array($userid,$_SESSION['UserName'],$_SESSION['test_flag'],$moneyf,$fShiftMoney*-1,$currency_after,40,6,$insertId,"OG电子游艺额度转换"));
                        if($moneyLogRes){
                            ...
                            if (!$res['success']){
                                mysqli_rollback($dbMasterLink);
                                exit(json_encode( [ 'code'=>'-12','msg'=>'OG上分失败' ] ));
                            }
                            else{
                ...
                if($lockMoney){
                    ...
                    if($up){
                        ...
                        if($in){
                            ...
                            $moneyLogRes=addAccountRecords(array($userid,$aUser['UserName'],$aUser['test_flag'],$rowMoney['Money'],$fShiftMoney,$rowMoney['Money']+$fShiftMoney,41,6,$insertId,"OG电子游艺额度转换"));
                            if($moneyLogRes){
                                mysqli_commit($dbMasterLink);
                                mysqli_close($dbMasterLink);
                                $data = [
...
function og_game_key_redis_token(){
    ...
    if ($resp['success']){
        ...
        exit(json_encode(['code' => -4, 'msg' => '取得游戏金钥失败'.json_encode($resp)]));
    }
}