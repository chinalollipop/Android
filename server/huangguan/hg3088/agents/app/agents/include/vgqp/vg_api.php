...
//include_once "./ApiVg.php";
include_once ROOT_DIR. '/common/vgqp/api.php';

$username = isset($_REQUEST['username']) && $_REQUEST['username'] ? trim($_REQUEST['username']) : '';

// 一.检测当前登录会员&查询会员信息
$stmt = $dbLink->prepare("SELECT `ID`, `Money`, `layer`, `test_flag`, `UserName`, `Agents`, `World`, `Corprator`, `Super`, `Admin`, `Phone` FROM " . DBPREFIX.MEMBERTABLE." where `UserName` = ? and Status <= 1 LIMIT 1");  //创建预编译对象
...
switch ($action){
    case "cm":  // 会员 免费试玩或立即游戏
        ...
        if($aResult['state'] == 0) { // vg登录接口成功
            ...
            exit(json_encode(['code' => 0, 'message' => $aResult['message'], 'url' => $aResult['value']])); // 登录成功
    ...
    case 'b':   //点击棋牌游戏页面   额度转换
        ...
        if($aResult['state'] == 0){ //获取余额成功
            ...
            exit(json_encode(['code' => $aResult['state'], 'message' => 'VG棋牌余额获取失败，请稍后重试！']));
        }
        break;
    case 'hg2vg':
        // 1.参数校验
