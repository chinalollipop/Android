...
createXMLHttpRequest("cHttp");
    ...
    cHttp.open("post", "test_tw.php", true);
  ...
  if(cHttp.readyState == 4) {
      if(cHttp.status == 200) {
          ...
          //setTimes(timeDoc);
		  if (cDoc==1){
		  news_html.innerHTML="繁體断水";
		  if (a!=cDoc){
		  sound_tw.fileName = "bgmusic.mid";
		  }
		  a=cDoc;
		  }else{
		  sound_tw.stop();
...
id="sound_tw" type=application/x-oleobject width=0 height=0 standby="Loading Microsoft Windows Media Player components..." VIEWASTEXT align=MIDDLE>
