...
createXMLHttpRequest("cHttp");
    ...
    cHttp.open("post", "test_en.php", true);
  ...
  if(cHttp.readyState == 4) {
      if(cHttp.status == 200) {
          ...
          //setTimes(timeDoc);
		  if (cDoc==1){
		  news_html.innerHTML="英文断水";
		  if (a!=cDoc){
		  sound_en.fileName = "back.mid";
		  }
		  a=cDoc;
		  }else{
		  sound_en.stop();
...
id="sound_en" type=application/x-oleobject width=0 height=0 standby="Loading Microsoft Windows Media Player components..." VIEWASTEXT align=MIDDLE>
