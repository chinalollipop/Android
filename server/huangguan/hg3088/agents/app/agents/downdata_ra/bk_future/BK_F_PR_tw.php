...
foreach($accoutArr as $key=>$value){ //在扩展表中获取账号重新刷水
	$curl->set_referrer("".$value['Datasite']."/app/member/BK_future/index.php?rtype=pr&uid=".$value['Uid']."&langx=".$langx."&mtype=3");
	$html_data=$curl->fetch_url("".$value['Datasite']."/app/member/BK_future/body_var.php?rtype=pr&uid=".$value['Uid']."&langx=".$langx."&mtype=3");
