...
foreach($accoutArr as $key=>$value){//在扩展表中获取账号重新刷水
    for($page_no=0;$page_no<10;$page_no++){
        $curl->set_referrer("".$value['Datasite']."/app/member/BK_future/index.php?rtype=p3&uid=".$value['Uid']."&langx=".$langx."&mtype=3");
        $html_data=$curl->fetch_url("".$value['Datasite']."/app/member/BK_future/body_var.php?rtype=r_main&g_date=ALL&uid=".$value['Uid']."&langx=".$langx."&mtype=3&page_no=".$page_no);
        ...
        if($cou>0){//可以抓到数据
            for($i=0;$i<$cou;$i++){
                ...
                $MID=$datainfo[0];
                $dataArray[$datainfo[0]]=(array($datainfo[7],$datainfo[8],$datainfo[9],$datainfo[10],$datainfo[11],$datainfo[12],$datainfo[13],$datainfo[14],$datainfo[15],$datainfo[16],$datainfo[17],$datainfo[18],$datainfo[19],$datainfo[61],$datainfo[62],$datainfo[63],$datainfo[64],$datainfo[65],$datainfo[66],$datainfo[67],$datainfo[68],$datainfo[96],$datainfo[97],$datainfo[98])); // 把数据放在二维数组里面
...
<table width="100" height="70" border="0" cellpadding="0" cellspacing="0">
  <tr> 
    <td width="100" height="70" align="center">
      ...
      <input type=button name=button value="繁體 <?php echo $cou_total; ?>" onClick="window.location.reload()"></td>
  </tr>
</table>
</body>
</html>
