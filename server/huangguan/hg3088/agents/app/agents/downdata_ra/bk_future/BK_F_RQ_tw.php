...
$settime=$refurbishTimeData[0]['udp_fu_v'];
...
foreach($accoutArr as $key=>$value){//在扩展表中获取账号重新刷水
	$curl->set_referrer("".$value['Datasite']."/app/member/BK_future/index.php?rtype=rq4&uid=".$value['Uid']."&langx=en-us&mtype=3");
	$html_data=$curl->fetch_url("".$value['Datasite']."/app/member/BK_future/body_var.php?rtype=rq4&uid=".$value['Uid']."&langx=en-us&mtype=3");
...
if($cou>0 and count($dataArray)>0) { //可以抓到数据
    ...
    $t_sql .="MB_LetB_Rate = CASE MID ";
    $l_sql .="TG_LetB_Rate = CASE MID ";
    $tp_sql .="MB_Dime = CASE MID ";
    $mb_sql .="TG_Dime = CASE MID ";
    $tb_sql .="TG_Dime_Rate = CASE MID ";
    $mr_sql .="MB_Dime_Rate = CASE MID ";
    $tr_sql .="S_Single_Rate = CASE MID ";
    $pr_sql .="S_Double_Rate = CASE MID ";
    $rq_sql .="RQ_Show = CASE MID ";
    foreach ($dataArray as $id => $ordinal) {
        ...
        $pr_sql .= "WHEN $id THEN '$ordinal[8]' " ; // 拼接SQL语句
        $rq_sql .= "WHEN $id THEN '1' " ; // 拼接SQL语句
    }
    $sql .= $m_sql.$e_sql.$t_sql.$e_sql.$l_sql.$e_sql.$tp_sql.$e_sql.$mb_sql.$e_sql.$tb_sql.$e_sql.$mr_sql.$e_sql.$tr_sql.$e_sql.$pr_sql.$e_sql.$rq_sql ;
