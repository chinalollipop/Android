...
foreach($accoutArr as $key=>$value){ //在扩展表中获取账号重新刷水
    for($page_no=0;$page_no<10;$page_no++){
        $curl->set_referrer("".$value['Datasite']."/app/member/BK_future/index.php?rtype=p3&uid=".$value['Uid']."&langx=zh-cn&mtype=3");
        ...
        for($i=0;$i<$cou;$i++){
            ...
            $dataArray[$datainfo[0]]=(array($datainfo[7],$datainfo[8],$datainfo[9],$datainfo[10],$datainfo[11],$datainfo[12],$datainfo[13],$datainfo[14],$datainfo[15],$datainfo[16],$datainfo[17],$datainfo[18],$datainfo[19])); // 把数据放在二维数组里面
...
if($cou_total>0 and count($dataArray)>0) { //可以抓到数据
    ...
    $sql .= $m_sql.$e_sql.$t_sql.$e_sql.$l_sql.$e_sql.$tg_sql.$e_sql.$mp_sql.$e_sql.$tp_sql.$e_sql.$mr_sql.$e_sql.$tr_sql.$e_sql.$sp_sql.$e_sql.$spd_sql.$e_sql.$mv_sql.$e_sql.$tv_sql.$e_sql.$mf_sql.$e_sql.$p3_sql ;
   // echo $sql ;
