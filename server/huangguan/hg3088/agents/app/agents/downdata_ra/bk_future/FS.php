...
$curl->set_referrer("".$site."/app/member/browse_FS/loadgame_R.php?uid=".$uid."&langx=zh-cn&mtype=3");
$html_data=$curl->fetch_url("".$site."/app/member/browse_FS/reloadgame_R.php?uid=".$uid."&langx=zh-cnchoice=ALL&LegGame=&pages=1&records=40&FStype=&area_id=&item_id=&rtype=fs");
...
//	$sql="update ".DBPREFIX.SPORT_FLUSH_FS_MATCH_TABLE." set MB_Team='$team',M_League='$datainfo[2]',M_Item='$datainfo[3]' where MID='$datainfo[0]'";
...
if($cou>0) { //可以抓到数据
    ...
    $m_sql .="MB_Team = CASE MID " ;
    $t_sql .="M_League = CASE MID ";
    $l_sql .="M_Item = CASE MID ";
