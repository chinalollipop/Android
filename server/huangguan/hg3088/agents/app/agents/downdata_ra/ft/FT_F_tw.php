...
foreach($accoutArr as $key=>$value){
	for($page_no=0;$page_no<15;$page_no++){		
		$curl->set_referrer("".$value['Datasite']."/app/member/FT_browse/index.php?rtype=f&uid=".$value['Uid']."&langx=".$langx."&mtype=4");
		$html_data=$curl->fetch_url("".$value['Datasite']."/app/member/FT_browse/body_var.php?rtype=f&uid=".$value['Uid']."&langx=".$langx."&mtype=4&page_no=".$page_no);
