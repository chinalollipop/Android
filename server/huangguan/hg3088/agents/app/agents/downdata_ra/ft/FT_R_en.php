...
foreach($accoutArr as $key=>$value){
    ...
    $xml_data=str_replace('&', 'and', $xml_data); // 特殊字符
...
$redisObj->setOne("FT_Today_Num",(int)$allcount);
...
if(sizeof($m)<1)
{
	...
	if ($row['msg_update']==1 and $msg_tw[0]!=''){
		$sql="update ".DBPREFIX."web_system_data set msg_system='$msg_tw[0]'";
		mysqli_query($dbMasterLink,$sql) or die ("公告更新操作失败!");		
	}
}
