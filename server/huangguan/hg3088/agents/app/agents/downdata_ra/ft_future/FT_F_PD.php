...
$curl->set_referrer("".$site."/app/member/FT_future/index.php?rtype=pd&uid=".$uid."&langx=zh-cn&mtype=3");
$allcount=0;
$dataArray= array() ; // 把需要的数据重新放在数组里面
for($page_no=0;$page_no<15;$page_no++){
    $html_data=$curl->fetch_url("".$site."/app/member/FT_future/body_var.php?rtype=pd&uid=".$uid."&langx=zh-cn&mtype=3&page_no=".$page_no);
    //echo $html_data;exit;
    $a = array(
    "if(self == top)",
    "<script>",
