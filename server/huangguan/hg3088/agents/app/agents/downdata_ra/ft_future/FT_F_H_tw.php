...
foreach($accoutArr as $key=>$value){
	for($page_no=0;$page_no<15;$page_no++){
		$curl->set_referrer("".$value['Datasite']."/app/member/FT_future/index.php?rtype=hr&uid=".$value['Uid']."&langx=".$langx."&mtype=3");
		$html_data=$curl->fetch_url("".$value['Datasite']."/app/member/FT_future/body_var.php?rtype=hr&uid=".$value['Uid']."&langx=".$langx."&mtype=3&page_no=".$page_no);
...
if($allcount>0 and count($dataArray)>0) { //可以抓到数据
    ...
    $tgr_sql .="TG_Win_Rate_H = CASE MID ";
    $mf_sql .="M_Flat_Rate_H = CASE MID ";
    $pr_sql .="H_Show = CASE MID ";
    foreach ($dataArray as $id => $ordinal) {
        ...
        $tgr_sql .= "WHEN $id THEN '$ordinal[9]' " ; // 拼接SQL语句
        $mf_sql .= "WHEN $id THEN '$ordinal[10]' " ; // 拼接SQL语句
        $pr_sql .= "WHEN $id THEN '1' " ; // 拼接SQL语句
    }
    $sql .= $m_sql.$e_sql.$t_sql.$e_sql.$l_sql.$e_sql.$tp_sql.$e_sql.$mb_sql.$e_sql.$tb_sql.$e_sql.$mr_sql.$e_sql.$tr_sql.$e_sql.$tt_sql.$e_sql.$tgr_sql.$e_sql.$mf_sql.$e_sql.$pr_sql ;
