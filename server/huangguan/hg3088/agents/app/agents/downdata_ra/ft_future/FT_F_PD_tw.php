...
$settime=$refurbishTimeData[0]['udp_fu_pd'];
...
if(SPORT_FLUSH_WAY == 'ra686'){
    global $FT_FUTURE_PD_API;
    // 取出早盘的联赛ID，根据联赛ID去捞取早盘的波胆数据，50个联赛请求一次
    $lid = $redisObj->getSimpleOne('FUTURE_FT_LID');
    $aLid = explode(',',$lid);
    $aLid2 = array_chunk_vertical($aLid, 50); // 分割成以50个联赛ID一组
    foreach ($aLid2 as $k => $v){
        $lid = implode(',',$v);
        $jsonData = $curl->fetch_url($FT_FUTURE_PD_API.$lid);  // 请求波胆主盘口
...
//		$curl->set_referrer("".$value['Datasite']."/app/member/FT_future/index.php?rtype=pd&uid=".$value['Uid']."&langx=".$langx."&mtype=3");
//		$html_data=$curl->fetch_url("".$value['Datasite']."/app/member/FT_future/body_var.php?rtype=pd&uid=".$value['Uid']."&langx=".$langx."&mtype=3&page_no=".$page_no);
//		$html_data=$curl->fetch_url("".$value['Datasite']."/app/member/FT_future/body_var.php?rtype=pd&uid=".$value['Uid']."&langx=".$langx."&mtype=4&page_no=".$page_no."&league_id=&hot_game=&g_date=ALL");
//        $matches = get_content_deal($html_data);
        ...
        if ($aData['status']=='success'){
            if (count($aData['classifier']['region'])>0){
                ...
                exit('success 没有足球早盘的联赛数据'.$langx);
            }
        }
        else{
            exit('error 没有足球早盘的联赛数据'.$langx);
        }
