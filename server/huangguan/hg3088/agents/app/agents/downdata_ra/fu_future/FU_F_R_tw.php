...
$curl->set_referrer("".$site."/app/member/browse/index.php?ptype=S&gtype=FU_future&&uid=".$uid."&langx=big5");
$html_data=$curl->fetch_url("".$site."/app/member/browse/var.php?ptype=S&rtype=&gtype=FU_future&pctype=&uid=".$uid."&langx=big5&ltype=3");
...
for($i=0;$i<$cou;$i++){
	...
	if($check==0){
		$sql = "INSERT INTO match_stock(MID,M_Start,M_Date,M_Time,MB_Team_tw,M_League_tw,MB_MID,TG_MID,MB_Dime_Rate,TG_Dime_Rate,S_Single,S_Double,F_S_Show) VALUES ('$mid','$timestamp','$m_date','$m_time','$mb_team_tw','$m_league_tw','$datainfo[2]','$datainfo[3]','$datainfo[17]','$datainfo[18]','$datainfo[15]','$datainfo[16]','1')";
		mysqli_query($dbMasterLink,$sql) or die ("操作失敗");
	}else{
		$sql = "update match_stock set MB_Team_tw='$mb_team_tw',M_Start='$timestamp',M_Date='$m_date',M_Time='$m_time',M_League_tw='$m_league_tw',MB_Dime_Rate='$datainfo[17]',TG_Dime_Rate='$datainfo[18]',S_Single='$datainfo[15]',S_Double='$datainfo[16]',F_S_Show=1 where MID='$mid'";
		mysqli_query($dbMasterLink,$sql) or die ("操作失敗!");
	}
}
