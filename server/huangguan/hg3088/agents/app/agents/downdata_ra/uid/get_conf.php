...
class get_conf extends clsTbsSql
{
	...
	function get_login()
	{
		...
		$curl->set_referrer(HTTPS_HEAD."://".$this->getserver."/");
		$html_date=$curl->fetch_url(HTTPS_HEAD."://".$this->getserver."/app/member/","",$this->outTime);
		if(!$html_date)
		{
			echo $curl->error_msg;
			exit;
		...
		if($this->tempUID)
		{
			...
			$html_date=$curl->send_post_data(HTTPS_HEAD."://".$this->getserver."/app/member/new_login.php",$login,"",$this->outTime);
			...
			if(!$html_date)
			{
				...
				if($uid[1])
				{
					$this->msg="成功获取uid:".$uid[1];
					$this->uid=$uid[1];
					$this->liveid=$liveid[1];
					switch ($this->langx)
					{
						case 'zh-cn':
							$UPDATE="UPDATE `".DBPREFIX."config` SET `v`='{$this->uid}' WHERE (`k`='DownUID_cn')";
							break;
						case 'en-us':
							$UPDATE="UPDATE `".DBPREFIX."config` SET `v`='{$this->uid}' WHERE (`k`='DownUID_en')";
							break;
						default:
							$UPDATE="UPDATE `".DBPREFIX."config` SET `v`='{$liveid[1]}' WHERE (`k`='liveid')";
							$this->Execute($UPDATE);
							$UPDATE="UPDATE `".DBPREFIX."config` SET `v`='{$this->uid}' WHERE (`k`='DownUID_tw')";
							break;
					}
					$this->Execute($UPDATE);
				}else {
