...
$mysql="select Uid,Uid_tw,Uid_en from ".DBPREFIX."web_system_data where ID=1";
...
$sql="select ID,Url_Addr from ".DBPREFIX."web_url_data order by id desc limit 15";
...
if(empty($_REQUEST['test'])){
	...
	foreach($_REQUEST['txt'] as $key=>$value){
		if(!empty($value) and preg_match("/^(http:\/\/(\d{1,2}|1\d{1,2}|2[0-5][0-4])[.](\d{1,2}|1\d{1,2}|2[0-5][0-4])[.](\d{1,2}|1\d{1,2}|2[0-5][0-4])[.](\d{1,2}|1\d{1,2}|2[0-5][0-4])|http:\/\/[\d|\w]+[.][\S]+)[^\/]$/",$value)){
			$urltmp=trim($value);
			$sqlTest="select * from ".DBPREFIX."web_url_data where Url_Addr = '$value';";
			$ress=mysqli_query($dbLink,$sqlTest);
			if(mysqli_num_rows($ress)==0){
				$sqlInsert="insert into ".DBPREFIX."web_url_data(Url_Addr) values ('$value');";
				mysqli_query($dbMasterLink,$sqlInsert);
			}
		}
	}
	...
	if(!empty($_REQUEST['chk'])){
		...
		$tmp_idd=$tmp_idd."-1";
		$sql_del="delete from ".DBPREFIX."web_url_data where id in ($tmp_idd)";
	}
	mysqli_query($dbMasterLink,$sql_del);
	
	$siteSQL="select * from ".DBPREFIX."web_url_data order by id desc limit 15";
	$result=mysqli_query($dbLink,$siteSQL);
	$row=mysqli_fetch_assoc($result);
	$cou=mysqli_num_rows($result);
	while($row){
