...
$settime=$refurbishTimeData[0]['udp_bk_rq'];
...
foreach($accoutArr as $key=>$value){
		$curl->set_referrer("".$value['Datasite']."/app/member/BK_browse/index.php?rtype=rq4&uid=".$value['Uid']."langx=".$langx."&mtype=4");
		$html_data=$curl->fetch_url("".$value['Datasite']."/app/member/BK_browse/body_var.php?rtype=rq4&uid=".$value['Uid']."&langx=".$langx."&mtype=4");
		...
		if($cou>0){//可以抓到数据
			for($i=0;$i<$cou;$i++){
                ...
                $dataArray[$datainfo[0]]=(array($datainfo[8],$datainfo[9],$datainfo[10],$datainfo[11],$datainfo[12],$datainfo[13],$datainfo[14],$datainfo[17],$datainfo[18])); // 把数据放在二维数组里面
			}
			break;
		} 
}
...
if($cou>0 and count($dataArray)>0) { //可以抓到数据
    ...
    $m_sql .="M_LetB = CASE MID " ;
    $l_sql .="MB_LetB_Rate = CASE MID ";
    $tp_sql .="TG_LetB_Rate = CASE MID ";
    $mb_sql .="MB_Dime = CASE MID ";
    $tb_sql .="TG_Dime = CASE MID ";
    $mr_sql .="TG_Dime_Rate = CASE MID ";
    $tr_sql .="MB_Dime_Rate = CASE MID ";
    $ss_sql .="S_Single_Rate = CASE MID ";
    $sd_sql .="S_Double_Rate = CASE MID ";
    $pr_sql .="RQ_Show = CASE MID ";
    foreach ($dataArray as $id => $ordinal) {
        $m_sql .= "WHEN $id THEN '$ordinal[0]' " ; // 拼接SQL语句
        $l_sql .= "WHEN $id THEN '$ordinal[1]' " ; // 拼接SQL语句
        $tp_sql .= "WHEN $id THEN '$ordinal[2]' " ; // 拼接SQL语句
        $mb_sql .= "WHEN $id THEN '$ordinal[3]' " ; // 拼接SQL语句
        $tb_sql .= "WHEN $id THEN '$ordinal[4]' " ; // 拼接SQL语句
        $mr_sql .= "WHEN $id THEN '$ordinal[5]' " ; // 拼接SQL语句
        $tr_sql .= "WHEN $id THEN '$ordinal[6]' " ; // 拼接SQL语句
        $ss_sql .= "WHEN $id THEN '$ordinal[7]' " ; // 拼接SQL语句
        $sd_sql .= "WHEN $id THEN '$ordinal[8]' " ; // 拼接SQL语句
        $pr_sql .= "WHEN $id THEN '1' " ; // 拼接SQL语句
    }
    $sql .= $m_sql.$e_sql.$l_sql.$e_sql.$tp_sql.$e_sql.$mb_sql.$e_sql.$tb_sql.$e_sql.$mr_sql.$e_sql.$tr_sql.$e_sql.$ss_sql.$e_sql.$sd_sql.$e_sql.$pr_sql ;
