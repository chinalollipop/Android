...
$keyList=DATA_CENTER_PREFIX."_FT_R_List";
$keyListLen = $dcRedisObj->lenMessage($keyList);
$keyListLen[0] = $keyListLen[0] > 2000 ? 2000 : $keyListLen[0]+1;

$result=[];
$insertKeys = "REPLACE INTO `".DBPREFIX.SPORT_FLUSH_MATCH_TABLE."` (MID,Type,MB_Team,TG_Team,MB_Team_tw,TG_Team_tw,MB_Team_en,TG_Team_en,M_Date,M_Time,M_Start,M_League,M_League_tw,M_League_en,M_Type,MB_MID,TG_MID)VALUES";
for($i=0;$i<$keyListLen[0];$i++) {
    $gid = '';
    $gid = $dcRedisObj->popMessage($keyList);
    if($gid[0]>0){
