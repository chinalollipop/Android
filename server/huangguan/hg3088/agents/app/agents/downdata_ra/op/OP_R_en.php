...
$mysql = "select datasite_en,Uid_en,udp_op_r from ".DBPREFIX."web_system_data where ID=1";
$result = mysqli_query($dbLink,$mysql);
$row = mysqli_fetch_assoc($result);
$uid =$row['Uid_en'];
$site=$row['datasite_en'];
$settime=$row['udp_op_r'];

$curl = new Curl_HTTP_Client();
$curl->store_cookies("cookies.txt"); 
$curl->set_user_agent("Mozilla/4.0 (compatible; MSIE 6.0; Windows NT 5.1)");
$curl->set_referrer("".$site."/app/member/OP_browse/index.php?rtype=r&uid=".$uid."&langx=en-us&mtype=3");
$html_data=$curl->fetch_url("".$site."/app/member/OP_browse/body_var.php?rtype=r&uid=".$uid."&langx=en-us&mtype=3");
//echo $html_data;exit;
$a = array(
"if(self == top)",
"<script>",
