...
function backUpBetData()
{
    ...
    $sql = 'INSERT INTO ' . DBPREFIX . 'fire_history_projects SELECT * FROM ' . DBPREFIX . 'fire_projects WHERE `modified_datetime` BETWEEN "' . $startTime . '" AND "' . $limitTime . '" AND result_status in (\'WIN\',\'LOSS\',\'DRAW\',\'CANCELLED\')';
    if(!$inserted = mysqli_query($dbMasterLink, $sql)){
        $dbMasterLink->rollback();
        exit("【" . date('Y-m-d H:i:s') . "】转移历史注单失败，原因：入库失败【{$inserted}】\n");
    }
    $sql = 'DELETE FROM ' . DBPREFIX . 'fire_projects WHERE `modified_datetime` <= "' . $limitTime . '"';
