...
function transfer_history_bill($stop_time, $conn){
	...
	// ag注单表中的数据都是已结算完毕的数据
	$sql = "insert into ".DBPREFIX."ag_buyu_projects_history(`id`,`BillId`,`userid`,`UserName`,`Agents`,`RoomId`,`RoomBet`,`BuyuUserId`,`BombFish`,`Hunted`,`FishType`,`FishLife`,`FishId`,`CannonBoost`,`CannonCost`,`FishCost`,
	`UserCashBefore`,`UserCashDelta`,`UserCashCurrent`,`SceneId`,`Time`,`UserCashPay`,`UserCashEarn`,`ProductId`,`UserType`,`JackPotComm`,`OperType`) select * from ".DBPREFIX."ag_buyu_projects where `Time` < '{$stop_time}' ";
	...
	$sql = "delete from ".DBPREFIX."ag_buyu_projects where `Time`<'{$stop_time}' ";
	$result=mysqli_query($conn, $sql);
	if(!$result) {
		$result=mysqli_query($conn, "ROLLBACK");
