...
function backUpBetData()
{
    ...
    $sql = 'INSERT INTO ' . DBPREFIX . 'kl_history_projects SELECT * FROM ' . DBPREFIX . 'kl_projects WHERE `gametime` BETWEEN "' . $startTime . '" AND "' . $limitTime . '"';
    if(!$inserted = mysqli_query($dbMasterLink, $sql)){
        $dbMasterLink->rollback();
        exit("【" . date('Y-m-d H:i:s') . "】转移历史注单失败，原因：入库失败【{$inserted}】\n");
    }
    $sql = 'DELETE FROM ' . DBPREFIX . 'kl_projects WHERE `gametime` <= "' . $limitTime . '"';
