...
function backUpBetData()
{
    ...
    $sql = 'INSERT INTO ' . DBPREFIX . 'web_third_projects_history_data SELECT * FROM ' . DBPREFIX . 'web_third_projects_data WHERE `bought_time` BETWEEN "' . strtotime($startTime) . '" AND "' . strtotime($limitTime) . '"';

    if(!$inserted = mysqli_query($dbMasterLink, $sql)){
        $dbMasterLink->rollback();
        exit("【" . date('Y-m-d H:i:s') . "】转移历史注单失败，原因：入库失败【{$inserted}】\n");
    }
    $sql = 'DELETE FROM ' . DBPREFIX . 'web_third_projects_data WHERE `bought_time` <= "' . strtotime($limitTime) . '"';
