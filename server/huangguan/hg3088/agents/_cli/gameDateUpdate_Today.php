...
$redisObj = new Ciredis();
	...
	$bill_ids = array('TODAY_FT_M_ROU_EO','TODAY_BK_M_ROU_EO','TODAY_FT_PD','TODAY_FT_HPD','TODAY_FT_T','TODAY_FT_F','TODAY_FT_P3','TODAY_BK_P3'); 	
...
function getTodayDataByMethod(swoole_process $worker) {
	...
	while($row=mysqli_fetch_assoc($result)){
        if (strpos($row['MB_Team'],'角球')!==false){ continue; }
...
switch ($rtype){
	...
	case "TODAY_BK_M_ROU_EO":$oldRtype='all';break;
	...
	case "TODAY_BK_P3":		 $oldRtype='p3';break;
}	
?>
<HEAD>
<TITLE>足球变数值</TITLE>
...
case "TODAY_FT_F":  //半场/全场
	...
	case "TODAY_FT_P3": // 综合过关
                ...
                foreach($resulTotal as $key=>$row){
					...
					$newDataArray[$row[MID]]['more']=$show;
					$newDataArray[$row[MID]]['all']=$row["more"];;
	...
	case "TODAY_BK_P3": // 综合过关
	        ...
	        foreach($resultArr as $key=>$row){
				...
				//echo $newDataArray[$MID]['ior_POUC'].'===='.$newDataArray[$MID]['ior_POUH'];
				$newDataArray[$MID]['str_odd'] =$Single;
				$newDataArray[$MID]['str_even']=$Double;
