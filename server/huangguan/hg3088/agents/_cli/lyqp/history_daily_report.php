...
function countDailyWinLoss()
{
    ...
    // 1. 判断数据是否已统计
    if($isRepeat){
        echo "【" . date('Y-m-d H:i:s') . "】乐游棋牌重新统计数据\n";
    }else{
//        $sql = "SELECT `id`, `last_report_date`, `is_count` FROM " . DBPREFIX . "ly_report_log
//            WHERE `is_count` = 1
...
//                exit("【" . date('Y-m-d H:i:s') . "】乐游棋牌统计日期错误【{$startTime}~{$endTime}】，最近最后一次统计日期【{$lastCountDate}】\n");
//            }
//        }
    }
    echo "【" . date('Y-m-d H:i:s') . "】乐游棋牌统计日期美东时间：【{$startTime}~{$endTime}】\n";

    // 2.统计
    // 2.1.删除重复历史统计数据&日志报表数据
    mysqli_query($dbMasterLink, "DELETE FROM " . DBPREFIX . "ly_history_report WHERE `count_date` >= '{$startTime}' AND `count_date` < '{$endTime}'");
    mysqli_query($dbMasterLink, "DELETE FROM " . DBPREFIX . "ly_report_log WHERE `last_report_date` >= '{$startTime}' AND `last_report_date` < '{$endTime}'");

    // 2.2.事务处理
    $dbMasterLink->autocommit(false);
    $sql = "SELECT `userid`, `username`, `is_test`, `agents`, SUM(1) AS `total_times`, SUM(`cellscore`) AS `total_cellscore`, SUM(`allbet`) AS `total_bet`, SUM(`profit`) AS `total_profit`, 
            SUM(`revenue`) AS `total_revenue`, DATE_FORMAT(game_endtime,'%Y-%m-%d') AS `count_date` 
    ...
    if($count == 0){
        $countDate[] = $startTime;
        echo "【" . date('Y-m-d H:i:s') . "】乐游棋牌暂无注单记录\n";
    }else{
        $countData = $countDate = [];
        ...
        $sql = "INSERT INTO " . DBPREFIX . "ly_history_report {$keys} VALUES {$values}";
        if(!$inserted = mysqli_query($dbMasterLink, $sql)){
            $dbMasterLink->rollback();
            exit("【" . date('Y-m-d H:i:s') . "】乐游棋牌统计报表生成失败，原因：入库报表失败【{$inserted}】\n");
        }
    }
    ...
    $sql = "INSERT INTO " . DBPREFIX . "ly_report_log {$keys} VALUES {$values} ON DUPLICATE KEY UPDATE `is_count` = 1";
    if(!$inserted = mysqli_query($dbMasterLink, $sql)){
        $dbMasterLink->rollback();
        exit("【" . date('Y-m-d H:i:s') . "】乐游棋牌统计报表生成失败，原因：入库日志表失败【{$inserted}】\n");
    }
    $dbMasterLink->commit();
    exit("【" . date('Y-m-d H:i:s') . "】乐游棋牌统计报表生成成功\n");
}
