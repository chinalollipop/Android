...
if (php_sapi_name() == "cli") {
    ...
    $logFile = $logPath . '/ly_get_bet_data_' . date('Ymd', strtotime('+ 12 hour'));
...
function getBetData($startTime, $endTime)
{
    ...
    // 1.调用三方接口拉取数据
    writeLog("【" . date('Y-m-d H:i:s') . "】实际拉取数据日期区间【" . date('Y-m-d H:i:s', $startTime / 1000) . '至' . date('Y-m-d H:i:s', $endTime / 1000) . "】，时间戳：【{$startTime}到{$endTime}】");
    ...
    // 2.查询乐游会员
    writeLog("【" . date('Y-m-d H:i:s') . "】乐游会员数【" . count($aUsername) . "】，查询会员【" . implode(',', $aUsername) . "】");
    $sql = 'SELECT `id`, `userid`, `username`, `agents`, `world`, `corporator`, `super`, `admin`, `is_test` FROM ' . DBPREFIX . 'ly_member_data WHERE `username` IN ("' . implode('","', $aUsername) . '")';
    $result = mysqli_query($dbLink, $sql) or die("【" . date('Y-m-d H:i:s') . "】乐游会员查询失败\n");
    $aUser = [];
    ...
    $sql = "INSERT IGNORE INTO `" . DBPREFIX . "ly_projects` {$keys} VALUES {$values}";
 ...
 * @return array|mixed
 */
function getLyApi($params){
    $oly = new LyApiProxy();
    try {
        ob_start();
        $res = $oly->lyMain($params);
