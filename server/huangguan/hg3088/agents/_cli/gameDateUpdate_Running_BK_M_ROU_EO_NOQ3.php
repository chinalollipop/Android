        ...
        <SCRIPT language=JavaScript>
            ...
            switch ($rtype){
                case "BK_M_ROU_EO": // 从今日赛事切换到滚球 滚球
                    ...
                    // 关闭篮球第三节（篮球从第三节结束，也就是第四节开始时不允许下注，下半场倒计时10分钟）
...
//	                    if( $mbteamLast==$datainfo[5] && $tgteamLast==$datainfo[6] && $leagueLast==$datainfo[2] ){
    ...
    $filesName=strtolower("Running".$open.$rtype.'_NOQ3').time().".html";
    ...
    unset($future_r_data);
    unset($newDataArray);
    $redis_error = $redisObj->setOne($rtype.'_NOQ3'.'_'.$open.'_URL',$filesName);
}
?>
