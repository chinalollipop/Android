...
function backUpBetData()
{
    ...
    $sql = 'INSERT INTO ' . DBPREFIX . 'jx_bbin_history_projects SELECT * FROM ' . DBPREFIX . 'jx_bbin_projects WHERE `WagersDate` BETWEEN "' . $startTime . '" AND "' . $limitTime . '"';
    if(!$inserted = mysqli_query($dbMasterLink, $sql)){
        $dbMasterLink->rollback();
        exit("【" . date('Y-m-d H:i:s') . "】转移历史注单失败，原因：入库失败【{$inserted}】\n");
    }
    $sql = 'DELETE FROM ' . DBPREFIX . 'jx_bbin_projects WHERE `WagersDate` <= "' . $limitTime . '"';
