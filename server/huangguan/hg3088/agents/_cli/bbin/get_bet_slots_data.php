...
if (php_sapi_name() == "cli") {
    ...
    $logFile = $logPath . '/bbin_get_bet_slots_data_' . date('Ymd', strtotime('+ 12 hour'));
...
function getBetData($startTime, $endTime)
{
    ...
    foreach ($aBetData as $key => &$value){
        ...
        if(in_array($value['GameType'] , ['38001'] )) { //BB捕鱼大师
            $GameKind = '38';
        }else if (in_array($value['GameType'] , ['30598' , '30599'] )) { //BB捕鱼达人
            $GameKind = '30';
        }else {
            $GameKind = isset($bbinGameTypeKind[$type]) ? $bbinGameTypeKind[$type] : '';    //BB电子
        }
