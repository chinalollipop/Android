...
if (php_sapi_name() == "cli") {
    ...
    $logFile = $logPath . '/bbin_get_bet_lottery_data_' . date('Ymd', strtotime('+ 12 hour'));
...
function getBetData($startTime, $endTime)
{
    ...
    $type = 'bet_lottery';  //彩票注单 彩票游戏(BB彩票)
    //$types = ['bet_lottery' , 'bet_3d'];  //彩票注单 3D彩票
    ...
    foreach ($aBetData as $key => &$value){
        ...
        $aTemp = [
            ...
            'GameKind' => isset($bbinGameTypeKind[$type]) ? $bbinGameTypeKind[$type] : '',  //GameKind BBIN彩票12
    ...
    $sql = "REPLACE  INTO `" . DBPREFIX . "jx_bbin_projects` {$keys} VALUES {$values}";
    //echo $sql;
