...
function backUpBetData()
{
    ...
    $sql = 'INSERT INTO ' . DBPREFIX . 'ky_history_projects SELECT * FROM ' . DBPREFIX . 'ky_projects WHERE `game_endtime` BETWEEN "' . $startTime . '" AND "' . $limitTime . '"';
    if(!$inserted = mysqli_query($dbMasterLink, $sql)){
        $dbMasterLink->rollback();
        exit("【" . date('Y-m-d H:i:s') . "】转移历史注单失败，原因：入库失败【{$inserted}】\n");
    }
    $sql = 'DELETE FROM ' . DBPREFIX . 'ky_projects WHERE `game_endtime` <= "' . $limitTime . '"';
