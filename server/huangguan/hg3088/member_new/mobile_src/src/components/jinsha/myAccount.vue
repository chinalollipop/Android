    ...
    <div class="content-center">
      ...
      <ul class="account-list">
        <li v-for="(lists,item) in menusList" :key="item">
          ...
          <a v-else-if="lists.path=='yhsq'" @click="openNewGame(baseSettingData.promo_url,'','no')" class="to-page" >
            <span :class="lists.iClassName"> </span>
            <p>{{lists.name}}</p>
          </a>
  ...
  data () {
    return {
      menusList:[
          ...
          { name:'优惠申请', className:'',iClassName:'fa-yhdt', path:'yhsq'},
          ...
          { name:'关于公司', className:'',iClassName:'fa-gytyc', path:'aboutus'},
...
<style scoped>
  ...
  .user_bg{text-align: center;background: url(/static/images/suncity/user_bg.png) no-repeat center top;height: 11.42rem;width: 100%;background-size: cover;}
  ...
  .account-list li a span{display: inline-block;vertical-align: top;width: 50px;height: 44px;margin: 9px 0 0 0;line-height: 32px;background: url(/static/images/suncity/user_icon.png) no-repeat;background-position: -5px -8px;transform: scale(.7);}
  ...
  .account-list li .fa-yhdt {background: url(/static/images/jinsha/yhdt_icon.png) no-repeat;transform: scale(.8);background-position: 4px 4px;}
  .account-list li a .fa-agent-log{background:url(/static/images/suncity/agent_icon.png) center no-repeat;}
</style>
