        ...
        <div class="swiper-wrapper">
          ...
          <div class="swiper-slide">
            <template v-for="(lists,item) in gamesList">
              <a class="game-list" :key="item" v-if="lists.hot=='yes' && gametypeList.indexOf(lists.type)>=0" @click="openNewGame(lists.path,lists.type)">
          ...
          <div class="swiper-slide">
            <template v-for="(lists,item) in gamesList">
              <a class="game-list" :key="item" v-if="item>2 && item<=4" @click="openNewGame(lists.path,lists.type)">
                <span class="game-icon" :class="lists.className" :style="{backgroundImage: 'url(/static/images/bet365/index/'+lists.className+'.png)'}"></span>
                <div class="right list-right">
                  <span class="hot"></span>
                  <span class="text">{{lists.name}}</span>
...
export default {
  ...
  data () {
    return {
      noticeMsg:'欢迎光临',
      gamesList:[
          { name:'系统体育', className:'hg_sports',hot:'yes', type:'hg',path:'sport'},
          ...
          { name:'信用玩法', className:'xycp',hot:'yes', type:'cp',path:'gameswin?gametype=cp'},
