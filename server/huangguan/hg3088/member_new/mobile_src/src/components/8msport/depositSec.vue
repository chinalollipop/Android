        ...
        <template v-if="show_type=='company'">
          <div class="payWay bank_deposit_3" data-area="bank_pay">
              ...
              <div class="bg_yy">
                  ...
                  <div id="seebank" class="seebank-div linear-color-1" @click="checkAction">查看银行账号</div>
                  <div class="bank_list" id="show_bank_list" v-show="!checkStatus">
                      <div class="bank_list_li bank_list_line" v-for="(list,index) in dataList" :key="index">{{list.bank_name}}<br>
                          <span :class="'bank_username_'+list.id">{{list.bank_user}}</span>
                          <a class="linear-color-1" :class="'cp_bank_username_'+list.id" :data-clipboard-target="'.bank_username_'+list.id">复制</a><br>
                          <span :class="'bank_useraccount_'+list.id">{{list.bank_account}}</span>
                          <a class="linear-color-1" :class="'cp_bank_username_'+list.id" :data-clipboard-target="'.bank_useraccount_'+list.id">复制</a>
                      </div>
                  </div>
...
<style scoped>
    ...
    .payWayTit{text-align:left;padding:.5rem 5%;color:#000;font-size:1.2rem}
    .sm_tip{font-size: .9rem;}
</style>
