...
//        @error_log('捕鱼王用户场景捕鱼结算数据共拉取到：'.$rowCount . PHP_EOL, 3, '/tmp/group/AGbuyuScene.log');
        ...
        try {
            ...
            foreach ($rows as $k => $v) {
                ...
                $tmp['BulletOutNum'] = 0;
                $tmp['RoomBet'] = $v['betx']; // 倍率
                $tmp['Cost'] = $v['totalbulletcost']; // 子弹总价值
                $tmp['Earn'] = $v['totalfishcost']; // 捕鱼总价值
                $tmp['RoomId'] = $v['roomid'];
                $tmp['userid'] = $id;
                $tmp['UserName'] = $v['username'];
                $tmp['Agents'] = $aUser['Agents'];
                $tmp['JpDraw'] = 0;
                $tmp['SceneEx'] = 0;
                $tmp['JackPotComm'] = $v['totaljpcontribute'];
                $tmp['Remark'] = $v['remark'];
                $tmp['OperType'] = $v['devicetype'];
                $tmp['BillHitNum'] = 0;
                $tmp['BillType'] = 0;
...
//        @error_log(date('Y-m-d H:i:s').'-捕鱼王游戏注单信息共拉取到：'.$rowCount . PHP_EOL, 3, '/tmp/group/AGbuyubet.log');
        ...
        try {
            ...
            foreach ($rows as $k => $v) {
                ...
                $tmp['UserCashBefore'] = $v['src_amount'];
                $tmp['UserCashDelta'] = $v['account'];
                $tmp['UserCashCurrent'] = $v['dst_amount'];
                $tmp['SceneId'] = $v['sceneid'];
                $tmp['Time'] = $v['billtime'];
                $tmp['UserCashPay'] = $v['account'];
                $tmp['UserCashEarn'] = $v['cus_account']>0?$v['cus_account']:0;
                $tmp['ProductId'] = $v['productid'];
                $tmp['UserType'] = '';
                $tmp['JackPotComm'] = $v['jackpotcontribute'];
                $tmp['OperType'] = $v['devicetype'];
