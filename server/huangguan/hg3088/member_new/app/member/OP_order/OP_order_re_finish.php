...
$mysql = "select MB_Team,TG_Team,MB_Team_tw,TG_Team_tw,MB_Team_en,TG_Team_en,M_Date,ShowTypeRB,M_League,M_League_tw,M_League_en,MB_Ball,TG_Ball,MB_Win_RB,TG_Win_RB,M_Flat_RB,MB_LetB_Rate_RB,TG_LetB_Rate_RB,M_LetB_RB,MB_Dime_Rate_RB,TG_Dime_Rate_RB,MB_Dime_RB,TG_Dime_RB,MB_MID,TG_MID from `".DBPREFIX.SPORT_FLUSH_MATCH_TABLE."` where Type='OP' and `MID`='$gid' and Open=1 and MB_Team!='' and MB_Team_tw!='' and MB_Team_en!=''";
$result = mysqli_query($dbLink,$mysql);
	...
	switch ($line){
  	case 21:
		...
		case "H":
			...
			$s_m_place=$s_mb_team;
			$w_m_rate=change_rate($open,$row["MB_Win_RB"]);
		...
		case "C":
			...
			$s_m_place=$s_tg_team;
			$w_m_rate=change_rate($open,$row["TG_Win_RB"]);
			$turn_url="/app/member/OP_order/OP_order_rm.php?gid=".$gid."&uid=".$uid."&type=".$type."&gnum=".$gnum."&odd_f_type=".$odd_f_type;
			$w_gtype='MC';
			break;
		case "N":
			...
			$w_m_place_en="Flat";
			$s_m_place=$Draw;
			$w_m_rate=change_rate($open,$row["M_Flat_RB"]);
			$turn_url="/app/member/OP_order/OP_order_rm.php?gid=".$gid."&uid=".$uid."&type=".$type."&gnum=".$gnum."&odd_f_type=".$odd_f_type;
			$w_gtype='MN';
			break;
		}
