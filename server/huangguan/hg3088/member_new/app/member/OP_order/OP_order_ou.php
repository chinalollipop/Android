...
$sql = "select UserName,Money,CurType,Pay_Type,OpenType,Language,OP_OU_Scene,OP_OU_Bet from ".DBPREFIX.MEMBERTABLE." where Oid='$uid' and Status=0";
...
$btset=singleset('OU');
$GMIN_SINGLE=$btset[0];
$GMAX_SINGLE=$row['OP_OU_Scene'];
$GSINGLE_CREDIT=$row['OP_OU_Bet'];
$open=$row['OpenType'];
$langx=$row['Language'];
...
$mysql = "select M_Date,M_Time,$mb_team as MB_Team,$tg_team as TG_Team,$m_league as M_League,MB_Dime,TG_Dime,MB_Dime_Rate,TG_Dime_Rate from `".DBPREFIX.SPORT_FLUSH_MATCH_TABLE."` where `M_Start`>now() and `MID`='$gid' and Cancel!=1 and Open=1 and $mb_team!=''";
...
if($cou==0){
	...
	if ($row['M_Date']==date('Y-m-d')){
		...
		$active=66;
		$class="OOM";
		$caption=$Order_Other.$Order_Early_Market.$Order_Over_Under_betting_order;
	}
    ...
    $sql = "select CS,OU from ".DBPREFIX."match_league where  $m_league='$M_League' and Type='OP'";
    $result = mysqli_query($dbLink,$sql);
