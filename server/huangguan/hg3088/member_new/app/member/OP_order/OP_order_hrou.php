...
$btset=singleset('ROU');
$GMIN_SINGLE=$btset[0];
$GMAX_SINGLE=$row['OP_ROU_Scene'];
$GSINGLE_CREDIT=$row['OP_ROU_Bet'];
$open=$row['OpenType'];
$langx=$row['Language'];
...
$sgid=$gid-1;
$mysql = "select M_Date,M_Time,$mb_team as MB_Team,$tg_team as TG_Team,$m_league as M_League,MB_Dime_RB_H,TG_Dime_RB_H,MB_Dime_Rate_RB_H,TG_Dime_Rate_RB_H,MB_Ball,TG_Ball from `".DBPREFIX.SPORT_FLUSH_MATCH_TABLE."` where Type='OP' and `MID`='$sgid' and Cancel!=1 and Open=1 and $mb_team!=''";
$result = mysqli_query($dbLink,$mysql);
...
if($cou==0){
	...
	$havesql="select sum(BetScore) as BetScore from ".DBPREFIX."web_report_data where M_Name='$memname' and MID='$sgid' and LineType=20 and Mtype='$mtype' and Active=1";
	$result = mysqli_query($dbLink,$havesql);
	$haverow = mysqli_fetch_assoc($result);  
	$have_bet=$haverow['BetScore']+0;  
	 
    $sql = "select CS,VROU from ".DBPREFIX."match_league where  $m_league='$M_League'";
    $result = mysqli_query($dbLink,$sql);
...
<form name="LAYOUTFORM" action="/app/member/OP_order/OP_order_hre_finish.php" method="post" onSubmit="return false">
  <div class="ord">
      ...
      <div class="main">
            ...
            <div class="gametype"><?php echo $U_39 ?></div>
