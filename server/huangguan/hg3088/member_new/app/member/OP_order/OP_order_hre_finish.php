...
case '20':
	...
	break;
case '19':
	$html_data=$curl->fetch_url("".$site."/app/member/OP_order/OP_order_hre.php?gid=$gid&uid=$suid&type=$type&gnum=$gnum&strong=$strong&odd_f_type=$odd_f_type");
	break;
case '31':
	$html_data=$curl->fetch_url("".$site."/app/member/OP_order/OP_order_hrm.php?gid=$gid&uid=$suid&type=$type&gnum=$gnum&odd_f_type=$odd_f_type");
	break;
}
preg_match("/其他上半/Usi",$html_data,$m_temp);
if(!$m_temp){ 
...
$mysql = "select MB_Team,TG_Team,MB_Team_tw,TG_Team_tw,MB_Team_en,TG_Team_en,M_Date,ShowTypeRB,ShowTypeHRB,M_League,M_League_tw,M_League_en,MB_Ball,TG_Ball,MB_Win_RB,TG_Win_RB,M_Flat_RB,MB_LetB_Rate_RB,TG_LetB_Rate_RB,M_LetB_RB,MB_Dime_Rate_RB,TG_Dime_Rate_RB,MB_Dime_RB,TG_Dime_RB,MB_Win_RB_H,TG_Win_RB_H,M_Flat_RB_H,M_LetB_RB_H,MB_Dime_Rate_RB_H,TG_Dime_Rate_RB_H,MB_Dime_RB_H,TG_Dime_RB_H,MB_MID,TG_MID from `".DBPREFIX.SPORT_FLUSH_MATCH_TABLE."` where Type='OP' and `MID`='$sgid' and Open=1 and MB_Team!='' and MB_Team_tw!='' and MB_Team_en!=''";
$result = mysqli_query($dbLink,$mysql);
...
if($cou==0){
	...
	case 31:
		...
		$caption=$Order_Other.$Order_1st_Half_Running_1_x_2_betting_order;
		switch ($type){
		case "H":
			$w_m_place=$w_mb_team;
			...
			$w_m_rate=$row["MB_Win_RB_H"];
			$turn_url="/app/member/OP_order/OP_order_hrm.php?gid=".$sgid."&uid=".$uid."&type=".$type."&gnum=".$gnum."&odd_f_type=".$odd_f_type;
			$w_gtype='VMH';
			break;
		case "C":
			$w_m_place=$w_tg_team;
	...
	case 19:
		...
		$btype="-<font color=red><b>[$Order_1st_Half]</b></font>";	
		$caption=$Order_Other.$Order_1st_Half_Running_Ball_betting_order;
		...
		case "H":
			...
			$turn_url="/app/member/OP_order/OP_order_hre.php?gid=".$sgid."&uid=".$uid."&type=".$type."&gnum=".$gnum."&strong=".$strong."&odd_f_type=".$odd_f_type;
			$w_gtype='VRRH';
			break;
		case "C":
			$w_m_place=$w_tg_team;
	...
	case 20:	
		...
		$btype="- <font color=red><b>[$Order_1st_Half]</b></font>";		
		$caption=$Order_Other.$Order_1st_Half_Running_Ball_Over_Under_betting_order;
		...
		case "C":
			...
			$w_m_rate=change_rate($open,$rate[0]);
			$turn_url="/app/member/OP_order/OP_order_hrou.php?gid=".$sgid."&uid=".$uid."&type=".$type."&gnum=".$gnum."&odd_f_type=".$odd_f_type;
			$w_gtype='VROUH';
			break;
		case "H":
