...
for ($i=0;$i<$teamcount;$i++){
	 ...
	 if($type!=""){
        $mysql = "select MB_Team,TG_Team,MB_Team_tw,TG_Team_tw,MB_Team_en,TG_Team_en,M_League,M_League_tw,M_League_en,M_P_Flat_Rate,MB_MID,TG_MID,MB_P_LetB_Rate,TG_P_Win_Rate,ShowTypeP,M_P_LetB,TG_P_LetB_Rate,MB_P_Dime,MB_P_Dime_Rate,TG_P_Dime,TG_P_Dime_Rate,M_Date,S_P_Single_Rate,S_P_Double_Rate,MB_P_Win_Rate_H,TG_P_Win_Rate_H,M_P_Flat_Rate_H,MB_P_LetB_Rate_H,ShowTypeHP,M_P_LetB_H,TG_P_LetB_Rate_H,M_P_LetB_H,MB_P_Dime_H,TG_P_Dime_H,TG_P_Dime_Rate_H from `".DBPREFIX.SPORT_FLUSH_MATCH_TABLE."` where Type='OP' and `M_Start`>now() and MID='$mid' and Cancel!=1 and Open=1 and MB_Team!='' and MB_Team_tw!='' and MB_Team_en!=''";
		...
		case 'HPMH':
			...
			$w_m_place_en=$w_mb_team_en;
			$place=$s_mb_team;
			$w_m_rate=change_rate($open,$row['MB_P_Win_Rate_H']);
			$Mtype='VMH';
			$sign   = 'VS.';
			$m_place='VMH';
			$mmid="(".$row['MB_MID'].")";
		...
		case 'HPMC':
			...
			$w_m_place_en=$w_tg_team_en;		
			$place=$s_tg_team;
			$w_m_rate=change_rate($open,$row['TG_P_Win_Rate_H']);	
			$Mtype='VMC';
			$sign   = 'VS.';
			$m_place='VMC';
			$mmid="(".$row['TG_MID'].")";
		...
		case 'HPMN':
			...
			$w_m_place_en="flat";
			$place=$Draw;
			$w_m_rate=change_rate($open,$row['M_P_Flat_Rate_H']);
			$Mtype='VMN';
			$sign   = 'VS.';
			$m_place='VMN';
			$mmid="";
