...
$sql = "select UserName,Money,CurType,Pay_Type,OpenType,Language,OP_RE_Scene,OP_RE_Bet from ".DBPREFIX.MEMBERTABLE." where Oid='$uid' and Status=0";
...
preg_match("/其他上半滚球交易单/Usi",$html_data,$m_temp);
if(!$m_temp){ 
...
$mysql = "select M_Date,M_Time,$mb_team as MB_Team,$tg_team as TG_Team,$m_league as M_League,ShowTypeHR,M_LetB_RB_H,MB_LetB_Rate_RB_H,TG_LetB_Rate_RB_H,MB_Ball,TG_Ball from `".DBPREFIX.SPORT_FLUSH_MATCH_TABLE."` where Type='OP' and `MID`='$sgid' and cancel!=1 and Open=1 and mb_team!='' and mb_team_tw!='' and mb_team_en!=''";
$result = mysqli_query($dbLink,$mysql);
...
if($cou==0){
	...
	$havesql="select sum(BetScore) as BetScore from ".DBPREFIX."web_report_data where M_Name='$memname' and MID='$sgid' and LineType=19 and Mtype='$mtype' and Active=1";
	$result = mysqli_query($dbLink,$havesql);
	$haverow = mysqli_fetch_assoc($result);  
	$have_bet=$haverow['BetScore']+0;

    $sql = "select CS,VRB from ".DBPREFIX."match_league where  $m_league='$M_League'";
    $result = mysqli_query($dbLink,$sql);
...
<title></title>
<link href="/style/member/mem_order_ft.css" rel="stylesheet" type="text/css">
</head>
...
<form name="LAYOUTFORM" action="/app/member/OP_order/OP_order_hre_finish.php" method="post" onSubmit="return false">
  <div class="ord">
    ...
    <div class="main">
      ...
      <div class="gametype"><?php echo $U_40 ?></div>
...
<input type="hidden" name="line_type" value="19">
