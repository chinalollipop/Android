...
$btset=singleset('R');
$GMIN_SINGLE=$btset[0];
$GMAX_SINGLE=OP_R_Scene;
$GSINGLE_CREDIT=OP_R_Bet;
$open=$row['OpenType'];
$langx=$row['Language'];
...
$mysql = "select M_Date,M_Time,$mb_team as MB_Team,$tg_team as TG_Team,$m_league as M_League,ShowTypeHR,M_LetB_H,MB_LetB_Rate_H,TG_LetB_Rate_H from `".DBPREFIX.SPORT_FLUSH_MATCH_TABLE."` where Type='OP' and `M_Start`>now() and `MID`='$gid' and Cancel!=1 and Open=1 and $mb_team!=''";
...
if($cou==0){
    ...
    $sql = "select CS,VR from ".DBPREFIX."match_league where  $m_league='$M_League' and Type='OP'";
    $result = mysqli_query($dbLink,$sql);
