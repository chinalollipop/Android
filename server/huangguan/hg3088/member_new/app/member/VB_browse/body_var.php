...
case "re":
	...
	for($i=0;$i<$cou;$i++){
		...
		$opensql = "select * from `".DBPREFIX.SPORT_FLUSH_MATCH_TABLE."` where Type='VB' and  MID='$datainfo[0]'";
...
case "pd":
	...
	break;
case "p":
	$mysql = "select MID,M_Date,M_Time,$mb_team as MB_Team,$tg_team as TG_Team,$m_league as M_League,MB_P_Win_Rate,TG_P_Win_Rate,MB_MID,TG_MID,ShowTypeP from `".DBPREFIX.SPORT_FLUSH_MATCH_TABLE."` where Type='VB' and `M_Start` > now( ) AND `M_Date` ='$m_date' and P_Show=1 and $mb_team<>'' order by M_Start,MID";
	$result = mysqli_query($dbMasterLink, $mysql);
