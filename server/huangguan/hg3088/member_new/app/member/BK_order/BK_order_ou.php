...
if($cou==0 && $couMore==0){
	...
	if($cou==0){
		$mysql = "select M_Date,M_League,MB_Team,TG_Team from `".DBPREFIX.SPORT_FLUSH_MATCH_TABLE."` where `m_start`>now() and `MID`='{$_REQUEST['id']}' and Cancel!=1 and Open=1 and $mb_team!=''";
...
<form name="LAYOUTFORM" action="/app/member/BK_order/BK_order_finish.php" method="post" onSubmit="return false">
  <div class="ord">
   	<div class="title"><h1>籃球</h1><div class="tiTimer" onClick="orderReload();"><span id="ODtimer">10</span><input type="checkbox" id="checkOrder" onClick="onclickReloadTime()" checked value="10"></div></div>
      <div class="main">
