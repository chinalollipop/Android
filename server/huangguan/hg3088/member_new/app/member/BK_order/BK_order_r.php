...
$mysqlL = "select `MID` from `".DBPREFIX.SPORT_FLUSH_MATCH_TABLE."` where `M_Start`>now() and MID='$gid' and Cancel!=1 and Open=1 and MB_Team!='' and $mb_team!=''";
...
if($cou==0 && $couMore==0){
	...
	if($cou==0){
		$mysql = "select M_Date,M_League,MB_Team,TG_Team,ShowTypeR from `".DATAHGPREFIX.SPORT_FLUSH_MATCH_TABLE."` where `m_start`>now() and `MID`='{$_REQUEST['id']}' and Open=1 and $mb_team!=''";
