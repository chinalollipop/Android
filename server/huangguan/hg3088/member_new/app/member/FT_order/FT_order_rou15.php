...
if($cou==0 || $couMore==0){
	...
	if($detailsData["sw_".$wtype]=="Y" && $detailsData["ior_".$wtype."O"]>0 && $detailsData["ior_".$wtype."U"]>0){
		$ior_Rate_H = $detailsData["ior_".$wtype."O"]; 
		$ior_Rate_C = $detailsData["ior_".$wtype."U"]; 
	}
	...
	switch ($type){
		case "O":
			if ($langx=="zh-cn"){
		        $M_Place="大&nbsp;".$detailsData['ratio_'.strtolower($rtype)];
			}else if ($langx=="zh-cn"){
		        $M_Place='大&nbsp;'.$detailsData['ratio_'.strtolower($rtype)];
			}else if ($langx=="en-us" or $langx=="th-tis"){
		        $M_Place='Over&nbsp;'.$detailsData['ratio_'.strtolower($rtype)];
			}
		...
		case "U":
			if ($langx=="zh-cn"){
		        $M_Place="小&nbsp;".$detailsData['ratio_'.strtolower($rtype)];
			}else if ($langx=="zh-cn"){
		        $M_Place='小&nbsp;'.$detailsData['ratio_'.strtolower($rtype)];
			}else if ($langx=="en-us" or $langx=="th-tis"){
		        $M_Place='Under&nbsp;'.$detailsData['ratio_'.strtolower($rtype)];
			}
			$M_Rate=change_rate($open,$rate[1]);
			$mtype='OUC';	
			break;
	}
	$havesql="select sum(BetScore) as BetScore from ".DBPREFIX."web_report_data where M_Name='$memname' and MID='$gid' and LineType=3 and Mtype='$mtype' and (Active=1 or Active=11)";  
	...
	if(substr($_REQUEST['wtype'],0,1)=="A"){ $gametype=$U_74.' '.$Running_Ball.':'.$U_74A.'-'.$U_OU; }
	if(substr($_REQUEST['wtype'],0,1)=="B"){ $gametype=$U_74.' '.$Running_Ball.':'.$U_74B.'-'.$U_OU; }
	if(substr($_REQUEST['wtype'],0,1)=="C"){ $gametype=$U_74.' '.$Running_Ball.':'.$U_74C.'-'.$U_OU; }
	if(substr($_REQUEST['wtype'],0,1)=="D"){ $gametype=$U_74.' '.$Running_Ball.':'.$U_74D.'-'.$U_OU; }
	if(substr($_REQUEST['wtype'],0,1)=="E"){ $gametype=$U_74.' '.$Running_Ball.':'.$U_74E.'-'.$U_OU; }
	if(substr($_REQUEST['wtype'],0,1)=="F"){ $gametype=$U_74.' '.$Running_Ball.':'.$U_74F.'-'.$U_OU; }
?>
<html>
<head>
