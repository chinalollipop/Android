...
if($gid%2==0){
    $mysql = "select `MID` from `".DBPREFIX.SPORT_FLUSH_MATCH_TABLE."` where `m_start`>now() and `MID`='$gid' and Cancel!=1 and Open=1 and $mb_team!=''";
}elseif($gid%2==1){
    //$mysql = "select `MID` from `".DBPREFIX.SPORT_FLUSH_MATCH_TABLE."` where `m_start`>now() and `MID`=$gid-1 and Cancel!=1 and Open=1 and $mb_team!=''";
    $mysql = "select `MID` from `".DBPREFIX.SPORT_FLUSH_MATCH_TABLE."` where `m_start`>now() and `MID`=$gid and Cancel!=1 and Open=1 and $mb_team!=''";
}
...
if($cou==0 || $couMore==0){
	...
	if($detailsData['sw_PG']=="Y" && $detailsData["ior_".$rtype]>0){
	...
	$caption=$Order_FL_Ball_In_betting_order; 
