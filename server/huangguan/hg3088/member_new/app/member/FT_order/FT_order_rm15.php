...
if($cou==0 || $couMore==0){
	...
	if( !$ior_Rate_H || !$ior_Rate_C || !$ior_Rate_N ){
	    echo attention("$Order_This_match_is_closed_Please_try_again",$uid,$langx);
	    exit;
	}
	...
	case "H":
		$M_Place=$MB_Team;
		$M_Rate=change_rate($open,$ior_Rate_H);
		$mtype='MH';
		break;
	case "C":
		$M_Place=$TG_Team;
		$M_Rate=change_rate($open,$ior_Rate_C);
		$mtype='MC';
		break;
	case "N":
		$M_Place=$Draw;
		$M_Rate=change_rate($open,$ior_Rate_N);
		$mtype='MN';
		break;
	}
    ...
    $sql = "select M from ".DBPREFIX."match_league where  $m_league='$M_League'";
    $result = mysqli_query($dbLink,$sql);
	...
	if(substr($_REQUEST['wtype'],0,1)=="A"){ $gametype=$U_74.' '.$Running_Ball.':'.$U_74A.'-'.$U_M; }
	if(substr($_REQUEST['wtype'],0,1)=="B"){ $gametype=$U_74.' '.$Running_Ball.':'.$U_74B.'-'.$U_M; }
	if(substr($_REQUEST['wtype'],0,1)=="C"){ $gametype=$U_74.' '.$Running_Ball.':'.$U_74C.'-'.$U_M; }
	if(substr($_REQUEST['wtype'],0,1)=="D"){ $gametype=$U_74.' '.$Running_Ball.':'.$U_74D.'-'.$U_M; }
	if(substr($_REQUEST['wtype'],0,1)=="E"){ $gametype=$U_74.' '.$Running_Ball.':'.$U_74E.'-'.$U_M; }
	if(substr($_REQUEST['wtype'],0,1)=="F"){ $gametype=$U_74.' '.$Running_Ball.':'.$U_74F.'-'.$U_M; }
...
<form name="LAYOUTFORM" action="/app/member/FT_order/FT_order_finish.php" method="post" onSubmit="return false">
  <div class="ord">
   	<div class="title"><h1 class="football_title">足球</h1><div class="tiTimer" onClick="orderReload();"><span id="ODtimer">10</span><input type="checkbox" id="checkOrder" onClick="onclickReloadTime()" checked value="10"></div></div>
    <div class="main">
      ...
      <div class="betdata">
          <p class="amount">
              交易金额：<input name="gold" type="text" class="txt" id="gold" onKeyPress="return CheckKey(event)" onKeyUp="return CountWinGold()" size="8" maxlength="10" placeholder="投注额"><span class="clean_bet_money" id="betClear"></span>
          </p>
