...
preg_match("/网球滚球/Usi",$html_data,$m_temp);
if(!$m_temp){ 
...
$mysql = "select * from `".DBPREFIX.SPORT_FLUSH_MATCH_TABLE."` where Type='TN' and `MID`='$gid' and Open=1 and MB_Team!='' and MB_Team_tw!='' and MB_Team_en!=''";
$result = mysqli_query($dbLink,$mysql);
...
if($cou==0){
	...
	case 9:
		...
		$caption=$Order_TN.$Order_Running_Ball_betting_order;
		...
		case "H":
			...
			$turn_url="/app/member/TN_order/TN_order_re.php?gid=".$gid."&uid=".$uid."&type=".$type."&gnum=".$gnum."&strong=".$strong."&odd_f_type=".$odd_f_type;
			$w_gtype='RRH';
			break;
		case "C":
			$w_m_place=$w_tg_team;
	...
	case 10:	
		...
		$caption=$Order_TN.$Order_Running_Ball_Over_Under_betting_order;
		...
		case "C":
			...
			$w_m_rate=change_rate($open,$rate[0]);
			$turn_url="/app/member/TN_order/TN_order_rou.php?gid=".$gid."&uid=".$uid."&type=".$type."&gnum=".$gnum."&odd_f_type=".$odd_f_type;
			$w_gtype='ROUH';
			break;
		case "H":
