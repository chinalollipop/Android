...
$btset=singleset('ROU');
$GMIN_SINGLE=$btset[0];
$GMAX_SINGLE=VB_ROU_Scene;
$GSINGLE_CREDIT=VB_ROU_Bet;
$open=$row['OpenType'];
$langx=$row['Language'];
...
$curl->set_referrer("".$site."/app/member/TN_index.php?rtype=re&uid=$suid&langx=zh-cn&mtype=3");
$html_data=$curl->fetch_url("".$site."/app/member/TN_order/TN_order_rou.php?gid=$gid&uid=$suid&type=$type&gnum=$gnum&odd_f_type=$odd_f_type");
preg_match("/网球滚球大小交易单/Usi",$html_data,$m_temp);
if(!$m_temp){ 
	echo attention("$Order_This_match_is_closed_Please_try_again",$uid,$langx);
	exit();
}
//////////////////////////////////////////////////
$mysql = "select M_Date,M_Time,$mb_team as MB_Team,$tg_team as TG_Team,$m_league as M_League,MB_Dime_RB,TG_Dime_RB,MB_Dime_Rate_RB,TG_Dime_Rate_RB,MB_Ball,TG_Ball from `".DBPREFIX.SPORT_FLUSH_MATCH_TABLE."` where Type='TN' and `MID`=$gid and Cancel!=1 and Open=1 and $mb_team!=''";
$result = mysqli_query($dbLink,$mysql);
...
if($cou==0){
	...
	$havesql="select sum(BetScore) as BetScore from ".DBPREFIX."web_report_data where M_Name='$memname' and MID='$gid' and LineType=10 and Mtype='$mtype' and Active=4";
	$result = mysqli_query($dbLink,$havesql);
	$haverow = mysqli_fetch_assoc($result);  
	$have_bet=$haverow['BetScore']+0;  
	  
    $sql = "select CS,ROU from ".DBPREFIX."match_league where  $m_league='$M_League' and Type='TN'";
    $result = mysqli_query($dbLink,$sql);
