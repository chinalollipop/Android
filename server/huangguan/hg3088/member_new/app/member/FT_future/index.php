...
$ft_cou_num = $dcRedisObj->getSimpleOne("FT_Future_Num");
$bk_cou_num = $dcRedisObj->getSimpleOne("BK_Future_Num");
?>
...
<meta http-equiv="Content-Type" content="text/html; charset=utf-8">
    ...
    <SCRIPT LANGUAGE="JAVASCRIPT">
        ...
        parent.frames.header.document.getElementById('FT_games').innerHTML = '<?php echo $ft_cou_num?>' ; // 足球数量
        parent.frames.header.document.getElementById('BK_games').innerHTML = '<?php echo $bk_cou_num?>' ; // 篮球数量
...
switch ($rtype){
	case "r":
        if ($isClosedDJFT) {
            $getRedisUrl=$redisObj->getSimpleOne("FUTURE_R_DJFT_{$open}_URL");
	...
	case "pd":
        if ($isClosedDJFT) {
            $getRedisUrl=$redisObj->getSimpleOne("FUTURE_PD_DJFT_{$open}_URL");
            if($getRedisUrl && file_exists($getRedisUrl)){
                $staticHtmlUrl = $getRedisUrl;
            }
        }
	...
	case "hpd":
        if ($isClosedDJFT) {
            $getRedisUrl=$redisObj->getSimpleOne("FUTURE_HPD_DJFT_{$open}_URL");
            if($getRedisUrl && file_exists($getRedisUrl)){
                $staticHtmlUrl = $getRedisUrl;
            }
        }
	...
	case "t":
        if ($isClosedDJFT) {
            $getRedisUrl=$redisObj->getSimpleOne("FUTURE_T_DJFT_{$open}_URL");
            if($getRedisUrl && file_exists($getRedisUrl)){
                $staticHtmlUrl = $getRedisUrl;
            }
        }
	...
	case "f":
        if ($isClosedDJFT) {
            $getRedisUrl=$redisObj->getSimpleOne("FUTURE_F_DJFT_{$open}_URL");
            if($getRedisUrl && file_exists($getRedisUrl)){
                $staticHtmlUrl = $getRedisUrl;
            }
        }
	...
	case "p3":
        if ($isClosedDJFT) {
            $getRedisUrl=$redisObj->getSimpleOne("FUTURE_FT_P3_DJFT_{$open}_URL");
            if($getRedisUrl && file_exists($getRedisUrl)){
                $staticHtmlUrl = $getRedisUrl;
            }
        }
