...
//var mid = "echo $uid;?>".match(/m\d*l\d*$/);
//mid = mid[0];
...
function reloadGameDataComplete(objData){
				...
				show_gameInfo(gid_ary[0],ObjDataFT);//game_info 头部信息主客队名称
				TV_title();
											...
											for(var s = 0;s < wtypeAndOU.length;s++){
												...
												for(var t=0; t<rtypeMap[tmpWtype].length; t++){
													...
													if(wtypeAndOU.length != 1){
														...
														str_rtype = top["str_"+tmpWtype1.substr(tmpWtype1.length-1)+str_rtype];
													}
...
function closeClickEvent(){
		...
		try{
            ...
            //parent.body_browse.document.getElementById('MFU').className="bodyset FUR";
            parent.body_browse.document.getElementById('box').style.display="";
            parent.body_browse.document.getElementById('p3Title').style.display="";
            parent.body_browse.document.getElementById('right_div').style.display="";
            parent.body_browse.document.getElementById('refresh_right').style.display="";
...
function betEvent(gid,rtype,ratio,wtype){
	...
	if(wtype!='NFS'){
		...
		var param = 'gametype=FT&gid='+old_gid+'&uid='+top.uid+'&rtype='+rtype+'&wtype=FS'+'&langx='+top.langx;
...
function getParam(gid,wtype,rtype,ratio){
	...
	console.log(rtype);
	console.log(GameFT);
	console.log(rateOnChange);
	*/
	
	//赔率效验
	if(wtype=='T' || wtype=='EO' || wtype=='HEO'){
		if(rtype=='0~1')	var rateCur=rateOnChange['T01'];	
		if(rtype=='2~3')	var rateCur=rateOnChange['T23'];
		if(rtype=='4~6')	var rateCur=rateOnChange['T46'];
		if(rtype=='OVER')	var rateCur=GameFT['ior_OVER'];		//暂时跳过没有数据
		if(rtype=='ODD')	var rateCur=rateOnChange['EOO'];	
		if(rtype=='HODD')	var rateCur=rateOnChange['HEOO'];	
		if(rtype=='EVEN')	var rateCur=rateOnChange['EOE'];	
		if(rtype=='HEVEN')	var rateCur=rateOnChange['HEOE'];	
		if(rateCur != ratio){
			alert('赔率被修改！');
			return false;
		}
	}else{
		if(rateOnChange[rtype] != ratio){
			//alert('赔率被修改！');
			//return false;
		}
	}
	...
	var param = 'gid='+gid+'&uid='+top.uid+'&odd_f_type='+top.odd_f_type+'&langx='+langx+'&rtype='+rtype;
...
function fixMoreWindow(){
	...
	var MFU = parent.document.getElementById('MFU');
	//parent.document.getElementById('showdata').width = MFU.offsetWidth;
	//parent.document.getElementById('showdata').height = get_max(tab_show.offsetHeight,MFU.offsetHeight);
...
function parse_ior(gid,rtype,ior_value){
	...
	if(ior_value!=""){
		...
		//parent.Mathfloor(ior_value);
		ior_value=Mathfloor(ior_value);
		//ior_value=parent.printf(ior_value,2);
		// 2017-07-28 CRM-230 單盤（without spread）玩法賠率的四捨五入邏輯 (會員端)
		//ior_value=parent.chgForm_Single_ratio(ior_value,rtype);
		ior_value=chgForm_Single_ratio(ior_value,rtype);
...
//	var wtype_chk_ary = [];
	...
	var strong;
	(wtype=="HR")?strong = gdata['hstrong']:strong = gdata['strong'];
	...
	//將 "OG","OT","TS","HTS" 從判斷式中拿掉, 球队得分大小,让球，大小 处理
	if( R_Regex.test(wtype) || OU_Regex.test(wtype) || wtype == 'EO' || wtype == 'HEO' || wtype == 'EOH' || wtype == 'EOC' || wtype == 'HEOH' || wtype == 'HEOC' ){
			var arry = new Array();
			if(wtype == 'EO' || wtype=='HEO' || wtype == 'EOH' || wtype == 'EOC' || wtype == 'HEOH' || wtype == 'HEOC') {
					...
					arry = get_other_ioratio("H",arry[0],arry[1],show_ior); // show_ior 固定为100 ，这个函数会造成球队得分大小赔率转换，后端也需要这样处理
	...
	$.each(ior, function(index, value, array) {
		rateOnChange[index] = value['ior'];	  
	});
...
<div id="div_model" style="display:none;" >
			...
			<!---------- R ---------->
		 	<table id="model_R" border="0" cellpadding="0" cellspacing="0" class="more_table2">
				...
				<!-- START DYNAMIC BLOCK: R -->  <!-- betEvent 替换 betOrder -->
				<tr class="*TR_CLASS*">
      ...
      <!---------- MTS ---------->
			...
			<!---------- F2G ---------->     
		 	<table id="model_F2G" border="0" cellpadding="0" cellspacing="0" class="more_table2">
				...
				<!--<tr class="more_white">                                                                                      
						<td id="*F2GN_GID*" onClick="betEvent('*GID*','F2GN','*IORATIO_F2GN*','F2G');" style="cursor:pointer"  class="*TD_CLASS_F2GN*" width="100%"><div class="more_font"><span class="m_team">两队都无</span><span class="m_red_bet" title="两队都无">*IORATIO_F2GN*</span></div></td>
				</tr>-->
			...
			<!---------- YC ---------->
		 	<table id="model_YC" border="0" cellpadding="0" cellspacing="0" class="more_table2">
				<tr>
						<th class="more_title4" colspan="3">
							<span style="float: left;">最先界外球 / 最后界外球</span>
							<span class="more_og"></span>
