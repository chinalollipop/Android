...
//    $data['enddate'] = '2018-03-30 04:59:00';
    ...
    if (AG_DATA_CENTER_SWITCH === TRUE) {
        ...
        $url = AG_DATA_CENTER_URL.'/dataApi.php?action=get_projects&platform='.$data['cagent'].$userPrefix.'&startdate='.$data['startdate'].'&enddate='.$data['enddate'];
        $res = file_get_contents($url);
        $res = json_decode($res,true);
        if ($res['total']>0){
            $oAg = new model_betlog($dbMasterLink);
            $oAg->zrsxInDb($res['rows'], $res['total'],$data['cagent'].$userPrefix."_");
        }
    }
    else{
    ...
    //    var_dump( $xml_array ); die;
        ...
        if ($xml_array['info'] == 0) {
            ...
            if ($total > 0 and $total <= 500 && $totalpages == 1) {
                @error_log('视讯：'.$data['startdate'].'-'.$data['enddate'] . PHP_EOL, 3, '/tmp/group/AGbet.log');
                $userLocalData=array();
                foreach ($xml_array['row'] as $k => $v){ // 获取自己平台用户数据
                    if(isset($v['@attributes'])) $v = $v['@attributes'];
                    if( strpos($v['playName'],$userPrefix."_") > 0 ){
                        $userLocalData[]=$v;
                    }
                }

                if(count($userLocalData)>0){
                    $oAg->zrsxInDb($userLocalData, count($userLocalData),$data['cagent'].$userPrefix."_");
                }else{
                    echo "\n\r-----------没有本平台用户,不作处理！------------\n\r";
                }
