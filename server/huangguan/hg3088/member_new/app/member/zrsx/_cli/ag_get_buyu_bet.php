...
//    $data['enddate'] = '2018-03-30 04:59:00';
    ...
    if (AG_DATA_CENTER_SWITCH === TRUE) {
        ...
        $url = AG_DATA_CENTER_URL . '/dataApi.php?action=get_buyu_projects&platform=' . $data['cagent'] . $userPrefix . '&startdate=' . $data['startdate'] . '&enddate=' . $data['enddate'];
        $res = file_get_contents($url);
        $res = json_decode($res, true);
        if ($res['total'] > 0) {
            $oAg = new model_betlog($dbMasterLink);
            $oAg->buyuDayuBetInDb($res['rows'], $res['total'], $data['cagent'] . $userPrefix . "_");
        }
    } else {
...
//            @error_log(date('Y-m-d H:i:s').'-捕鱼王获取游戏注单信息入库：'.date('Y-m-d H:i:s',$data['startdate']).'-'.date('Y-m-d H:i:s',$data['enddate']) . PHP_EOL, 3, '/tmp/group/AGbuyubet.log');
                ...
                if ($total == 1) {// 只获取到1条数据
                    ...
                    $oAg->buyuDayuBetInDb($tmp['row'], $rowCount, $agsxInitp['data_api_cagent'] . $userPrefix . "_");
                } else {

                    $oAg->buyuDayuBetInDb($xml_array['row'], $rowCount, $agsxInitp['data_api_cagent'] . $userPrefix . "_");
                }
...
//                @error_log(date("Y-m-d H:i:s") . ' : ' . $getBetUrl . PHP_EOL, 3, '/tmp/group/AGbuyubet.log');
                    ...
                    if (!isset($xml_array['row'][0])) {// 只获取到1条数据
                        $tmp['row'][0] = $xml_array['row'];
                        $oAg->buyuDayuBetInDb($tmp['row'], 1, $agsxInitp['data_api_cagent'] . $userPrefix . "_");
                    } else {
                        $oAg->buyuDayuBetInDb($xml_array['row'], count($xml_array['row']), $agsxInitp['data_api_cagent'] . $userPrefix . "_");
                    }
