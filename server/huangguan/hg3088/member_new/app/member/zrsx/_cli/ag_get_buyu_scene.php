...
//$data['pidtoken'] = $agsxInit['data_api_md5_key'];
...
//    $data['enddate'] = '2018-03-30 04:59:00';
    $data['cagent'] = $agsxInitp['data_api_cagent']; // 场景号
    if (isset($argv['1']) && isset($argv['2']) && isset($argv['3']) && isset($argv['4'])) {
        ...
        $time = time();
        $data['startdate'] = strtotime(date('Y-m-d H:i:s', $time - 600));
        $data['enddate'] = strtotime(date('Y-m-d H:i:s', $time - 300));
    }

    if (AG_DATA_CENTER_SWITCH === TRUE) {

        $url = AG_DATA_CENTER_URL . '/dataApi.php?action=get_buyu_scene&platform=' . $data['cagent'] . $userPrefix . '&startdate=' . $data['startdate'] . '&enddate=' . $data['enddate'];
        $res = file_get_contents($url);
        $res = json_decode($res, true);
        if ($res['total'] > 0) {
            $oAg = new model_betlog($dbMasterLink);
            $oAg->buyuDayuSceneInDb($res['rows'], $res['total'], $data['cagent'] . $userPrefix . "_");
        }
    } else {
        // $data['sceneid'] = ''; // 场景号
...
//            @error_log('捕鱼王用户场景捕鱼结算数据：'.date('Y-m-d H:i:s',$data['startdate']).'-'.date('Y-m-d H:i:s',$data['enddate']) . PHP_EOL, 3, '/tmp/group/AGbuyuScene.log');
                ...
                if (!isset($xml_array['row'][0])) {// 总共1页，请求一次只获取到1条数据
                    ...
                    $oAg->buyuDayuSceneInDb($tmp['row'], $rowCount, $agsxInitp['data_api_cagent'] . $userPrefix . "_");
                } else {

                    $oAg->buyuDayuSceneInDb($xml_array['row'], $rowCount, $agsxInitp['data_api_cagent'] . $userPrefix . "_");
                }
...
//                @error_log(date("Y-m-d H:i:s") . ' : ' . $getBetUrl . PHP_EOL, 3, '/tmp/group/AGbuyuScene.log');
                    ...
                    if (!isset($xml_array['row'][0])) {// 分页数大于1页，请求一次只获取到1条数据
                        $tmp['row'][0] = $xml_array['row'];
                        $oAg->buyuDayuSceneInDb($tmp['row'], 1, $agsxInitp['data_api_cagent'] . $userPrefix . "_");
                    } else {
                        $oAg->buyuDayuSceneInDb($xml_array['row'], count($xml_array['row']), $agsxInitp['data_api_cagent'] . $userPrefix . "_");
                    }
