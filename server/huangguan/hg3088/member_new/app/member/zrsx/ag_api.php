...
if ($_SESSION['Agents']=='demoguest'){
    exit(json_encode( ['err' => '-2', 'msg' => '请使用真实账号登入AG'] ) );
}
...
//            }
            ...
            if ($action == 'b'){
                exit( json_encode( ['err'=>0, 'balance_ag'=>number_format('0.00',2), 'balance_hg'=>formatMoney($row['Money']) ] ) );
            }
        }else{
...
//        break;
    case 'b': // 获取余额
        ...
        exit( json_encode( ['err'=>0, 'balance_ag'=>number_format($agGetbalance["balance"],2), 'balance_hg'=>formatMoney($row['Money']) ] ) );
        ...
        if($lock){
	                ...
	                if($up){
	                    ...
	                    if($in){
	                        ...
	                        $insertId=mysqli_insert_id($dbMasterLink);
	                        $rowMoney=mysqli_fetch_assoc($lockMoney);
	                    	$moneyLogRes=addAccountRecords(array($userid,$_SESSION['UserName'],$_SESSION['test_flag'],$rowMoney['Money'],$fShiftMoney,$rowMoney['Money']+$fShiftMoney,24,1,$insertId,"真人娱乐/电子游艺额度转换"));
	                    	if($moneyLogRes){
	                            mysqli_commit($dbMasterLink);
	                            mysqli_close($dbMasterLink);
	                        }else{
