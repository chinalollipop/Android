...
if ($aUser['test_flag']){
    exit(json_encode( ['err' => '-2', 'msg' => '请登录真实账号登入雷火电竞'] ) );
...
if($action=='getLaunchGameUrl'){ // 打开游戏，判断维护
    ...
    if ($aRow['state']==1 and in_array(1, $aTerminal)){
        exit(json_encode( [ 'err'=>'555','msg'=>'雷火电竞维护中，请选择其他游戏' ] ));
    }
}
...
if($action){
    ...
    if(!$tfExist){
        ...
        if($action == 'b'){ // 未创建账号前请求余额接口
            ...
            //$aRes = json_decode($res['body'], true);
            ...
            if ($aRes['success'] == true && $aRes['body']['member_code']) {
                ...
                if (!mysqli_query($dbMasterLink, $sql)) {
                    exit(json_encode(['err' => -5, 'msg' => 'thunFire账号异常，请您稍后重试！'], JSON_UNESCAPED_UNICODE));
                }else{
                    ...
                    if($action=='getLaunchGameUrl' && !($appRefer==13 || $appRefer==14)){ // 登录：初始化账号后直接进入游戏
                        ...
                        exit( json_encode(['err' => 200, 'msg' => 'thunFire账号初始化成功' ], JSON_UNESCAPED_UNICODE ) );
                    }
                }
            }else{
                exit(json_encode(['err' => -6, 'msg' => '注册thunFire账号失败'], JSON_UNESCAPED_UNICODE));
            }
        }
    }
}
...
switch ($action){
    case 'b':
        ...
        if (!$aRes['success'] || $aRes['body']['count'] !== 1){
            exit(json_encode( [ 'err'=>'-1','msg'=>'余额获取失败' ] ));
        }
        else{
            $data = [
            ...
            if ($lockMoney){
                ...
                if($up){
                    ...
                    if($in){
                        ...
                        $moneyLogRes=addAccountRecords(array($userid,$aUser['UserName'],$_SESSION['test_flag'],$moneyf,$fShiftMoney*-1,$currency_after,59,1,$insertId,"额度转换体育到雷火电竞"));
                        if($moneyLogRes){

                            $aRes = depositMember($operator_id, $aUser['UserName'], $fShiftMoney, $sTrans_no);
                            if (!$aRes['success']){
                                mysqli_rollback($dbMasterLink);
                                exit(json_encode( [ 'err'=>'-12','msg'=>'FIRE上分失败' ] ));
                            }
                            else{
                ...
                if($lockMoney){
                    ...
                    if($up){
                        ...
                        if($in){
                            ...
                            $moneyLogRes=addAccountRecords(array($userid,$aUser['UserName'],$_SESSION['test_flag'],$rowMoney['Money'],$fShiftMoney,$rowMoney['Money']+$fShiftMoney,60,1,$insertId,"额度转换雷火电竞到体育"));
                            if($moneyLogRes){
                                mysqli_commit($dbMasterLink);
                                mysqli_close($dbMasterLink);
