...
$sql = "select UserName,Money,CurType,Pay_Type,OpenType,Language,VB_M_Scene,VB_M_Bet from ".DBPREFIX.MEMBERTABLE." where Oid='$uid' and Status=0";
...
$btset=singleset('M');
$GMIN_SINGLE=$btset[0];
$GMAX_SINGLE=$row['VB_M_Scene'];
$GSINGLE_CREDIT=$row['VB_M_Bet'];
$open=$row['OpenType'];
$langx=$row['Language'];
...
$mysql = "select M_Date,M_Time,$mb_team as MB_Team,$tg_team as TG_Team,$m_league as M_League,MB_Win_Rate,TG_Win_Rate from `".DBPREFIX.SPORT_FLUSH_MATCH_TABLE."` where Type='VB' and `M_Start`>now() and `MID`='$gid' and Cancel!=1 and Open=1 and $mb_team!=''";
...
if($cou==0){
	...
	if ($row['M_Date']==date('Y-m-d')){
		...
		$active=55;
		$class="OVU";
		$caption=$Order_VB.$Order_Early_Market.$Order_1_x_2_betting_order;
	}
	...
	$havesql="select sum(BetScore) as BetScore from ".DBPREFIX."web_report_data where m_name='$memname' and MID='$gid' and linetype=1 and Mtype='$mtype' and (Active=5 or Active=55)";
	$result = mysqli_query($dbLink,$havesql);
	$haverow = mysqli_fetch_assoc($result);
	$have_bet=$haverow['BetScore']+0;
	
    $sql = "select CS,M from ".DBPREFIX."match_league where  $m_league='$M_League' and Type='VB'";
    $result = mysqli_query($dbLink,$sql);
