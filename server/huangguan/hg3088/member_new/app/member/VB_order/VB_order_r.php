...
$sql = "select UserName,Money,CurType,Pay_Type,OpenType,Language,VB_R_Scene,VB_R_Bet from ".DBPREFIX.MEMBERTABLE." where Oid='$uid' and Status=0";
...
$mysql = "select M_Date,M_Time,$mb_team as MB_Team,$tg_team as TG_Team,$m_league as M_League,ShowTypeR,MB_LetB_Rate,TG_LetB_Rate,M_LetB from `".DBPREFIX.SPORT_FLUSH_MATCH_TABLE."` where Type='VB' and `M_Start`>now() and `MID`='$gid' and Cancel!=1 and Open=1 and $mb_team!=''";
...
if($cou==0){
	...
	$havesql="select sum(BetScore) as BetScore from ".DBPREFIX."web_report_data where m_name='$memname' and MID='$gid' and linetype=2 and Mtype='$mtype' and (Active=5 or Active=55)";
	$result = mysqli_query($dbLink,$havesql);
	$haverow = mysqli_fetch_assoc($result);
	$have_bet=$haverow['BetScore']+0;

    $sql = "select CS,OU,R from ".DBPREFIX."match_league where  $m_league='$M_League' and Type='VB'";
    $result = mysqli_query($dbLink,$sql);
...
<form name="LAYOUTFORM" action="/app/member/VB_order/VB_order_finish.php" method="post" onSubmit="return false">
  <div class="ord">
    ...
    <div class="main">
      ...
      <div class="gametype"><?php echo $U_43 ?></div>
