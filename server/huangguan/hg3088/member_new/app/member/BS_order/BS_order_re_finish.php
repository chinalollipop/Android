...
case '10':
	...
	break;
case '9':
	$html_data=$curl->fetch_url("".$site."/app/member/BS_order/BS_order_re.php?gid=$gid&uid=$suid&type=$type&gnum=$gnum&strong=$strong&odd_f_type=$odd_f_type");
	break;
}
preg_match("/棒球滚球/Usi",$html_data,$m_temp);
if(!$m_temp){ 
...
$mysql = "select MB_Team,TG_Team,MB_Team_tw,TG_Team_tw,MB_Team_en,TG_Team_en,M_Date,ShowTypeRB,M_League,M_League_tw,M_League_en,MB_Ball,TG_Ball,MB_LetB_Rate_RB,TG_LetB_Rate_RB,M_LetB_RB,MB_Dime_Rate_RB,MB_Dime_Rate_RB,TG_Dime_Rate_RB,MB_Dime_RB,TG_Dime_RB,MB_MID,TG_MID from `".DBPREFIX.SPORT_FLUSH_MATCH_TABLE."` where Type='BS' and `MID`='$gid' and Open=1 and MB_Team!='' and MB_Team_tw!='' and MB_Team_en!=''";
$result = mysqli_query($dbLink,$mysql);
...
if($cou==0){
	...
	case 9:
		...
		$caption=$Order_BS.$Order_Running_Ball_betting_order;
		...
		case "H":
			...
			$turn_url="/app/member/BS_order/BS_order_re.php?gid=".$gid."&uid=".$uid."&type=".$type."&gnum=".$gnum."&strong=".$strong."&odd_f_type=".$odd_f_type;
			$w_gtype='RRH';
			break;
		case "C":
			$w_m_place=$w_tg_team;
	...
	case 10:	
		...
		$caption=$Order_BS.$Order_Running_Ball_Over_Under_betting_order;
		...
		case "C":
			...
			$w_m_rate=change_rate($open,$rate[0]);
			$turn_url="/app/member/BS_order/BS_order_rou.php?gid=".$gid."&uid=".$uid."&type=".$type."&gnum=".$gnum."&odd_f_type=".$odd_f_type;
			$w_gtype='ROUH';
			break;
		case "H":
