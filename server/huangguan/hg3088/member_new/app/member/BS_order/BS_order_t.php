...
$btset=singleset('T');
$GMIN_SINGLE=$btset[0];
$GMAX_SINGLE1=BS_EO_Scene;
$GSINGLE_CREDIT1=BS_EO_Bet;
$GMAX_SINGLE2=BS_T_Scene;
$GSINGLE_CREDIT2=BS_T_Bet;
$open=$row['OpenType'];
$langx=$row['Language'];
...
$mysql = "select M_Date,M_Time,$mb_team as MB_Team,$tg_team as TG_Team,$m_league as M_League,S_Single_Rate,S_Double_Rate,S_0_1,S_2_3,S_4_6,S_7UP from `".DBPREFIX.SPORT_FLUSH_MATCH_TABLE."` where `m_start`>now() and `MID`='$gid' and Cancel!=1 and Open=1 and $mb_team!=''";
//echo $mysql;exit;
...
if($cou==0){
	...
	$havesql="select sum(BetScore) as BetScore from ".DBPREFIX."web_report_data where m_name='$memname' and MID='$gid' and linetype='$linetype' and (Active=3 or Active=33)";
	$result = mysqli_query($dbLink,$havesql);
	$haverow = mysqli_fetch_assoc($result); 
	$have_bet=$haverow['BetScore']+0; 

if ($rtype=='ODD' or $rtype=='EVEN'){
    $sql = "select CS,EO,T from ".DBPREFIX."match_league where  $m_league='$M_League' and Type='BS'";
    $result = mysqli_query($dbLink,$sql);
    $league = mysqli_fetch_assoc($result);
    $bettop=$league['EO'];
}else{
    $sql = "select CS,T from ".DBPREFIX."match_league where  $m_league='$M_League' and Type='BS'";
    $result = mysqli_query($dbLink,$sql);
    $league = mysqli_fetch_assoc($result);
    $bettop=$league['T'];
}
