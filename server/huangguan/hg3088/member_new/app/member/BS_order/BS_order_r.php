...
$mysql = "select M_Date,M_Time,$mb_team as MB_Team,$tg_team as TG_Team,$m_league as M_League,ShowTypeR,MB_LetB_Rate,TG_LetB_Rate,M_LetB from `".DBPREFIX.SPORT_FLUSH_MATCH_TABLE."` where `M_Start`>now() and `MID`='$gid' and Cancel!=1 and Open=1 and $mb_team!=''";
...
if($cou==0){
	...
	if ($row['M_Date']==date('Y-m-d')){
		...
		$active=33;
		$class="OBE";
		$caption=$Order_BS.$Order_Early_Market.$Order_Handicap_betting_order;
	}
	...
	$havesql="select sum(BetScore) as BetScore from ".DBPREFIX."web_report_data where m_name='$memname' and MID='$gid' and linetype=2 and Mtype='$mtype' and (Active=3 or Active=33)";
	$result = mysqli_query($dbLink,$havesql);
	$haverow = mysqli_fetch_assoc($result);
	$have_bet=$haverow['BetScore']+0;

    $sql = "select CS,R from ".DBPREFIX."match_league where  $m_league='$M_League' and Type='BS'";
    $result = mysqli_query($dbLink,$sql);
