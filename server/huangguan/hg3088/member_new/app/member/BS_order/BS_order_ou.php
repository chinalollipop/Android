...
$btset=singleset('OU');
$GMIN_SINGLE=$btset[0];
$GMAX_SINGLE=BS_OU_Scene;
$GSINGLE_CREDIT=BS_OU_Bet;
$open=$row['OpenType'];
$langx=$row['Language'];
...
if($cou==0){
	...
	if ($row['M_Date']==date('Y-m-d')){
		...
		$active=33;
		$class="OBE";
		$caption=$Order_BS.$Order_Early_Market.$Order_Over_Under_betting_order;
	}
	...
	$havesql="select sum(BetScore) as BetScore from ".DBPREFIX."web_report_data where M_Name='$memname' and MID='$gid' and LineType=3 and Mtype='$mtype' and (Active=3 or Active=33)";  
