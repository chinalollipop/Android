...
$backupUrl = getSysConfig('backup_web_url');
...
<html ><head>
    ...
    <style>
        .deposit-ui #d20 {background-position: -1638px -369px;}
        .deposit-ui #d20 h1 {background-position: -690px -457px;}
        .deposit-ui #d20:hover h1 {background-position: -460px -460px;}
        .deposit-ui #d20:hover p {color: #28b226}
        .deposit-ui #d21 {background-position: -1456px -369px;}
        .deposit-ui #d21 h1 {background-position: -690px -340px;}
        .deposit-ui #d21:hover h1 {background-position: -460px -343px;}
    </style>
</head>
<body >
<!-- 充值方式列表 开始-->
<div class="pay-list-all">
    <div class="deposit-ui">
        ...
        <?php if(!empty($aUsdt_pay)) {  // USDT虚拟币支付
            foreach ($aUsdt_pay as $key => $val) {
                ?>
                <div id="d21" class="deposit-list">
                    <h1>USDT虚拟货币</h1>
                    <p>USDT虚拟货币扫码，及时入账</p>
                    <a href="bank_type_USDT.php?uid=<?php echo $uid; ?>&langx=<?php echo $langx; ?>&bankid=<?php echo $val['id']?>"
        ...
        <?php if(!empty($aThird_bank_pay_youhui)) {  // 三方网银autopay，
            foreach ($aThird_bank_pay_youhui as $k => $v){
            ?>
            <div id="d2" class="deposit-list">
                <h1>公司入款</h1>
                <p>人工转账，需要审核</p>
                <a href="pay_type_third_bank_youhui.php?uid=<?php echo $uid;?>&langx=<?php echo $langx;?>&pid=<?php echo $v['id'];?>" data-val="wx_pay">进入存款</a>
            </div>
            <?php
            }
        }?>
...
<!--    </div>-->
    <dl class="deposit-help">
        <dt>备用网址：<a href="<?php echo $backupUrl;?>" target="_blank"><?php echo $backupUrl;?>(易记收藏)</a></dt>
        <dd>
            <p>
                <strong>特别提示：</strong>
