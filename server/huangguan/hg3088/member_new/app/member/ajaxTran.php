	            ...
	            if($lockUser && $lockMoney){
	                ...
	                if($addFundRecordFrom){
	                    ...
	                    if($updateUserFundFrom){
	                    		$moneyLogRes=addAccountRecords(array($id,$userName,$rowMoney['test_flag'],$moneyf,$moneyCur,$currency_after,$logType,1,$inserId,$logDes));
	                    		if($moneyLogRes){
			                    	...
			                    	if($addFlagRecordFrom){
			                    		...
			                    		mysqli_query($dbMasterLink,"ROLLBACK");
			                    		echo json_encode(array('status'=>-16,'message'=>'添加中间记录失败！')); exit; 
			                    	}
	                    		}else{
	                    			mysqli_query($dbMasterLink,"ROLLBACK");
