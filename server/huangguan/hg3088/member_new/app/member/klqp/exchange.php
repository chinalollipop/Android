...
<table border="0" cellspacing="1" cellpadding="0" class="game" width="350" style="width:350px;">
    ...
    <tr class="b_rig">
        <td>快乐棋牌</td>
        <td align="left"><span id="kl_balance">加载中，请稍候...</span></td>
    </tr>
    <tr class="b_rig">
        <td>转账</td>
        <td align="left">
...
<script language="javascript">
    ...
    $(function () {
        if(agent != 'demoguest') {
            ...
            $.ajax({
                type: 'POST',
                url: '/app/member/klqp/kl_api.php?_=' + Math.random(),
                data: data,
                dataType: 'json',
                success: function (item) {
                    if (item.code == 0) {
                        $('#hg_balance').html(item.data.hg_balance);
                        $('#kl_balance').html(item.data.kl_balance);
            ...
            $.ajax({
                ...
                success: function (item) {
                    if (item.code == 0) {
                        ...
                        $('#hg_balance').html(item.data.hg_balance);
                        $('#kl_balance').html(item.data.kl_balance);
                        $('#kl_money', window.parent.body.document).html(item.data.kl_balance) ; // 更新父级余额
                        alert('转账成功，请查看余额！');
                    } else {
                        alert(item.message)
                    }
