...
//                    exit(json_encode(['err' => -5, 'msg' => 'OG账号异常，请您稍后重试！']));
                    ...
                    $status = '200';
                    $describe = 'OG账号初始化成功，请您继续转账';
                    original_phone_request_response($status,$describe,$aData);  // 这个地方不传data ，
                }
            }else{
...
//                    exit(json_encode( [ 'err'=>'-6','msg'=>'余额不足~~' ] ));
                ...
                if($up){
                    ...
                    if($in){
                        $moneyLogRes=addAccountRecords(array($userid,$_SESSION['UserName'],$_SESSION['test_flag'],$moneyf,$fShiftMoney*-1,$currency_after,40,$playSource,$insertId,"OG电子游艺额度转换"));
                        if($moneyLogRes){
                ...
                if($lockMoney){
                    ...
                    if($up){
                        ...
                        if($in){
                            ...
                            $moneyLogRes=addAccountRecords(array($userid,$_SESSION['UserName'],$_SESSION['test_flag'],$rowMoney['Money'],$fShiftMoney,$rowMoney['Money']+$fShiftMoney,41,$playSource,$insertId,"OG电子游艺额度转换"));
                            if($moneyLogRes){
                                mysqli_commit($dbMasterLink);
                                mysqli_close($dbMasterLink);
                                $balance[0] = [
