...
if($action){
    ...
    if(!$scExist){ // 未创建账号前请求余额接口
        if($action == 'b'){
            ...
            original_phone_request_response('401.2', '抱歉，请您先进入皇冠体育后再尝试转账！');
        }
    }
}
...
switch ($action){
        ...
        if($aResult['code'] == 1) { // 成功请求登录
            if($scExist == 0){ // 若无平台皇冠体育账号，则注册会员入库
                ...
                if (!mysqli_query($dbMasterLink, $sql)) {
                    original_phone_request_response('401.3', '皇冠体育账号异常，请您稍后重试！');
                }
            ...
            original_phone_request_response(200, 'SUCCESS', $aResult['data']);
        } else {
            original_phone_request_response('401.4', '皇冠体育账号异常，请您稍后重试！');
        }
        break;
    case 'b':
        ...
        if($aResult['code'] == 1){
            ...
            original_phone_request_response('401.5', '皇冠体育余额获取失败，请稍后重试！');
        }
        break;
    case 'hg2sc':
        // 1.参数校验
        if(!preg_match("/^[1-9][0-9]*$/", $score))
            original_phone_request_response('401.6', '转账金额格式错误，请重新输入！');

        if ($score > 10000000){
            original_phone_request_response('401.7', '单次上分不能超过一千万，请重新输入！');
        }
        ...
        if(intval($beforeBalance) < intval($score)) { // 余额不足
            original_phone_request_response('401.8', '钱包中心余额不足！');
        }
        ...
        if(!$updated = mysqli_query($dbMasterLink, 'UPDATE ' . DBPREFIX.MEMBERTABLE.' SET `Money` = ' . $afterBalance . ' WHERE `ID` = ' . $aUser['ID'])) {
            ...
            original_phone_request_response('401.9', '额度转换失败，请您稍后重试！');
        }
        // 皇冠体育-orderId
    ...
    case 'sc2hg':
        // 1.参数校验
        if(!preg_match("/^[1-9][0-9]*$/", $score))
            original_phone_request_response('401.15', '转账金额格式错误，请重新输入！');

        if ($score > 10000000){
            original_phone_request_response('401.16', '单次上分不能超过一千万，请重新输入！');
        }
        // 2.查询皇冠体育可下分余额
        $aResult = checkBalance($aUser['UserName']);
        if(isset($aResult['code']) && $aResult['code'] == 1){
            ...
            original_phone_request_response('401.17', '皇冠体育余额获取失败，请稍后重试！');
        }
        if(intval($scBalance) < intval($score))
            original_phone_request_response('401.18', '皇冠体育余额不足！');
        ...
        if(!$updated = mysqli_query($dbMasterLink, 'UPDATE ' . DBPREFIX.MEMBERTABLE.' SET `Money` = ' . $afterBalance . ' WHERE `ID` = ' . $aUser['ID'])) {
            $dbMasterLink->rollback();
            original_phone_request_response('401.19', '额度转换失败，请您稍后重试！');
        }
        // 皇冠体育-orderId
        $orderId = uniqid($aUser['UserName'], true);
        ...
        break;
    default:
        original_phone_request_response('401.25', '抱歉，您的请求不予处理！');
        break;
}
