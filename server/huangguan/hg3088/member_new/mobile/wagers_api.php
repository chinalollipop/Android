...
// 交易状况页面为未结算注单
$sql = "select ID,MID,LineType,Active,Gtype,M_Date,BetTime,orderNo,$middle as Middle,$bettype as BetType,BetScore,Gwin,OddsType,Cancel,Danger,Confirmed,M_Result,Checked,ShowType from ".DBPREFIX."web_report_data where Gtype ='$gtype' and M_Name='$name' ".$sWhere." order by orderby,BetTime desc";
//echo $sql;
...
foreach ($data2 as $k => $row){
    ...
    $data['rows'][$k]['BetScore']=floor($row['BetScore']);
...
foreach ($data['rows'] as $k=> $v){
    ...
    elseif ($v['LineType']==8){ // 综合过关
        ...
        foreach ($Middle2 as $k2 => $v2){
            ...
            $mysql="select `Type`,MB_Inball,TG_Inball,MB_Inball_HR,TG_Inball_HR from `".DBPREFIX.SPORT_FLUSH_MATCH_TABLE."`  where MID=".$mid[$k2];
            ...
            $Middle2[$k2]['inball_mb_tg']='';
            // 投注哪一节就显示哪一节的比分
            switch ($row1['Type']){
                case 'BK':
                    $Middle2[$k2]['inball_mb_tg'] = '主'.$row1['MB_Inball'].' : 客'.$row1['TG_Inball'];
                    break;
                case 'FT':
                    if (strpos($bet_content, '[上半]')){
                        $Middle2[$k2]['inball_mb_tg'] = '主'.$row1['MB_Inball_HR'].' : 客'.$row1['TG_Inball_HR'];
                    }else{
                        $Middle2[$k2]['inball_mb_tg'] = '主'.$row1['MB_Inball'].' : 客'.$row1['TG_Inball'];
                    }
                    break;
...
//        $data['rows'][$k]['vs_team_id2'] = $vs_team_id2;
        ...
        switch ($gtype){
            case 'BK':
//                if(strpos($row1['MB_Team'],'上半')||strpos($row1['MB_Team'],'下半')||strpos($row1['MB_Team'],'第1节')||strpos($row1['MB_Team'],'第2节')||strpos($row1['MB_Team'],'第3节')||strpos($row1['MB_Team'],'第4节')){
                    $data['rows'][$k]['inball_mb_tg'] = '主'.$row1['MB_Inball'].' : 客'.$row1['TG_Inball'];
//                }
                break;
            case 'FT':
                if (strpos($v['Title'],'半')){
                    $data['rows'][$k]['inball_mb_tg'] = '主'.$row1['MB_Inball_HR'].' : 客'.$row1['TG_Inball_HR'];
                }else{
                    $data['rows'][$k]['inball_mb_tg'] = '主'.$row1['MB_Inball'].' : 客'.$row1['TG_Inball'];
                }
                break;
