...
// 第三方支付
$sSql = "SELECT id,thirdpay_code,url,minCurrency,maxCurrency,title FROM `".DBPREFIX."gxfcy_pay` WHERE `account_company` = 5 AND `depositNum` <= $depositTimes AND `status` = 1 AND FIND_IN_SET('{$_SESSION['pay_class']}',class)";
...
foreach ($aData as $k => $v){
    ...
    if ( $v['thirdpay_code'] == 'sf' ){ // 闪付支付宝
        $aUrl[$k] =  $v['url'].'/sfpay.php';
    }

    if ( $v['thirdpay_code'] == 'rx' ){  // 仁信支付宝
    ...
    if ( $v['thirdpay_code'] == 'jbf' ){  // 聚宝付支付宝
        $aUrl[$k] =  $v['url'].'/jbfpay.php';
    }

    if ( $v['thirdpay_code'] == 'xft' ){ // 信付通支付宝
        ...
        <div class="" data-area="third_ali_pay">
            <form method="post" name="third_ali_pay_form" id="third_ali_pay_form" action="" target="_self">
                ...
                <div class="form-item">
                    ...
                    <span class="textbox">
                        <input class="deposit-input money-textbox" name="order_amount"  id="third_ali_pay_amount" type="number"  placeholder="请输入汇款金额" />
...
<script type="text/javascript">
    ...
    setLoginHeaderAction('支付宝支付','','',usermon,uid) ;
