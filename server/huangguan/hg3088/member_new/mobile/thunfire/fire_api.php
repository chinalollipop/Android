...
switch ($action){
    case "cm":  // 会员 免费试玩或立即游戏
        if($testFlag !== 'test') { // 正式会员
            ...
            if($resUrl){ // 账号存在，Player登入成功 , 更新会员信息
                $token = $aResult['body']['token'];
                mysqli_query($dbMasterLink, "update " . DBPREFIX . "fire_member_data set launch_times = launch_times + 1, last_launch_time = '$now'  WHERE userid = '{$aUser['ID']}'");
            }
            ...
            if ($lockMoney){
                ...
                if($up){
                    ...
                    if($in){
                        ...
                        $moneyLogRes=addAccountRecords(array($userid,$aUser['UserName'],$_SESSION['test_flag'],$moneyf,$fShiftMoney*-1,$currency_after,59,$playSource,$insertId,"额度转换体育到雷火电竞"));
                        if($moneyLogRes){
                ...
                if($lockMoney){
                    ...
                    if($up){
                        ...
                        if($in){
                            ...
                            $moneyLogRes=addAccountRecords(array($userid,$aUser['UserName'],$_SESSION['test_flag'],$rowMoney['Money'],$fShiftMoney,$rowMoney['Money']+$fShiftMoney,60,$playSource,$insertId,"额度转换雷火电竞到体育"));
                            if($moneyLogRes){
                                mysqli_commit($dbMasterLink);
                                mysqli_close($dbMasterLink);
...
function doLoginAction(){
    global $aUser,$iframe_url,$public_token,$mem_token,$appRefer,$gametype;
    // 获取游戏地址,进入大厅
