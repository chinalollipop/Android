...
function countall($StartTime, $EndTime, $param){
    ...
    if ($cou>0){
        ...
        foreach ($jinji_data as $k => $v){
            ...
            @error_log('用户id'.$v['userid'].'-----jsdKey:'. $jsdKey.PHP_EOL, 3, '/tmp/group/api_zhenren_week_jinji.log');
            ...
            if ($jsdKey!==false){
                ...
                foreach ($numbers as $k3 => $v3){
                    ...
                    $start++;
                    @error_log('补齐之前的晋级记录一起k3:'.$k3.'--v3:'.$v3.'--入库审核sql:'. $sql.PHP_EOL, 3, '/tmp/group/api_zhenren_week_jinji.log');
                }
            }
            else{
                $numbers=range(1,$v['level']);
                foreach ($numbers as $k3 => $v3){
                    ...
                    $start++;
                    @error_log('第一次晋级k3:'.$k3.'--v3:'.$v3.'入库审核sql:'. $sql.PHP_EOL, 3, '/tmp/group/api_zhenren_week_jinji.log');
                }
            }
        }
