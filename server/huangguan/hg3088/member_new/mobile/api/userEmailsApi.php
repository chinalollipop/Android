...
if($action == 'index'){ // 首页获取滚动公告
    $data = [
        'notice' => getScrollMsg()
    ];
} else if($action == 'notice'){ // 会员公告
...
//    }
    ...
    if(!$readstatus || $readstatus=='readed'){
        $memsql = "SELECT message_status FROM `".DBPREFIX."web_member_data` WHERE ID='$userid' AND Oid='$uid' ";
    ...
    if($readstatus =='notread' || $readstatus==''){ // 更新已读 首次为空
        $memmysql="update ".DBPREFIX."web_member_data set message_status='readed' WHERE ID='$userid' AND Oid='$uid' "; // 更新所有会员信息读取状态
