...
include_once("../include/activityFlbf.class.php");

/**
 * 1. 会员请求
 *   每月8号    大于等于 188元   8元、
 *   每月18号   大于等于 888元   38元
 *   每月28号   大于等于 2888元  88元
 * 3. 数据表插入
 *      如果用户不存在 插入
...
if(date('d') == '8') {
    $sixnumBets = 188;
    $sixTime['start'] =  mktime(0,0,0,date('m'),8,date('Y')); //当月6号start
    $sixTime['end'] = mktime(23,59,59,date('m'),8,date('Y')); //当月6号end
}elseif(date('d') == '18') {
    $sixnumBets = 888;
    $sixTime['start'] =  mktime(0,0,0,date('m'),18,date('Y')); //当月16号start
    $sixTime['end'] = mktime(23,59,59,date('m'),18,date('Y')); //当月16号end
}elseif(date('d') == '28') {
    $sixnumBets = 2888;
    $sixTime['start'] =  mktime(0,0,0,date('m'),28,date('Y')); //当月26号start
    $sixTime['end'] = mktime(23,59,59,date('m'),28,date('Y')); //当月26号end
}

if( !in_array( date('d') , array('8' , '18' , '28')) ) {
    $status = array('status'=>'0', 'info'=>'请于美东时间每月8,18,28号申请逢八活动哦!');
    echo json_encode($status);exit;
}
...
if($numBets < $sixnumBets){
    $status = array('status'=>'0', 'info'=>'当月该逢八'. date('d').'号公司存款不符合要求，不允许申请哦!');
    echo json_encode($status);exit;
}
...
if(!$res){
    ...
    $status = array('status'=>'1', 'info'=>'已申请逢8彩金,请联系客服等待派发!');
    echo json_encode($status);
}
exit;
