...
switch ($_REQUEST['action']){
    case 'getZhenrenLevelSalaryInfo':
        ...
        $zhenrenLevelSalaryInfo = array_values($zhenrenLevelSalaryInfo);

        $status = 200;
        $describe = '';
        original_phone_request_response($status,$describe,$zhenrenLevelSalaryInfo);

        break;
    case 'getSalaryRecords':

        $data=[];
        $zhenren_level =$next_level_need_valid_money= $user_total_bet=$total_jinji_salary=$total_month_salary=0;

        // 累计有效投注
        $sql = "select userid, username, total, total_ag, total_og, total_bbin, total_manual, `count_date_start`,`count_date_end`, is_free from ".DBPREFIX."zhenren_week_report 
        ...
        // 上表格累计晋级礼金-累计月俸禄
        $sql = "select userid,EventName,`level`,`total`,`gift_gold`,`count_date_start`,`count_date_end`,`status` from ".DBPREFIX."zhenren_jinji_salary where userid=$user_id order by id desc";
        $result = mysqli_query($dbLink, $sql);
        $cou=mysqli_num_rows($result); // 总数
        $jinji_records=[];
        while ($row = mysqli_fetch_assoc($result)){
            if ($row['EventName']=='真人晋级礼金'){
...
//                }
        ...
        $zhenren_level = isset($jinji_data[$user_id]['level'])?$jinji_data[$user_id]['level']:0;
...
//            unset($jinji_data[$k]);
            ...
            if ($v['total']>=200000 and $v['total']<500000){
                ...
                $jinji_data[$k]['month_salary']= $zhenrenLevelSalaryInfo[1]['month_salary'];
            }
            if ($v['total']>=500000 and $v['total']<1000000){
                ...
                $jinji_data[$k]['month_salary']= $zhenrenLevelSalaryInfo[2]['month_salary'];
            }
            if ($v['total']>=1000000 and $v['total']<2000000){
                ...
                $jinji_data[$k]['month_salary']= $zhenrenLevelSalaryInfo[3]['month_salary'];
            }
            if ($v['total']>=2000000 and $v['total']<3000000){
                ...
                $jinji_data[$k]['month_salary']= $zhenrenLevelSalaryInfo[4]['month_salary'];
            }
            if ($v['total']>=3000000 and $v['total']<5000000){
                ...
                $jinji_data[$k]['month_salary']= $zhenrenLevelSalaryInfo[5]['month_salary'];
            }
            if ($v['total']>=5000000 and $v['total']<7000000){
                ...
                $jinji_data[$k]['month_salary']= $zhenrenLevelSalaryInfo[6]['month_salary'];
            }
            if ($v['total']>=7000000 and $v['total']<10000000){
                ...
                $jinji_data[$k]['month_salary']= $zhenrenLevelSalaryInfo[7]['month_salary'];
            }
            if ($v['total']>=10000000 and $v['total']<13000000){
                ...
                $jinji_data[$k]['month_salary']= $zhenrenLevelSalaryInfo[8]['month_salary'];
            }
            if ($v['total']>=13000000 and $v['total']<16000000){
                ...
                $jinji_data[$k]['month_salary']= $zhenrenLevelSalaryInfo[9]['month_salary'];
            }
            if ($v['total']>=16000000 and $v['total']<20000000){
                ...
                $jinji_data[$k]['month_salary']= $zhenrenLevelSalaryInfo[10]['month_salary'];
            }
            if ($v['total']>=20000000 and $v['total']<25000000){
                ...
                $jinji_data[$k]['month_salary']= $zhenrenLevelSalaryInfo[11]['month_salary'];
            }
            if ($v['total']>=25000000 and $v['total']<30000000){
                ...
                $jinji_data[$k]['month_salary']= $zhenrenLevelSalaryInfo[12]['month_salary'];
            }
            if ($v['total']>=30000000 and $v['total']<35000000){
                ...
                $jinji_data[$k]['month_salary']= $zhenrenLevelSalaryInfo[13]['month_salary'];
            }
            if ($v['total']>=35000000 and $v['total']<40000000){
                ...
                $jinji_data[$k]['month_salary']= $zhenrenLevelSalaryInfo[14]['month_salary'];
            }
            if ($v['total']>=40000000 and $v['total']<50000000){
                ...
                $jinji_data[$k]['month_salary']= $zhenrenLevelSalaryInfo[15]['month_salary'];
            }
            if ($v['total']>=50000000 and $v['total']<60000000){
                ...
                $jinji_data[$k]['month_salary']= $zhenrenLevelSalaryInfo[16]['month_salary'];
            }
            if ($v['total']>=60000000 and $v['total']<80000000){
                ...
                $jinji_data[$k]['month_salary']= $zhenrenLevelSalaryInfo[17]['month_salary'];
            }
            if ($v['total']>=80000000 and $v['total']<100000000){
                ...
                $jinji_data[$k]['month_salary']= $zhenrenLevelSalaryInfo[18]['month_salary'];
            }
            if ($v['total']>=100000000 and $v['total']<120000000){
                ...
                $jinji_data[$k]['month_salary']= $zhenrenLevelSalaryInfo[19]['month_salary'];
            }
            if ($v['total']>=120000000 and $v['total']<150000000){
                ...
                $jinji_data[$k]['month_salary']= $zhenrenLevelSalaryInfo[20]['month_salary'];
            }
            if ($v['total']>=150000000 and $v['total']<200000000){
                ...
                $jinji_data[$k]['month_salary']= $zhenrenLevelSalaryInfo[21]['month_salary'];
            }
            if ($v['total']>=200000000 and $v['total']<250000000){
                ...
                $jinji_data[$k]['month_salary']= $zhenrenLevelSalaryInfo[22]['month_salary'];
            }
            if ($v['total']>=250000000 and $v['total']<300000000){
                ...
                $jinji_data[$k]['month_salary']= $zhenrenLevelSalaryInfo[23]['month_salary'];
            }
            if ($v['total']>=300000000 and $v['total']<400000000){
                ...
                $jinji_data[$k]['month_salary']= $zhenrenLevelSalaryInfo[24]['month_salary'];
            }
            if ($v['total']>=400000000 and $v['total']<500000000){
                ...
                $jinji_data[$k]['month_salary']= $zhenrenLevelSalaryInfo[25]['month_salary'];
            }
            if ($v['total']>=500000000 and $v['total']<700000000){
                ...
                $jinji_data[$k]['month_salary']= $zhenrenLevelSalaryInfo[26]['month_salary'];
            }
            if ($v['total']>=700000000 and $v['total']<950000000){
                ...
                $jinji_data[$k]['month_salary']= $zhenrenLevelSalaryInfo[27]['month_salary'];
            }
            if ($v['total']>=950000000 and $v['total']<1200000000){
                ...
                $jinji_data[$k]['month_salary']= $zhenrenLevelSalaryInfo[28]['month_salary'];
            }
            if ($v['total']>=1200000000 and $v['total']<1500000000){
                ...
                $jinji_data[$k]['month_salary']= $zhenrenLevelSalaryInfo[29]['month_salary'];
            }
            if ($v['total']>=1500000000){
                ...
                $jinji_data[$k]['month_salary']= $zhenrenLevelSalaryInfo[30]['month_salary'];
            }
...
//            unset($jinji_data[$k]);
            ...
            if ($v['until_current_total']>=30000000 and $v['until_current_total']<35000000){
                $jinji_data[$k]['level'] = 13; // 当前等级
            }
            if ($v['until_current_total']>=35000000 and $v['until_current_total']<40000000){
                $jinji_data[$k]['level'] = 14; // 当前等级
            }
            if ($v['until_current_total']>=40000000 and $v['until_current_total']<50000000){
                $jinji_data[$k]['level'] = 15; // 当前等级
            }
            if ($v['until_current_total']>=50000000 and $v['until_current_total']<60000000){
                $jinji_data[$k]['level'] = 16; // 当前等级
            }
            if ($v['until_current_total']>=60000000 and $v['until_current_total']<80000000){
                $jinji_data[$k]['level'] = 17; // 当前等级
            }
            if ($v['until_current_total']>=80000000 and $v['until_current_total']<100000000){
                $jinji_data[$k]['level'] = 18; // 当前等级
            }
            if ($v['until_current_total']>=100000000 and $v['until_current_total']<120000000){
                $jinji_data[$k]['level'] = 19; // 当前等级
            }
            if ($v['until_current_total']>=120000000 and $v['until_current_total']<150000000){
                $jinji_data[$k]['level'] = 20; // 当前等级
            }
            ...
            if ($v['until_current_total']>=300000000 and $v['until_current_total']<400000000){
                $jinji_data[$k]['level'] = 24; // 当前等级
            }
            if ($v['until_current_total']>=400000000 and $v['until_current_total']<500000000){
                $jinji_data[$k]['level'] = 25; // 当前等级
            }
            if ($v['until_current_total']>=500000000 and $v['until_current_total']<700000000){
                $jinji_data[$k]['level'] = 26; // 当前等级
            }
            if ($v['until_current_total']>=700000000 and $v['until_current_total']<950000000){
                $jinji_data[$k]['level'] = 27; // 当前等级
            }
            if ($v['until_current_total']>=950000000 and $v['until_current_total']<1200000000){
                $jinji_data[$k]['level'] = 28; // 当前等级
            }
            if ($v['until_current_total']>=1200000000 and $v['until_current_total']<1500000000){
                $jinji_data[$k]['level'] = 29; // 当前等级
            }
            if ($v['until_current_total']>=1500000000){
                $jinji_data[$k]['level'] = 30; // 当前等级
            }
