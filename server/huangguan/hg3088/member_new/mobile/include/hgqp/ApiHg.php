...
class ApiHg
{
    ...
    protected $logPath = '/tmp/ff_log_mobile';
    protected $logFile;
    ...
    {
        if ($this->debug)
            @file_put_contents($this->logFile, date('Y-m-d H:i:s') . '-mobile-' . $log . "\n", FILE_APPEND);
    }
    ...
    protected function curl_get_content($url, $conn_timeout=10, $timeout=60, $user_agent=null)
    {
        ...
        if (($err) || ($httpcode !== 200)) {
            @error_log('【' . date('Y-m-d H:i:s') . '】mobile-ERRNO: ' . $err . ' ERROR:' . $error . 'HTTP_CODE:' . $httpcode . ' URL:' . $url. "\n", 3, '/tmp/ff_log/ff_api_error.log');
            return null;
        }
        return $res;
    }
