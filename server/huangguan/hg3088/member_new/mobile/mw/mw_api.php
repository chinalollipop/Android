...
$gameId = $gameId ?$gameId:$_REQUEST['game_id']; // 为了兼容,跟其他统一
...
switch ($action){
    ...
    case 'getLaunchGameUrl':// 兼容，跟其他电子参数一样

        // 授权
        $toURL = $domainUrl.'api/oauth?';
//        $res = oauth($toURL, 'oauth', $mw_uid, $utoken, '2'); // APP
        ...
        if($lock){
            ...
            if ($lockMoney){
                ...
                if($up){
                    ...
                    if($in){
                        $moneyLogRes=addAccountRecords(array($userid,$_SESSION['UserName'],$_SESSION['test_flag'],$moneyf,$fShiftMoney*-1,$currency_after,44,$playSource,$insertId,"MW电子游艺额度转换"));
                        if($moneyLogRes){
...
//                                exit(json_encode( [ 'err'=>'-12','msg'=>'mw上分货币转入准备失败-'.$res['msg'].'-'.$transferOrderTime ] ));
                    ...
                    if($lockMoney){
                        ...
                        if($up){
                            ...
                            if($in){
                                ...
                                $moneyLogRes=addAccountRecords(array($userid,$_SESSION['UserName'],$_SESSION['test_flag'],$rowMoney['Money'],$fShiftMoney,$rowMoney['Money']+$fShiftMoney,45,$playSource,$insertId,"MW电子游艺额度转换"));
                                if($moneyLogRes){
                                    mysqli_commit($dbMasterLink);
                                    mysqli_close($dbMasterLink);
                                    $balance[0] = [
    ...
    case 'gamelist_dianzi': // 兼容，跟AG一样统一
        if ($platform==14 || !$platform){
            foreach ($aWmGames as $k => $v){
                $data[$k]['gameid'] = $v['gameId'];
                $data[$k]['name'] = $v['gameName'];
                $data[$k]['gameurl'] = '/images/mwgame/'.$v['gameIcon'];
            }
            $mwGames = $data;
        }
        else{
            foreach ($aWmGames as $k => $v){
                $data[$k]['gameId'] = $v['gameId'];
                $data[$k]['gameName'] = $v['gameName'];
                $data[$k]['gameIcon'] = '/images/mwgame/'.$v['gameIcon'];
                $data[$k]['gameRuleUrl'] = $v['gameRuleUrl'];
            }
            $mwGames['mwGames'] = $data;
        }

        $status = '200';
        $describe = 'MW游戏列表获取成功';
        original_phone_request_response($status,$describe,$mwGames);
