...
$pid=$_REQUEST['bankid'];
// 第三方银行支付
$sSql = "SELECT id,thirdpay_code,url,minCurrency,maxCurrency,title FROM `".DBPREFIX."gxfcy_pay` WHERE `account_company` = 2 AND `depositNum` <= $depositTimes AND `status` = 1 AND FIND_IN_SET('{$_SESSION['pay_class']}',class) and has_company_youhui=1 and id =$pid";
...
foreach ($aData as $k => $v){
    ...
    switch ( $v['thirdpay_code']){
        ...
        case 'autopay':  // autopay
            ...
            break;
        case 'xingchen':  // 星辰
            $aUrl[$k] = $v['url'] . '/'.$v['thirdpay_code'] .'/xingchenpay.php';
            $aBanklist[$k] = array(
                'bank_transfer'=>'银行卡转账',
            );
            break;
