...
$main_host=$_REQUEST['loginwebsite']; // 注册成功后要登录的网址
if ($intr==''){
...
//    $ratio=$_REQUEST['radio'];// 性别
    ...
    if(!$_REQUEST['verifycode'])
        exit("<script>alert('请输入验证码!');window.close();</script>");
    if(strtolower($_REQUEST['verifycode']) != $_SESSION['authcode'])
        exit("<script>alert('验证码输入错误!');window.close();</script>");


    publicRegValidate($username,$intr,$password,$password2,$alias,$paypassword,$phone,$wechat,'','ad');
...
if ($keys=='add'){
	...
	//新注册用户呢登录=创建登录旧版连接地址 redis串相同

	 $oldUnionCode = getUnionCode();
	 $redisRes = $redisObj->setOne($row[ID].'_NEW_CHANGE_OLD',serialize($oldUnionCode));
	 $keyOld = md5(passwordEncryption($password,$username).$oldUnionCode.md5($username));		
	 $idOld=$row[ID]+9876889;
	 //$LoginUrl=$main_host.'/login.php?id='.$idOld.'&sign=newchangeold&username='.$username.'&password='.passwordEncryption($password,$username).'&key='.$keyOld.'&langx='.$langx;
	 $LoginUrl=$main_host.'/login.php?username='.$username.'&password='.$password.'&langx='.$langx.'&code=first&yzm_input='.rand();

?>
    <script language='JavaScript'>alert('恭喜注册已成功！\n帐号：<?php echo  $username?>\n密码：<?php echo  $password?>\n手机号码：<?php echo $phone?>');parent.location='<?php echo $LoginUrl; ?>';</script>
<?php
}
?>