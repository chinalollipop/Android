...
if (TPL_FILE_NAME=='0086'){
    require_once '../class/wanyin0086/payment_common.php';
}elseif(TPL_FILE_NAME=='6668'){
    require_once '../class/wanyin/payment_common.php';
}

@file_put_contents('/tmp/wanyinautopayback.php.log', date('Y-m-d H:i:s') . "\n", FILE_APPEND);
@file_put_contents('/tmp/wanyinautopayback.php.log', $_REQUEST['merchant_code'] . "\n", FILE_APPEND);
@file_put_contents('/tmp/wanyinautopayback.php.log', $_REQUEST['data'] . "\n", FILE_APPEND);
@file_put_contents('/tmp/wanyinautopayback.php.log', $_REQUEST['sign'] . "\n", FILE_APPEND);
...
if($count == 0) {
    echo "<script type='text/javascript'>alert('渠道信息错误,未找到wanyin出款支付渠道！');window.opener=null;window.open('', '_self');window.close();</script>";
    exit();
}
