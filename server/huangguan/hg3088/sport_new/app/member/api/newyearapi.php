...
if( !isset($_SESSION['Oid']) || $_SESSION['Oid'] == "" ) {
    $status = array('status'=>'502', 'info'=>'请重新登录!');
...
if(time() < $newYearBeginTime || time() > $newYearEndTime){
    $status = array('status'=>'401.2', 'info'=>'请于美东时间2月4号-10号期间签到领取红包哦!');
    echo json_encode($status);exit;
}
...
switch ($action){
    case 'mobilesign':// 签到
        if(!isPhone($mobile)) {  // 验证手机
            $status = array('status'=>'402.1', 'info'=>'手机号码不符合规范!');
            echo json_encode($status,JSON_UNESCAPED_UNICODE);exit;
        }

        $signInStatus = isSignIn($userid);  // 是否签到
        if($signInStatus){
            $status = array('status'=>'402.2', 'info'=>'已经签到，请点击领取红包!');
            echo json_encode($status,JSON_UNESCAPED_UNICODE);exit;
        }
        ...
        if(!$result){
            $status = array('status'=>'402.3', 'info'=>'异常情况，请重新签到!');
            echo json_encode($status,JSON_UNESCAPED_UNICODE);exit;
        }
        $status = array('status'=>'1', 'info'=>'签到成功，请按次数领取红包!');
        echo json_encode($status);exit;
        break;
    case 'get_remain_num':
        /*
    ...
    case 'receive_red_envelope': // 领取红包
        ...
        if(!$signInStatus){
            $status = array('status'=>'405.00', 'info'=>'请先输入有效手机号进行签到!');
            echo json_encode($status,JSON_UNESCAPED_UNICODE);exit;
        }
        ...
        if($last_date_times <=0 ){
            $status = array('status'=>'405.01', 'info'=>'不满足领取次数!');
            echo json_encode($status,JSON_UNESCAPED_UNICODE);exit;
        }
        ...
        if($beginFrom){
            ...
            if(mysqli_query($dbMasterLink,$sql)){
                ...
                if(mysqli_query($dbMasterLink,$sql)){
                            ...
                            if ($res) {
                                ...
                                if ($resBill){
                                    ...
                                    if ($moneyLogRes) {
                                        ...
                                        $last_times = get_remain_num();    // 获取剩余总次数
                                        $status = array('status'=>'1', 'info'=>'恭喜，抽取新春红包'.$gold.'元', 'last_times'=> $last_times);
                                        echo json_encode($status,JSON_UNESCAPED_UNICODE);exit;
                    ...
                    mysqli_query($dbMasterLink,"ROLLBACK");
                    $status = array('status'=>'401.10', 'info'=>'更新会员次数失败');
                    echo json_encode($status,JSON_UNESCAPED_UNICODE);exit;
                }
            }else{
                mysqli_query($dbMasterLink,"ROLLBACK");
                $status = array('status'=>'401.11', 'info'=>'锁定存留日期、次数、状态失败');
                echo json_encode($status,JSON_UNESCAPED_UNICODE);exit;
            }
        }else{
            mysqli_query($dbMasterLink,"ROLLBACK");
...
function get_remain_num() {
    ...
    //@error_log('CountNums:'.$sql_num_assoc['CountNums'].'--已领取:'.$row['cou'].PHP_EOL,  3,  '/tmp/aaa.log');
    return $last_times;
}
?>