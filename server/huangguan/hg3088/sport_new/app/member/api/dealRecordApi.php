...
//$date_start ? $sWhere .= " and addDate > '{$date_start}'": '';
//$date_end ? $sWhere .=" and addDate < '{$date_end}'" : '';
$date_start ? $sWhere .= " and addDate between '{$date_start}' and '{$date_end}'": '';
$mysql="select Checked,Gold,`Type`,UserName,AddDate,notes,reason,Order_code,`Date`,`Name`,Phone,Contact,Bank,Bank_Account,Bank_Address,`From`,`To` from ".DBPREFIX."web_sys800_data where $sWhere order by id desc";
//echo $mysql;
...
while ($myrow=mysqli_fetch_assoc($result)){
    ...
    if($myrow['Checked']=='-1'){ // Checked 0 未审核，1 成功，-1 失败，只有拒绝才需要展示
        $myrow['notes'] = $myrow['reason'] ;
    }else{
        if ($thistype=='Q'||$thistype=='R'){
            $myrow['notes'] = '' ;
        }else if($thistype=='S'){
            $myrow['notes'] = $myrow['notes'];
        }else{
            $myrow['notes'] = returnBankAccount($myrow['Bank_Account']) ;
        }
    }
    $data['rows'][] = $myrow ;
}
