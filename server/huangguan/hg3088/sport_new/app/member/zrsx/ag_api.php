...
//            }
            ...
            if ($action == 'b'){
                $aData = array(
                    'ag_balance'=>number_format('0.00',2),
                    'hg_balance'=>formatMoney($row['Money'])
                );
                $status = '200';
                $describe = "创建账号创建AG账号成功!";
                original_phone_request_response($status,$describe,$aData);
            }
        }else{
...
//        break;
    case 'b': // 获取余额
        ...
        $aData = array(
            'ag_balance'=>number_format($agGetbalance["balance"],2),
            'hg_balance'=>formatMoney($row['Money'])
        );
        $status = '200';
        $describe = 'AG余额获取成功!' ;
	            ...
	            if ($lockMoney){
	                ...
	                if($up){
	                    ...
	                    if($in){
                            ...
                            if ($aResult['code']==1){
                                //添加会员账变日志
                                $moneyLogRes=addAccountRecords(array($userid,$_SESSION['UserName'],$_SESSION['test_flag'],$rowMoney['Money'],$fShiftMoney*-1,$rowMoney['Money']-$fShiftMoney,23,22,$insertId,"真人娱乐/电子游艺额度转换"));
                                if($moneyLogRes){
        ...
        if($lock){
	                ...
	                if($up){
	                    ...
	                    if($in){
	                        ...
	                        $insertId=mysqli_insert_id($dbMasterLink);
	                        $rowMoney=mysqli_fetch_assoc($lockMoney);
	                    	$moneyLogRes=addAccountRecords(array($userid,$_SESSION['UserName'],$_SESSION['test_flag'],$rowMoney['Money'],$fShiftMoney,$rowMoney['Money']+$fShiftMoney,24,22,$insertId,"真人娱乐/电子游艺额度转换"));
	                    	if($moneyLogRes){
	                            mysqli_commit($dbMasterLink);
	                            mysqli_close($dbMasterLink);
	                        }else{
