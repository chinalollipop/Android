...
$mysql = "select M_Date,M_Time,$mb_team as MB_Team,$tg_team as TG_Team,$m_league as M_League,ShowTypeRB,M_LetB_RB,MB_LetB_Rate_RB,TG_LetB_Rate_RB from `".DATAHGPREFIX.SPORT_FLUSH_MATCH_TABLE."` where Type='BK' and `MID`='$gid' and Open=1 and $mb_team!=''";
...
if($cou==0){
	...
	$havesql="select sum(BetScore) as BetScore from ".DBPREFIX."web_report_data where M_Name='$memname' and MID='$gid' and LineType=9 and Mtype='$mtype' and Active=2";
	$result = mysqli_query($dbLink,$havesql);
	$haverow = mysqli_fetch_assoc($result);  
	$have_bet=$haverow['BetScore']+0;  
	
    //$sql = "select MB_Team,CS from ".DBPREFIX."match_league where  $m_league='$M_League' and Type='BK'";
    $sql = "select M_League,CS from ".DBPREFIX."match_league where  $m_league='$M_League' and Type='BK'";
...
<form name="LAYOUTFORM" action="/app/member/BK_order/BK_order_re_finish.php" method="post" onSubmit="return false">
  <div class="ord">
   	<div class="title"><h1><span class="info">i</span>篮球：<?php echo $U_44 ?></h1><div class="tiTimer" onClick="orderReload();"><span id="ODtimer">10</span><input type="hidden" id="checkOrder" onClick="onclickReloadTime()" checked value="10"></div></div>
