				...
				for(var j=0; j<div_model.children.length; j++){
						...
						if(tab_model.nodeName =="TABLE"&&tab_model.id.indexOf("model")!=-1){
								...
								for(var ms_num=0;ms_num<=6;ms_num++){
									...
									tmpScreen = tmpScreen.replace(/\*MS_STR\*/g, (ms_num!=0)?" - "+str_BK_MS[ms_num]:"");
...
function closeClickEvent(){
		...
		parent.body_browse.document.getElementById('MBU').className="bodyset BUR";
...
function show_gameInfo(gid,ObjData){
	...
	//tmp_repl = tmp_repl.replace('*MIDFIELD*',(midfield == 'Y')?str_midfield:"");
...
function betEvent(gid,rtype,ratio,wtype){
	...
	if(wtype!='NFS'){
		...
		var param = 'gametype=FT&gid='+gid+'&uid='+_REQUEST['uid']+'&rtype='+rtype+'&wtype=FS'+'&langx='+_REQUEST['langx'];
...
function fixMoreWindow(){
	...
	var MBU = parent.document.getElementById('MBU');
	parent.document.getElementById('showdata').width = MBU.offsetWidth;
	parent.document.getElementById('showdata').height = get_max(tab_show.offsetHeight,MBU.offsetHeight);
...
function alayer(tpl,adata,wtype,ior_arr,tr_color,ms_num){
	...
	for(var t=0; t<rtypeMap[wtype].length; t++){
		...
		//背景黃色
		if(more_bgYalloW == rtype+"_"+gid ) tpl.replace(new RegExp('\\*'+td_class+'\\*'), "bg_yellow");
