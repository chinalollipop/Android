...
switch ($action){
    case 'b': // 获取体育余额-AG余额-彩票余额
        ...
        $aData = array(
            'ag_balance'=> $agGetbalance["balance"],
        ...
        // exit( json_encode( ['err'=>0, 'ag_balance'=>$agGetbalance["balance"], 'hg_balance'=>number_format($aUser['Money'],2), 'balance_cp'=>number_format($cpFund,2) ] ) );
        break;
    case 'ag2hg': // 从AG真人转到体育。首先调用接口，然后成功后再更新本地资金
        ...
        $agGetbalance["balance"] = $agGetbalance["balance"];
...
//        }
        ...
        $oRes = mysqli_query($dbLink, "SELECT * FROM ".DBPREFIX."gxfcy_userlock WHERE userid = {$aUser['ID']}");
            ...
            if($lockMoney){
                ...
                if($up){
                    ...
                    if($in){
                        ...
                        $moneyLogRes=addAccountRecords(array($aUser['ID'],$aUser['UserName'],$aUser['test_flag'],$rowMoney['Money'],$fShiftMoney,$rowMoney['Money']+$fShiftMoney,24,1,mysqli_insert_id($dbMasterLink),"提款额度转换:真人电子到体育"));
                        if($moneyLogRes){
    ...
    case 'cp2hg': // 从彩票转账到体育，摘自彩票游戏中的转账
        ...
        if ($row['lcurrency'] < 0 || $row['lcurrency'] == 0){
            // exit(json_encode( [ 'err'=>'-7','msg'=>'彩票账号余额不足~~' ] ));
            $status = '400.14';
            $describe = "彩票账号余额不足!";
            original_phone_request_response($status,$describe,$aData);
        }
            ...
            if($lockUser && $lockMoney){
                ...
                if($addFundRecordFrom){
                    ...
                    if($updateUserFundFrom){
                        $moneyLogRes=addAccountRecords(array($id,$userName,$rowMoney['test_flag'],$rowMoney['Money'],$fund,$rowMoney['Money']+$fund,22,1,$inserId,"提款额度转换:彩票到体育"));
                        if($moneyLogRes){
                            ...
                            if($addFlagRecordFrom){
                                ...
                                if($beginTo){
                                    ...
                                    if($resultMcp){
                                        ...
                                        if($from=="cp" && $to=="hg"){
                                            if ($lcurrencyTO < $fund){
                                                ...
                                                mysqli_query($dbMasterLink,"ROLLBACK");
                                                // exit( json_encode(array( 'err'=>'-19','status'=>-2,'message'=>'额度有误，请确认金额！')));
                                                $status = '400.17';
                                                $describe = "额度有误，请确认金额!";
                                                original_phone_request_response($status,$describe,$aData);
                                            }
                                    ...
                                    mysqli_query($cpMasterDbLink,"ROLLBACK");
                                    //exit(json_encode( [ 'err'=>'-12','msg'=>'CP事务开启失败！' ] ));
                                    $status = '400.23';
                                    $describe = "CP事务开启失败!";
                                    original_phone_request_response($status,$describe,$aData);
                                }
...
//        echo json_encode(array( 'err'=>'-20','status'=>-100,'msg'=>'操作失败！'));
//        exit;
        $status = '400.29';
        $describe = "操作失败!";
        original_phone_request_response($status,$describe,$aData);
        break;
    default:
        // exit(json_encode(array( 'err'=>'-21','status'=>-999,'msg'=>'非法操作！')));
        $status = '400.30';
        $describe = "非法操作!";
        original_phone_request_response($status,$describe,$aData);
        break;
