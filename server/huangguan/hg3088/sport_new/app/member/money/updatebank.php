...
if($bank_account && !isBankNumber($bank_account)){
    exit(json_encode(['code' => -11, 'msg' => '抱歉，您输入的银行卡号不符合规范！']));
}
...
if( !isset($_SESSION['Oid']) || $_SESSION['Oid'] == "" ) {
    echo json_encode(array(['code'=>3, 'msg' => '抱歉，您输入的银行卡号不符合规范！'])); // 登录信息过期
    exit;
} else{ // 更新数据
...
$aData = array('code'=>1, 'msg' => '更换成功！','resdata'=>$aData) ;
if($aData){
    echo json_encode($aData);
}else{ // 更换失败
    echo json_encode(array(['code'=>2, 'msg' => '抱歉，更换失败！']));
}
