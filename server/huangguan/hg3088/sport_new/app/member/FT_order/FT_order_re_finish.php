...
$gnum=$_REQUEST['gnum'];
$strong=$_REQUEST['strong'];$abcdType = substr($rtype,3,1);
...
//	$msg_c=explode("@",$html_data);
	...
	//echo $aData['ioratio'].'--'.$allcount.'=='.$accoutArrNum;
//	if(sizeof($msg_c)>1){
	if($aData['ioratio']>0){
		break;
...
//        Array ( [code] => error [msg] => doubleLogin )
//        print_r($aData);
//        print_r($postdata);
        if ($aData['code']=='error'){
            @error_log(date("Y-m-d H:i:s").'--正网地址被踢出:'. $site .'--被踢账号:'. $value['Name'].'--ErrorMsg:'. $aData['msg'] .PHP_EOL, 3, '/tmp/group/logout_warn.log');
        }
...
$mysqlL = "select `MID` from `".DBPREFIX.SPORT_FLUSH_MATCH_TABLE."` where `MID`='$gid' and Open=1 and MB_Team!=''";
                            ...
                            if ($updateSt) {
                                ...
                                if ($setGames) {
                                    ...
                                    if ($comStatus) {
                                        ...
                                        if ($isCloseDJFT==1){
                                            echo attention("更多玩法数据为空dj1", $uid, $langx);
                                            exit();
                                        }
                                    }
                                    else{
                        ...
                        if ($rtStatus) {
                            ...
                            if ($setGames) {
                                ...
                                if ($comStatus) {
                                    ...
                                    $isCloseDJFT = closeDJFT($tmp_Obj[$gid]);
                                    if ($isCloseDJFT==1){
                                        echo attention("更多玩法数据为空dj2", $uid, $langx);
                                        exit();
                                    }
                                }
                                else{
...
// -------------------------------------------------------更多玩法刷水 End
	...
	$scoreMbHRAlready=$row['MB_Inball_HR'];
	$scoreTgHRAlready=$row['TG_Inball_HR'];
			...
			switch ($rtype){
				case "RTSY":
					...
					if($scoreMbTimely>0 && $scoreTgTimely>0){
                        echo attention("此时已经符合条件不能下注,非常抱歉!",$uid,$langx);
                        exit();
                    }
                    break;
				case "RTSN":
		...
		case 122:
			...
			switch ($rtype){
				...
				case "RMUAHU":	
                    ...
                    /*if($scoreTotalTimely>0 && $scoreTotalTimely <= 1){
                        echo attention("大小球数参数异常,请刷新赛事!",$uid,$langx);
                        exit();
                    }*/
				...
				case "RMUBHU":	
                    ...
                    /*if($scoreTotalTimely>0 && $scoreTotalTimely <= 2){
                        echo attention("大小球数参数异常,请刷新赛事!",$uid,$langx);
                        exit();
                    }*/
				...
				case "RMUCHU":	
                    ...
                    /*if($scoreTotalTimely>0 && $scoreTotalTimely <= 3){
                        echo attention("大小球数参数异常,请刷新赛事!",$uid,$langx);
                        exit();
                    }*/
				...
				case "RMUDHU":	
                    ...
                    /*if($scoreTotalTimely>0 && $scoreTotalTimely <= 4){
                        echo attention("大小球数参数异常,请刷新赛事!",$uid,$langx);
                        exit();
                    }*/
...
//                    }
	...
	case 244:
		...
		switch ($rtype){
			...
			case "HRUHO":
				$btype="-&nbsp;<font color=red><b>[$Order_1st_Half]</b></font>";
				$w_m_place="大&nbsp;".$detailsData['ratio_'.strtolower($rtype)]."&nbsp;-&nbsp;<font color=#666666>[上半]</font>";
				$w_m_place_tw="大&nbsp;".$detailsData['ratio_'.strtolower($rtype)]."&nbsp;-&nbsp;<font color=#666666>[上半]</font>";
				$w_m_place_en="OVER&nbsp;".$detailsData['ratio_'.strtolower($rtype)]."&nbsp;-&nbsp;</font><font color=#666666>[1st Half]</font>";
				$s_m_place="大&nbsp;".$detailsData['ratio_'.strtolower($rtype)];
                $bet_type='半场球队进球数'.' '.$w_mb_team.' -大/小';
                $bet_type_tw="半场球队进球数".' '.$w_mb_team_tw.' -大/小';
                $bet_type_en="Half_Order_Team_Ball_In".' '.$w_mb_team_en.' -大/小';
			...
			case "HRUHU":
				$btype="-&nbsp;<font color=red><b>[$Order_1st_Half]</b></font>";
				$w_m_place="小&nbsp;".$detailsData['ratio_'.strtolower($rtype)]."&nbsp;-&nbsp;<font color=#666666>[上半]</font>";
				$w_m_place_tw="小&nbsp;".$detailsData['ratio_'.strtolower($rtype)]."&nbsp;-&nbsp;<font color=#666666>[上半]</font>";
				$w_m_place_en="UNDER&nbsp;".$detailsData['ratio_'.strtolower($rtype)]."&nbsp;-&nbsp;</font><font color=#666666>[1st Half]</font>";
				$s_m_place="小&nbsp;".$detailsData['ratio_'.strtolower($rtype)];
                $bet_type='半场球队进球数'.' '.$w_mb_team.' -大/小';
                $bet_type_tw="半场球队进球数".' '.$w_mb_team_tw.' -大/小';
                $bet_type_en="Half_Order_Team_Ball_In".' '.$w_mb_team_en.' -大/小';
		...
		//球队进球数大小限制 此处只限制有一个大小球的数字类型，不限制类似1/1.5数据类型
        $Sign="VS.";
		$gwin=($w_m_rate)*$gold;
		$grape = $detailsData['ratio_'.strtolower($rtype)];
		$w_gtype=$rtype;
	...
	if(!isset($_REQUEST['autoOdd']) || $_REQUEST['autoOdd']!="Y"){
		if($w_m_rate!=$_REQUEST['ioradio_r_h']){
			$turn_url=$turn_url.'&error_flag=1';
			$tmpradio = $_REQUEST['ioradio_r_h'];
			print_r("m_rate=$w_m_rate, ioradio=$tmpradio");
			echo "<script language='javascript'>self.location='$turn_url';</script>";
			exit;
		}	
	}
	...
	if($begin && $lockResult){
		...
		$sql = "INSERT INTO ".DBPREFIX."web_report_data	(userid,Glost,playSource,QQ83068506,danger,MID,testflag,Active,orderNo,LineType,Mtype,M_Date,BetTime,BetScore,Middle,Middle_tw,Middle_en,BetType,BetType_tw,BetType_en,M_Place,M_Rate,M_Name,Gwin,OpenType,OddsType,ShowType,Agents,World,Corprator,Super,Admin,A_Point,B_Point,C_Point,D_Point,BetIP,Ptype,Gtype,CurType,Ratio,MB_MID,TG_MID,Pay_Type,Orderby,MB_Ball,TG_Ball) values ($memid,$Money,$play_Source,'$inball1','1','$gid',$test_flag,'$active','$showVoucher','$line','$w_gtype','$m_date','$bettime','$gold','$lines','$lines_tw','$lines_en','$bet_type','$bet_type_tw','$bet_type_en','$grape','$w_m_rate','$memname','$gwin','$open','$oddstype','$showtype','$agents','$world','$corprator','$super','$admin','$a_point','$b_point','$c_point','$d_point','$ip_addr','$ptype','FT','$w_current','$w_ratio','$w_mb_mid','$w_tg_mid','$pay_type','$order','$mb_ball','$tg_ball')";
		$insertBet=mysqli_query($dbMasterLink,$sql);
        if($insertBet){
            $lastId=mysqli_insert_id($dbMasterLink);
			$moneyLogRes=addAccountRecords(array($memid,$memname,$test_flag,$Money,$gold*-1,$havemoney,1,1,$lastId,"FT投注$w_gtype"));
			if($moneyLogRes){
  ...
  <div class="ord">
    <span><h1><?php echo $caption?></h1></span>
      <div id="info">
       ...
       <p><center><strong><font class="danger_not_confirm" >&nbsp;<?php echo $Order_Pending?>&nbsp;</font></strong></center></p>
...
<script type="text/javascript">
    // 获取注单状态
    $(function () {
        var orderNum = '<?php echo $showVoucher?>' ;

        var betSportInt = setInterval(function () {
            getBetSportStatus();
        },6000);
        function getBetSportStatus() {
            var url = '/app/member/api/getSportRbStatus.php' ;
            $.ajax({
                type : 'POST',
                url : url ,
                ...
                success:function(res) {
                    if(res){
                        if(res.data.cancel ==0 && res.data.danger == 0){ // Cancel -1 取消 ，0 已确认
                            $('.danger_not_confirm').addClass('danger_confirm').text('已确认') ;
                            clearInterval(betSportInt);
                        }else if(res.data.cancel == -1){ // 取消
                            $('.danger_not_confirm').text('已取消') ;
                            clearInterval(betSportInt);
                        }
