...
if($gid%2==0){
	...
	//$mysql = "select M_Date,M_Time,$mb_team as MB_Team,$tg_team as TG_Team,$m_league as M_League from `".DATAHGPREFIX.SPORT_FLUSH_MATCH_TABLE."` where `m_start`>now() and `MID`=$gid-1 and Open=1 and $mb_team!=''";
    $mysql = "select M_Date,M_Time,$mb_team as MB_Team,$tg_team as TG_Team,$m_league as M_League from `".DATAHGPREFIX.SPORT_FLUSH_MATCH_TABLE."` where `m_start`>now() and `MID`=$gid and Open=1 and $mb_team!=''";
}
      ...
      <div class="betdata">
          <p class="amount">
              ...
              if($wtype=='OUH' || $wtype=='OUC' || $wtype=='HOUH' || $wtype=='HOUC'){  // 球队进球数 大小不需要减去本金
                  echo '<input name="gold" type="text" class="txt" id="gold" onKeyPress="return CheckKey(event)" onKeyUp="return CountWinGold()" size="8" maxlength="10" placeholder="投注额"> <span class="clean_bet_money" id="betClear"></span>' ;
              }else{
                  echo '<input name="gold" type="text" class="txt" id="gold" onKeyPress="return CheckKey(event)" onKeyUp="return CountWinGold_dy_ds_dyh()" size="8" maxlength="10" placeholder="投注额"> <span class="clean_bet_money" id="betClear"></span>' ;
              }
              ?>
          </p>
