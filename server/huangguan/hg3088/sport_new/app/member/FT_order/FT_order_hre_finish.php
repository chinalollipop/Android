...
//			$html_data=$curl->fetch_url("".$site."/app/member/FT_order/FT_order_all.php?gid=$sgid&uid=$suid&odd_f_type=$odd_f_type&type=$type&gnum=$gnum&langx=zh-cn&ptype=&imp=N&rtype=HRM{$type}&wtype=HRM");
		...
		case '204':
			$sgid=$gid+1;
            $postdata['chose_team']=$wtype;// 波胆/上半场
            $postdata['wtype']='HRPD';
...
//------------------------------------------------------------
$mysql = "select `ECID`,`LID`,`ISRB` from `".DBPREFIX.SPORT_FLUSH_MATCH_TABLE."` where `MID`='$gid' and Open=1 and MB_Team!=''";
...
if($gid%2==1){
    //$mysqlL = "select `MID` from `".DBPREFIX.SPORT_FLUSH_MATCH_TABLE."` where `MID`=$gid-1 and Open=1 and MB_Team!=''";
...
if($gid%2 == 1){
	//$mysql = "select * from `".DATAHGPREFIX.SPORT_FLUSH_MATCH_TABLE."` where `MID`=$gid-1 and Open=1 and MB_Team!=''";
...
//	}
	...
	if ($w_m_rate==0 or $w_m_rate=='' or $grape==''){
		echo attention("$Order_This_match_is_closed_Please_try_again",$uid,$langx);
		exit;
	}
