...
var select_type = '' ; 
var playing = '' ; 
var tvwin = localStorage.getItem('iftvwin');
var lastidName = '';
var liveid = '';
...
function showBetList(){
    ...
    if(!playing)	document.getElementById("div_fake").style.display = "";
...
function showGameList(){
    ...
    //if(nolive_sw=="N") return;
    if(nolive_sw=="N"&&(!playing || (Gplaying == 0 && select_type == "gamelist"))) return;
    if(select_type=="gamelist"){
        showBetList();
        return;
    }
    select_type = "gamelist";
    ...
    */
    ...
    if(Gplaying == 0 && select_type=="gamelist")
    {
        document.getElementById("btn_game").className = "live_tvListBTN";
    }else if(Gplaying != 0 && select_type=="gamelist")
    {
        document.getElementById("btn_game").className = "live_tvListBTN_on";
    }else{}
...
function OpenTV_chgType(i,eventid){
    ...
    //console.log(top);
    betGtype = TV_gtype[0];
    //2017-09-28 Ricky 右邊TV跟左邊下注若選擇不同球類會破圖 , 因為 betGtype衝突到 , 所以改成用top._Gtype
    _Gtype = TV_gtype[0];
    select_type="betlist";
    nowPerformHash = null;
...
function playTV(i,eventid, isClick){
    ...
    if(isClick){
        if(ret["ret"]){
            ...
            playing = true;
            if(isOpen) opener.playing=playing;
        }
    }
}
...
function registVideo(videoVersion,videoCode){
    ...
    var base_url = util.getProtocal()+"//"+performUrl+"/app/member/live/";
...
function loadPerformPlayer(par){
    ...
    url = util.getProtocal()+"//"+performUrl;
...
function checkInitView(){
    if(is_init){
        ...
        if(select_type!="betlist")document.getElementById("time_list").style.display = "";
...
util.goToPage=function(filename, paramObj){
    ...
    if(paramObj.filename.indexOf(".php")!=-1){
        ...
        paramObj.filepath = "/tpl/member/"+langx+"/"+filename+".html";
        paramObj.method = "GET";
    }else{
...
/*function showOpenLive() {
    ...
    var url = "live_max.php?langx="+langx+"&uid="+uid+"&liveid="+liveid ;
    tvwin = window.open(url,"win","width=805,height=526,top=0,left=0,status=no,toolbar=no,scrollbars=yes,resizable=no,personalbar=no");
}*/

// 检测是视频窗口是否被关闭
function checkTvWinClose() {
    var loop = setInterval(function() {
        if(tvwin) { // 是否有打开tv 新窗口
            if (tvwin.closed) {
                // clearInterval(loop);
...
function GetVideoUnas(url,officeUrl){
    ...
    if (url != "") {
        ...
        //datas="+code+"&firstOpen="+firstOpen
...
function onloads(){
    ...
    //if(lastidName != "")
    if( typeof( lastidName ) != "undefined" && lastidName != "")
    {
        var temp = 'Bright(lastidName)';
        setTimeout(temp, 200);
    }
