...
(function ($) {
    ...
    $.fn.Page = function (options) {
        ...
        return this.each(function () {
            ...
            var str = '';
            var str1 = '<li><a href="javascript:" class="page_click '+ active +'" topage="1">1</a></li>';
            if (l > 1&&l < n+1) {
                for (i = 2; i < l + 1; i++) {
                    str += '<li><a href="javascript:" class="page_click" topage="'+ i +'">' + i + '</a></li>';
                }
            }else if(l > n){
                for (i = 2; i < n + 1; i++) {
                    str += '<li><a href="javascript:" class="page_click" topage="'+ i +'">' + i + '</a></li>';
            ...
            if(opts.hasFirstPage){
                dataHtml += '<div class="page_click first '+ (l==1?'none':'') +'" topage="1">' + opts.firstPage + '</div>';
            }
            dataHtml += '<ul class="pagingUl">' + str1 + str + '</ul>';

            if(opts.hasLastPage){
                dataHtml += '<div class="page_click last '+ (l==1?'none':'') +'" topage="'+ l +'">' + opts.lastPage + '</div>';
            }
            if(opts.hasPrv){
            ...
            function fpageShow(activePage){
                ...
                var str1 = '';
                for(i=0;i<n;i++){
                    str1 += '<li><a href="javascript:" class="page_click" topage="'+ (pageStart+i) +'">' + (pageStart+i) + '</a></li>'
                }
            ...
            function fpagePrv(index){
                ...
                if(l>n-1){
                    for(i=0;i<n;i++){
                        str1 += '<li><a href="javascript:" class="page_click" topage="'+ (i+1) +'">' + (i+1) + '</a></li>'
                    }
                }else{
                    for(i=0;i<l;i++){
                        str1 += '<li><a href="javascript:" class="page_click" topage="'+ (i+1) +'">' + (i+1) + '</a></li>'
            ...
            function fpageNext(index){
                ...
                if(l>n-1){
                    for(i=l-(n-1);i<l+1;i++){
                        str1 += '<li><a href="javascript:" class="page_click" topage="'+ i +'">' + i + '</a></li>'
                    }
                    obj.find('ul').html(str1);
                    obj.find('ul li').eq(index).find('a').addClass(active);
                }else{
