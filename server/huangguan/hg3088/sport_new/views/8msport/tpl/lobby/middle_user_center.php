...
<style>
    .bank_details{color:#bd4700;line-height:26px;margin:10px 0;padding:15px 0}
    .bank_details ul{width:50%;display:inline-block}
    .bank_details ul li{line-height:34px;margin-bottom:15px}
    .bank_details ul li span{text-align:right;display:inline-block;width:135px;padding:0 15px}
    .bank_details ul li span:last-child{text-align:left;width:188px;padding-left:10px;border:1px solid #ccc;border-radius:5px}
    .bank_details .nextBtn{text-align:left;display:inline-block;position:relative;top:-6px;left:105px;background:transparent;color:#333}
    .bank_details .band_btn_all{position:relative;display:inline-block;width:296px;height:136px;border:2px solid #ccc;border-radius:5px;margin-right:100px}
    .bank_details .band_btn_all:before{position:absolute;top:40px;left:35px;content:'';display:inline-block;width:60px;height:60px;background:url(<?php echo TPL_NAME;?>images/add_icon.png) center no-repeat}
        ...
        <div class="static-content">
           <!-- <div class="box-border profile-quicklook row">
            ...
            <div class="profile-info">
                <div class="tip_title"><span class="btn_game">1</span>个人资料</div>
                ...
                <div class="tip_title"><span class="btn_game">2</span>银行卡状态</div>
                <!-- 银行资料 开始-->
                <div class="bank_details">
                    <ul >
                        <li> <span> 银行卡账号：</span> <span class="show_bank_account"> <?php echo $_SESSION['Bank_Account']?returnBankAccount($_SESSION['Bank_Account']):$nobandtip ;?> </span></li>
                        <li> <span> 银行名称：</span> <span class="show_bank_name"> <?php echo $_SESSION['Bank_Name']?$_SESSION['Bank_Name']:$nobandtip ;?> </span></li>
                        <li> <span> 开户行地址：</span> <span class="show_bank_address"> <?php echo $_SESSION['Bank_Address']?$_SESSION['Bank_Address']:$nobandtip ;?> </span></li>
                    </ul>
                    <div class="band_btn_all fr">
                        <?php
                        if(!$Alias){
                            echo '<button class="change_user_details nextBtn" onclick="indexCommonObj.ifBindRealName(\''.$Alias.'\')">绑定个人资料</button>';
                        }else{
                            echo '<button class="change_user_bank nextBtn"> 更换您的银行卡 </button>';
                        }
                        ?>
                    </div>
    ...
    $(function () {
        ...
        function changePassWOrd() {
            $('.to_change_password').on('click',function () {
                ...
                var url = tplName+'/tpl/lobby/middle_change_password.php?type='+type ;
                var $title = '更改登录密码' ;
                if(type=='paypwd'){
                    $title = '更改支付密码' ;
                }
