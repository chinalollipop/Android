...
$sSql = "SELECT * FROM `".DBPREFIX."gxfcy_pay` WHERE `account_company` = 2 AND `status` = 1 AND `depositNum` <= $depositTimes AND FIND_IN_SET('{$_SESSION['pay_class']}',class) and has_company_youhui=1 and id =$pid";
...
foreach ($aData as $k => $v) {
    $minCurrency = bcmul(floatval($v['minCurrency']) , 1);
    $maxCurrency = bcmul(floatval($v['maxCurrency']) , 1);
    ...
    if($v['thirdpay_code'] == 'zb') { // 众宝
        $url = $v['url'] . '/'.$v['thirdpay_code'] .'/zhongbaopay.php';
        $aBanklist = array(
    ...
    if($v['thirdpay_code'] == 'wdf') { // 维多付
        $url = $v['url'] . '/'.$v['thirdpay_code'] .'/weiduofupay.php';
        $aBanklist = array(
    ...
    if($v['thirdpay_code'] == 'clzldz') { // 村里最靓的仔
        $url = $v['url'] . '/clzldzpay.php';
        $aBanklist = array(
            '920'=>'支付宝或网银',
        );
    }
    if($v['thirdpay_code'] == 'ccx') { // 璀璨星
        $url = $v['url'] . '/' . $v['thirdpay_code'] . '/ccxpay.php';
    ...
    if($v['thirdpay_code'] == 'csj') { // 创世纪
        $url = $v['url'] . '/'.$v['thirdpay_code'] .'/csjpay.php';
    ...
    if($v['thirdpay_code'] == 'autopay') { // autopay
        $url = $v['url'] . '/'.$v['thirdpay_code'] .'/AutoPaypay.php';
        $aBanklist = array(
            '5'=>'网银支付',
        );
    }
...
<div class="memberWrap">
    ...
    <form method="post" name="deposit_form" class="deposit_form" target="_blank" action="<?php echo $url;?>" onsubmit="return savePayAction()">
...
<!--                        <select name="pid" id="select_bank" onchange="getBanklist(this)">-->
                        <select name="pid" id="select_bank">
                            <?php
                            foreach ($aData as $k => $v){
                                ?>
                ...
                <tr>
                    ...
                    <td class="type_third" >
                        <select name="banklist" id="banklist_select">
                            <?php
                ...
                <tr>
                    ...
                    <td>
                        ...
                        <span class="tip"> <span class="left_s"><i class="caret"></i></span> 单笔限额存款为：<em><span id="min_money" class="min_money"><?php echo $minCurrency; ?></span> -<span id="max_money" class="max_money"><?php echo $maxCurrency; ?></span></em>元 </span>
                    </td>
                </tr>
                <tr>
                    <td>快速设置金额</td>
