 ...
 *时时彩走势图的绘画
 */
public class CQTrendChart extends ATrendChart {
    ...
    public CQTrendChart(Context context, LotteryTrendView lottoTrendView) {
    ...
    public void drawContent() {
        ...
        /*for (i = 0; i <= size-1; i++) {
            int i4 = i * this.mXItemHeight;
            TrendData trendData = (TrendData) this.mTrendData.get(i);
            //String[] splitRed = null;
            String[] split2 = trendData.getBlue().split(",");
            int i8 = this.mXItemHeight * i;
            if (i >= size - 4) {
                i8 += this.mDivHeight;
            }
            for (int is = 0; is < split2.length; is++) {
                if(!split2[is].equals("0")){
                    this.mRect.set(0, i4, i2, this.mXItemHeight + i4);
                    this.mPaint.setColor(this.mCAvgYilouBg);
                    beginRecording.drawRect(this.mRect, this.mPaint);
                    this.mPaint.setColor(this.mCDiv);
