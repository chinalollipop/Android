...
public class BindingCardFragment extends HGBaseFragment implements BindingCardContract.View {
    ...
    public void setEvents(@Nullable Bundle savedInstanceState) {
        ...
        if("1".equals(bincCard)){
            ...
            tvBindingCardSubmit.setText(getString(R.string.bcard_change_confirm));
        }else{
            tvBindingCardSubmit.setText(getString(R.string.bcard_confirm));
        }
        String aliasName = ACache.get(getContext()).getAsString(HGConstant.USERNAME_ALIAS);
        if(Check.isEmpty(aliasName)){
            tvBindingCardAccountName.setText(getString(R.string.bcard_user_no_alise));
        }else{
    ...
    private void onCheckWithdrawSubmit() {
        ...
        if(bankName.equals(getString(R.string.bcard_bank_name_hint))){
            showMessage(getString(R.string.bcard_bank_name_hint));
            return;
        }
        ...
        if("1".equals(bincCard)){
            ...
            if(Check.isEmpty(tvPwd2)||tvPwd2.length()<6){
                showMessage(getString(R.string.bcard_withdraw_pwd2_hint1));
                return;
            }

            if(Check.isEmpty(tvPwd2)){
                showMessage(getString(R.string.bcard_withdraw_pwd_hint2));
                return;
            }
